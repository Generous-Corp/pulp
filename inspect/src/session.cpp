...
bool contains(std::span<const InspectorCapability> capabilities, InspectorCapability capability) {
    return std::find(capabilities.begin(), capabilities.end(), capability) != capabilities.end();
}

void append_unique(std::vector<InspectorCapability>& capabilities, InspectorCapability capability) {
    if (!contains(capabilities, capability))
        capabilities.push_back(capability);
}

std::string authorization_data(std::string_view method, InspectorCapability capability,
                               InspectorProfile profile) {
    auto data = choc::value::createObject("");
    data.addMember("method", choc::value::createString(method));
    data.addMember("capability", choc::value::createString(capability_id(capability)));
    data.addMember("profile", choc::value::createString(profile_id(profile)));
    return choc::json::toString(data, false);
}

std::string lease_result_id(ControllerLeaseResult result) {
    switch (result) {
    case ControllerLeaseResult::Acquired:
        return "acquired";
    case ControllerLeaseResult::Renewed:
        return "renewed";
    case ControllerLeaseResult::HeldByOther:
        return "held_by_other";
    case ControllerLeaseResult::InvalidOwner:
        return "invalid_owner";
    }
    return "invalid_owner";
}

std::optional<InspectorMessage> validate_request_shape(const InspectorMessage& request) {
    const auto* method = find_inspector_method(request.method);
    if (request.id == 0 || (method && method->kind != InspectorMethodKind::Request)) {
        return make_error(request.id, "Inspector messages sent to a session must be requests",
                          "invalid_request");
    }
    return std::nullopt;
}

void deliver_scope_ends(const InspectorSessionInfo& info,
    ...
    for (auto& event : ended) {
        try {
            handler({info.session_id, info.instance_id, std::move(event.owner), event.reason});
        } catch (...) {
...
void append_audit(const std::shared_ptr<InspectorAuditLog>& audit_log,
                  const InspectorSessionInfo& info, std::string_view client_id,
                  const InspectorMessage& request, InspectorCapability capability,
                  InspectorAuditOutcome outcome, std::string error_code = {}) {
    if (!audit_log)
        return;
    audit_log->append({0, request.id, info.session_id, info.instance_id, std::string(client_id),
                       request.method, capability, outcome, std::move(error_code)});
}
...
InspectorAccessPolicy::InspectorAccessPolicy(InspectorPolicyConfig config)
    ...
    for (const auto capability : config.available_capabilities) {
        if (!capability_is_grantable(capability))
            continue;
        if (capability == InspectorCapability::RuntimeEval && !config.runtime_eval_enabled)
            continue;
    ...
    const bool can_acquire_controller = contains(available_, InspectorCapability::SessionControl) &&
    ...
    for (const auto capability : requested) {
        ...
        if (capability != InspectorCapability::SessionControl &&
            capability_requires_controller_lease(capability) && !can_acquire_controller) {
            continue;
        }
...
std::optional<InspectorMessage> InspectorAccessPolicy::authorize(const InspectorMessage& request,
    ...
    if (!method) {
        ...
        return make_error(request.id, "Unknown inspector method", "method_not_found",
    ...
    if (!capability_is_grantable(capability) || !is_available(capability)) {
        return make_error(request.id, "Inspector capability is unavailable",
                          "capability_unavailable", data);
    }
    if (!is_granted(capability)) {
        return make_error(request.id, "Inspector capability is not granted", "capability_denied",
                          data);
    }
    if (capability_requires_controller_lease(capability) && !owns_controller_lease) {
        return make_error(request.id, "A controller lease is required", "controller_lease_required",
                          data);
    }
    return std::nullopt;
}

InspectorControllerLease::InspectorControllerLease(std::chrono::milliseconds ttl, Clock clock)
    : ttl_(std::max(ttl, std::chrono::milliseconds(1))), clock_(std::move(clock)) {}
...
ControllerLeaseResult InspectorControllerLease::acquire(std::string_view owner) {
    if (owner.empty())
    ...
    return renewing ? ControllerLeaseResult::Renewed : ControllerLeaseResult::Acquired;
}

ControllerLeaseResult InspectorControllerLease::renew(std::string_view owner) {
    if (owner.empty())
        return ControllerLeaseResult::InvalidOwner;
    expire_if_needed();
...
bool InspectorControllerLease::release(std::string_view owner) {
    return release_with_reason(owner, TestInputReleaseReason::ControllerReleased);
}

bool InspectorControllerLease::release_with_reason(std::string_view owner,
...
void InspectorControllerLease::disconnect(std::string_view owner) {
    (void)release_with_reason(owner, TestInputReleaseReason::ClientDisconnected);
}
...
std::chrono::milliseconds InspectorControllerLease::remaining() {
    ...
    return std::max(std::chrono::milliseconds(0),
                    std::chrono::duration_cast<std::chrono::milliseconds>(expires_at_ - clock_()));
}
...
void InspectorControllerLease::end_operation(std::string_view owner) {
    if (active_operations_ == 0 || owner_.empty() || owner_ != owner)
        return;
    --active_operations_;
    if (active_operations_ == 0 && (release_pending_ || clock_() >= expires_at_)) {
        finish_scope(release_pending_ ? pending_reason_
...
std::vector<InspectorControllerLease::EndedScope> InspectorControllerLease::take_ended_scopes() {
...
class InspectorSession::State {
  public:
    State(InspectorSessionInfo session_info, RequestHandler request_handler,
          std::chrono::milliseconds lease_ttl, InspectorControllerLease::Clock clock)
        : info(std::move(session_info)), handler(std::move(request_handler)),
          lease(lease_ttl, std::move(clock)), expiry_thread([this](std::stop_token stop) {
              std::unique_lock lock(mutex);
              while (!stop.stop_requested()) {
                  const auto wait = lease.owner().has_value()
                                        ? std::max(lease.remaining(), std::chrono::milliseconds(1))
    ...
    bool begin_dispatch() {
        ...
        dispatch_cv.wait(lock, [this] { return !dispatch_accepting || !dispatch_active; });
        if (!dispatch_accepting)
            return false;
    ...
    void end_dispatch() {
        ...
        if (!dispatch_active || dispatch_owner != std::this_thread::get_id() ||
...
InspectorSession::InspectorSession(InspectorSessionInfo info, InspectorPolicyConfig policy,
                                   RequestHandler handler, std::chrono::milliseconds lease_ttl,
                                   InspectorControllerLease::Clock clock)
    : info_(std::move(info)), policy_(std::move(policy)),
      state_(std::make_shared<State>(info_, std::move(handler), lease_ttl, std::move(clock))) {}
...
void InspectorSession::set_main_thread_rpc(std::shared_ptr<InspectorMainThreadRpc> rpc) {
    std::lock_guard lock(state_->mutex);
    state_->main_thread_rpc = std::move(rpc);
}

void InspectorSession::set_controller_scope_end_handler(ControllerScopeEndHandler handler) {
    std::lock_guard lock(state_->mutex);
    state_->controller_scope_end_handler = std::move(handler);
}

void InspectorSession::set_audit_log(std::shared_ptr<InspectorAuditLog> audit_log) {
    std::lock_guard lock(state_->mutex);
...
InspectorMessage InspectorSession::handle_session_method(std::string_view client_id,
                                                         const InspectorMessage& request) {
    if (request.method == methods::kSessionGetCapabilities) {
        ...
        for (const auto capability : policy_.available_capabilities())
            available.addArrayElement(choc::value::createString(capability_id(capability)));
        auto effective = choc::value::createEmptyArray();
        for (const auto capability : policy_.effective_capabilities())
            effective.addArrayElement(choc::value::createString(capability_id(capability)));

        auto result = choc::value::createObject("");
        result.addMember("sessionId", choc::value::createString(info_.session_id));
        result.addMember("instanceId", choc::value::createString(info_.instance_id));
        result.addMember("pluginId", choc::value::createString(info_.plugin_id));
        result.addMember("protocolVersion", choc::value::createString(info_.protocol_version));
        result.addMember("profile", choc::value::createString(profile_id(policy_.profile())));
        result.addMember("available", available);
        result.addMember("effective", effective);
        if (const auto owner = state_->lease.owner())
            result.addMember("controller", choc::value::createString(*owner));
        return make_response(request.id, choc::json::toString(result, false));
    }

    const bool is_lease_method = request.method == methods::kSessionAcquireController ||
                                 request.method == methods::kSessionRenewController ||
                                 request.method == methods::kSessionReleaseController;
    if (!is_lease_method) {
        if (auto denied = policy_.authorize(request, false))
            return std::move(*denied);
        return make_error(request.id, "Unknown session method", "method_not_found");
    }
    ...
    if (!descriptor || !policy_.is_available(descriptor->capability)) {
        return make_error(request.id, "Inspector capability is unavailable",
                          ...
                          authorization_data(request.method, InspectorCapability::SessionControl,
                                             policy_.profile()));
    }
    ...
    if (request.method == methods::kSessionReleaseController) {
        if (!state_->lease.release(client_id)) {
            return make_error(request.id, "The caller does not own the controller lease",
    ...
    if (result == ControllerLeaseResult::HeldByOther) {
        return make_error(request.id, "The controller lease is held by another client",
                          "controller_lease_conflict");
    }
    if (result == ControllerLeaseResult::InvalidOwner) {
        return make_error(request.id, "Authenticated client identity is required",
                          "invalid_client_identity");
    }

    auto response = choc::value::createObject("");
    response.addMember("status", choc::value::createString(lease_result_id(result)));
    response.addMember("ttlMs", choc::value::createInt64(state_->lease.remaining().count()));
    return make_response(request.id, choc::json::toString(response, false));
}
