...
| Request/event delivery seam | `inspect/include/pulp/inspect/inspector_delivery.hpp` | Authenticated request identity plus broadcast and targeted-delivery callback contracts shared by domains and transport composition. | Domain helpers depend on these narrow contracts, not `InspectorServer` or `InspectorSession`; a future router may own delivery without changing domain behavior. |
