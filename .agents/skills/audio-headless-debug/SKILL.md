...
| Magnitude response / THD / phase + group delay from rendered buffers | `response_relative_to_input()`, `measure_thd()`, `measure_group_delay()` (gate on `defined_at(hz)` — a stopband reports undefined, and the delay accessors return NaN) | `pulp/audio/analysis/audio_spectrum.hpp` |
