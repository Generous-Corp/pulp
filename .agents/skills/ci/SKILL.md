...
- ctest's `<skipped message=...>` is the mechanism, not the reason. It reads
  ...
  `explicitly with message:`, which is where the summary step recovers it from.
  A row that shows the bare mechanism is a non-Catch2 skip, typically a script
  exiting `SKIP_RETURN_CODE`.
