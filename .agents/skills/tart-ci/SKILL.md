...
- **A lease denial can be a LIE, and it looks exactly like real contention.** The store is written when a VM or build acquires and released when it stops cleanly; nothing reconciles it against reality when the owner dies uncleanly (OOM, `kill -9`, host reboot, cancelled Actions job). On 2026-08-16 two `tart-macos-vm` leases held **12 of 14 cores on behalf of VMs that no longer existed** (`tart list` empty at the same moment), pinning a critical-path build to the `-j2` floor for ~25 min at 35% while the store read `0/14` minutes later; restarting it acquired the entitled 6-core lease and it finished in ~87 min — the first finishing build in five attempts, the previous four having `TIMEOUT`ed against the mac target's 7,200 s cap. **The floor is not the bug** (it is the melt guarantee from the July 2026 CPU/OOM incidents) and raising it reintroduces that; the bug is that nothing distinguishes *the host is busy* from *the store is stale*. `governed-build.sh` now prints every holder at denial — `id`, `kind`, `cores`, `owner`, `pid`, and **`alive=yes|no`** from a `kill(pid, 0)` probe — plus a "likely stale" line when any holder's process is gone. **`alive=no` means reap and restart, not wait**: an incremental `cmake --build` loses no objects. Note the wrapper acquires **once** and never re-negotiates, so a build that starts during a stale denial stays at the floor for its whole run even after the store recovers. The durable fix is reap-before-acquire inside `tartci leases acquire` (`leases reap --stale-secs` already exists and is scheduled by nothing) — that lives in the tartci repo, not here.

- **Shipyard's `local` mac backend builds IN THE CHECKOUT — do not edit a tree that is being built.** This is not the interrupted-build case `build-dir-sentinel.sh` covers; it is the inverse. A build that is *still running* while its sources are mutated does not fail cleanly: CMake keeps re-resolving a moving tree, the build crawls, and it dies on the lane's `timeout_secs` reporting `Validation timed out` — which names the target and not the cause. Observed 2026-08-16: an agent committed two merges into a worktree at 14:20 and 14:22 while `sy-20260816-b6bdc4` was building in it from 12:53; the run reached **11% in 2h3m** and timed out at 7,376s. The lease was fine (`cores=3`, first try), so this is a *second*, independent cause of the mac-lane timeout class — do not assume a clean lease line means the run is healthy. `governed-build.sh` now writes `.pulp-build-active` at the source-tree root for the life of a build (pid, start time, `-j`, lease, command), `tools/scripts/live_build_check.py` reads it, and `gates.sh` surfaces it on every push. **The pid is the load-bearing field**: the marker necessarily outlives a SIGKILL — the same property `build-dir-sentinel.sh` depends on — so presence proves nothing and `kill(pid, 0)` is what separates a live build from a dead one. If you see the warning, let the build finish or work in another worktree; if the pid is gone the marker is stale and safe to delete.
