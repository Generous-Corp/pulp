...
## Trimming a nested clip refuses when the content carries MIDI lanes

`sequence_content_lowerer.cpp` rebuilds a nested clip's content for the retained
window. For notes that is a filter, but a **controller lane has no correct trim**:
a point *outside* the window can be the value sounding *inside* it, so dropping it
changes what the controllers say and keeping it puts a point outside the clip.
Neither is right, and picking one silently is worse than refusing.

So a lane-bearing clip fails lowering with `InvalidStructure` rather than being
lowered without its lanes. Nothing shipped can author a lane yet, so this cannot
regress a real document — it converts a future silent loss into a named failure at
the point where the semantics must actually be decided.

**If you are implementing controller chase or expression semantics, this refusal
is your marker.** Resolving it means deciding what a trimmed controller stream
means — most likely synthesising a boundary point at the window edge from the last
value at or before it — not deleting the check.
