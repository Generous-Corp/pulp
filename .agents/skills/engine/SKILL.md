...
| `drawImage(img, …)` | 3 / 5 / 9-arg signatures supported. The 9-arg `(sx,sy,sw,sh,dx,dy,dw,dh)` form forwards the source rect to the bridge and renders through the source/destination-rect canvas path for sprite-sheet slicing. |
...
| `fillText(text,x,y)` / `strokeText` | `fillText` syncs global / text / fill state and forwards to `canvasFillText`. `strokeText` syncs stroke state and uses the dedicated `canvasStrokeText` bridge when present, with a fillText-style legacy fallback for older hosts. |
| `createLinearGradient` / `createRadialGradient` / `createConicGradient` | Return a `CanvasGradient` object with `_kind`, `_params`, `_stops`, and an `addColorStop(offset, color)` method. Gradients are NOT pushed to the bridge until they're assigned to `fillStyle` / `strokeStyle` AND a draw fires. Fill flushes via `canvasSetLinearGradient` / `canvasSetRadialGradient` / `canvasSetRadialGradientTwoCircles` / `canvasSetConicGradient`; strokeStyle mirrors that through the `canvasSetStroke*Gradient` bridge functions. |
| `fillStyle` / `strokeStyle` | Plain fields that accept CSS colours, CanvasGradient handles, or CanvasPattern handles. `_applyFillStyle()` and `_applyStrokeStyle()` flush the active style before each draw and clear stale gradient/pattern state when a later plain-colour assignment replaces it. |
...
| `createPattern` | Returns a CanvasPattern handle for string/image-like sources and the standard repetition values (`repeat`, `repeat-x`, `repeat-y`, `no-repeat`), or `null` for an empty source. The handle flushes via `canvasSetFillPattern` / `canvasSetStrokePattern`; backends that cannot resolve the image degrade visibly rather than throwing. |
