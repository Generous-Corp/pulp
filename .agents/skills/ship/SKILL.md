...
`release-cli.yml` also carries the **Tier-3 Intel gate** (`universal-arch-gate`, owned by the `intel-canary` skill): before the `release` job publishes, it builds the PulpGain example universal (`arm64;x86_64`), asserts `lipo -archs` + `codesign --verify` on every shipped bundle and embedded dylib via `tools/scripts/check_bundle_architectures.py --strict` (a raw lipo'd wgpu dylib fails `codesign --verify` and the arm64 slice is killed at load — always re-sign after lipo), and runs `auval` both natively and under `arch -x86_64`. **As of 2026-07-11 this gate is ADVISORY, not blocking**: it carries `continue-on-error: true` and is deliberately omitted from the `release` job's required `if:` conditions, because a known `auval` "Bad Max Frames" flake was wedging every publish. Re-blocking it (fixing the AU max-frames guard in `core/format/src/au_v2_adapter.cpp`, then restoring the hard `needs`/`if` requirement) is a tracked follow-up. See `docs/guides/intel-support.md`.

**Intel-Mac release slice — currently DISABLED (2026-07-11).** The native thin `darwin-x64` rows were removed from the build + smoke matrices. The GitHub-hosted `macos-15-intel` image CPU-pegs on a full CLI+SDK build (observed: a 71-min build cancelled at a 75-min cap, every run) and ships no artifact — and worse, its timeout **cancellation** (not a clean failure) makes `build-cli`'s matrix aggregate `cancelled`, which skips the `release` job entirely. The reliable x86_64-macOS path is the Tart Intel cross-build golden VM (arm64 host cross-compiling x86_64); Intel returns to the release through that lane. Until then Intel-Mac users source-build. **Load-bearing CI gotchas that caused a multi-hour release stall:**
...
- The `release` job's `if:` uses `always() && needs.build-cli.result == 'success' && needs.smoke-cli.result == 'success'` (NOT the implicit `success()` over all needs). `always()` forces evaluation so an advisory job (the arch gate) can't silently skip publish; the explicit `== 'success'` on the two real matrix jobs is what actually guards a broken build. A genuine build/smoke failure still blocks.
