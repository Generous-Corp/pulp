...
inline void stop(int) { stopping.store(true, std::memory_order_relaxed); }

inline std::shared_ptr<inspect::ControlSampleRegionTarget> editable_target(
    format::Processor& processor, state::StateStore& store) {
    auto* allpass = dynamic_cast<SampleRegionAllpassProcessor*>(&processor);
    if (!allpass || !allpass->ready()) return {};
...
inline std::unique_ptr<format::Processor> create_frozen() {
    ...
    if (!lowered.accepted || !lowered.plan) return {};
    ...
    if (!validated.accepted || !validated.plan || *validated.plan != *lowered.plan) return {};
    auto executable = host::load_baked(bytes, trust, host::BakedTypeRegistry::from({}));
    if (!executable.accepted || !executable.processor) return {};
    frozen_bytes = bytes; frozen_public_key = key->public_key;
...
inline std::shared_ptr<inspect::ControlSampleRegionTarget> frozen_target(
    format::Processor& processor, state::StateStore& store) {
    if (&processor != frozen_processor || !admitted_plan || !frozen_prepared) return {};
    auto target = inspect::ControlSampleRegionTarget::frozen(*admitted_plan, store, generation,
        [](std::uint32_t id, int maximum) {
            for (const auto& proof : admitted_proofs)
                if (proof.region_id == id && maximum == block_size) return proof;
            return host::SampleRegionProof{};
        }, [] { return frozen_prepared; });
    if (target) target->set_preparation_context(sample_rate, block_size);
    return target;
}

inline bool prepared_allpass(format::HeadlessHost& app) {
    audio::Buffer<float> input(1, block_size), output(1, block_size);
    input.clear(); input.channel(0)[0] = 1.0f;
    ...
    for (int frame = 0; frame < block_size; ++frame) {
        ...
        const double expected = coefficient * value + previous_input - coefficient * previous_output;
        if (!std::isfinite(out.channel(0)[frame]) || std::abs(out.channel(0)[frame] - expected) > 1.0e-6)
            return false;
        previous_input = value; previous_output = expected;
    }
    input.clear();
...
inline bool write_render_evidence(format::HeadlessHost& app, bool frozen) {
    ...
    try {
        ...
        for (int offset = 0; offset < 4096; offset += block_size) {
            input.clear();
            if (offset == 0) input.channel(0)[0] = 1.0f;
            auto out = output.view();
            app.process(out, std::as_const(input).view(), context);
            context.reset_requested = false;
            data.channels[0].insert(data.channels[0].end(), out.channel(0).begin(), out.channel(0).end());
        }
        if (!audio::write_wav_file((root / (stem + ".wav")).string(), data, audio::WavBitDepth::Float32)) return false;
        ...
        if (frozen) {
            if (!write("allpass.pulpbake", reinterpret_cast<const char*>(frozen_bytes.data()), frozen_bytes.size()) ||
                !write("publisher.pub", reinterpret_cast<const char*>(frozen_public_key.data()), frozen_public_key.size())) return false;
        } else {
            ...
            if (!write(stem + ".pulpgraph", graph_json.data(), graph_json.size())) return false;
        }
        ...
        receipt.setMember("state_generation", static_cast<std::int64_t>(app.state().state_generation()));
...
inline int run(format::ProcessorFactory factory,
               inspect::detail::StandaloneSampleRegionTargetFactory target_factory,
               bool frozen) {
    if (!inspect::detail::install_standalone_sample_region_target_factory(target_factory)) return 64;
    format::HeadlessHost app(factory);
    if (!app.valid() || !app.processor()) return 65;
    app.prepare(sample_rate, block_size, 1, 1);
    if (!prepared_allpass(app)) return 68;
    if (frozen) frozen_prepared = true;
    else {
        const auto* processor = dynamic_cast<SampleRegionAllpassProcessor*>(app.processor());
        if (!processor || !processor->ready()) return 66;
    }
    std::signal(SIGINT, stop); std::signal(SIGTERM, stop);
    auto control = inspect::make_control_standalone_host();
    if (!control || !control->start(*app.processor(), app.state(), nullptr, sample_rate)) return 67;
    std::uint64_t rendered_graph = 0, rendered_state = 0;
    while (!stopping.load(std::memory_order_relaxed) && control->ready()) {
        control->poll();
        if (rendered_graph != generation.value || rendered_state != app.state().state_generation()) {
            if (!write_render_evidence(app, frozen)) { control->stop(); return 69; }
            rendered_graph = generation.value; rendered_state = app.state().state_generation();
        }
