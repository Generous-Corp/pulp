...
public:
    ...
    void init(int sample_rate) override { sample_rate_ = sample_rate; }
    void instanceInit(int sample_rate) override { init(sample_rate); }
    void instanceConstants(int sample_rate) override { sample_rate_ = sample_rate; }
    void instanceResetUserInterface() override {}
    void instanceClear() override {}
    dsp* clone() override { return new InstalledGainDsp(*this); }
...
bool render_and_check(pulp::dsl::FaustProcessor<InstalledGainDsp>& processor,
                      int frames) {
    ...
    std::vector<const float*> input_ptrs{
        input.channel(0).data(), input.channel(1).data()};
    ...
    for (std::size_t channel = 0; channel < 2; ++channel) {
        for (int frame = 0; frame < frames; ++frame) {
            const float expected =
                input.channel(channel)[static_cast<std::size_t>(frame)] * 0.25f;
            if (std::abs(output.channel(channel)[static_cast<std::size_t>(frame)] -
                         expected) > 1e-7f) {
                return false;
            }
        }
...
int main() {
    ...
    if (!render_and_check(processor, 37)) return 1;
    if (!render_and_check(processor, 1024)) return 2;
    return 0;
}
