...
// same editor through create_view().
#include "spectral_lab.hpp"
#include <pulp/format/standalone.hpp>
#include <pulp/runtime/log.hpp>

int main() {
    pulp::format::StandaloneApp app(pulp::examples::create_spectral_lab);

    pulp::format::StandaloneConfig config;
    config.sample_rate = 48000.0;
    config.buffer_size = 256;
    config.input_channels = 2;     // Spectral Lab is an effect — freeze live input
    ...
    if (!app.run_with_editor(/*use_gpu=*/true)) {
        pulp::runtime::log_error("SpectralLab: failed to start standalone app");
        return 1;
    }
    return 0;
}
