...
| Verbatim transient relocation | **Deferred - gated** | Must beat baseline on transient-timing/pre-echo, seam-quality blocking tests, and blind A/B. The seam tests, A/B, and `rubberband` comparison cannot run headless. The checked-in baseline captures current onset behavior for when the gate can run. |
...
The engine ships with the current orchestrated quality path plus draft mode; the
gated refinements are queued, not abandoned.
