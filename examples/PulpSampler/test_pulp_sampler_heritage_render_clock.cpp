...
TEST_CASE("PulpSampler heritage render is bitwise callback-partition invariant",
    ...
    const auto mismatch = std::mismatch(contiguous.begin(), contiguous.end(),
                                        partitioned.begin(), partitioned.end());
    INFO("first mismatch frame: " <<
         std::distance(contiguous.begin(), mismatch.first));
...
TEST_CASE("PulpSampler typed bus runs once after each segment mix",
    ...
    constexpr std::array many{std::size_t{17}, std::size_t{63},
                              std::size_t{128}, std::size_t{304}};
...
TEST_CASE("PulpSampler routes rich typed voice blocks through native DSP",
    ...
    REQUIRE(contiguous != std::vector<float>(sample.begin(),
                                             sample.begin() + contiguous.size()));
}
...
TEST_CASE("PulpSampler credits tail frames consumed in the source-ending block",
    ...
    const auto remaining =
        PulpSamplerHeritageTestAccess::remaining_heritage_tail_frames(
            fixture.processor);
    REQUIRE(remaining > 0);
...
TEST_CASE("PulpSampler adapts typed streamed stereo to the legacy mono channel",
    ...
    REQUIRE(std::any_of(output.begin(), output.end(),
                        [](float sample) { return sample > 0.01f; }));
    REQUIRE(std::none_of(output.begin(), output.end(),
                         [](float sample) { return sample < -0.01f; }));
}
...
TEST_CASE("PulpSampler prepares typed voice and bus RNG continuation",
    ...
    auto& converter = std::get<audio::SampleHeritageVoiceConverterBlock>(
        voice.voice.front().parameters);
    converter.seed_policy =
        audio::SampleHeritageSeedPolicy::ContinueSerializedState;
    SamplerHeritageRuntime voice_runtime;
    REQUIRE(voice_runtime.configure(voice) ==
            PulpSamplerHeritageStatus::PendingPrepare);
    REQUIRE(voice_runtime.prepare(48000.0, 2, 128, 4.0) ==
            PulpSamplerHeritageStatus::Ready);
    ...
    auto& noise = std::get<audio::SampleHeritageBusNoiseIdleBlock>(
        bus.bus.front().parameters);
    noise.seed_policy =
        audio::SampleHeritageSeedPolicy::ContinueSerializedState;
    SamplerHeritageRuntime bus_runtime;
    REQUIRE(bus_runtime.configure(bus) ==
            PulpSamplerHeritageStatus::PendingPrepare);
    REQUIRE(bus_runtime.prepare(48000.0, 2, 128, 4.0) ==
            PulpSamplerHeritageStatus::Ready);
    ...
    REQUIRE(restart_runtime.configure(restart) ==
            PulpSamplerHeritageStatus::PendingPrepare);
    REQUIRE(restart_runtime.prepare(48000.0, 2, 128, 4.0) ==
            PulpSamplerHeritageStatus::Ready);
    REQUIRE_FALSE(restart_runtime.capture_runtime_state().valid());
}

TEST_CASE("PulpSampler rejects unsupported typed live stretch",
    ...
    audio::SampleHeritageProfile live{
        ...
        .voice = {{audio::SampleHeritageBlockDomain::Voice, false,
                   audio::SampleHeritageVoiceLiveCyclicStretchBlock{
                       1.0, 10.0, 1.0, true, false, 0, 0x1234u,
                       audio::SampleHeritageSeedPolicy::ContinueSerializedState}}},
    };
    SamplerHeritageRuntime live_runtime;
    REQUIRE(live_runtime.configure(live) ==
            PulpSamplerHeritageStatus::PendingPrepare);
    REQUIRE(live_runtime.prepare(48000.0, 2, 128, 4.0) ==
...
TEST_CASE("PulpSampler typed pitch factor one is the exact clean render path",
    ...
    for (const auto family : {audio::SampleHeritagePitchFamily::VariableClock,
        ...
        const auto latency = static_cast<std::size_t>(
            pitched.processor.latency_samples());
        REQUIRE(latency < pitched_output.size());
        REQUIRE(std::equal(pitched_output.begin() +
                               static_cast<std::ptrdiff_t>(latency),
...
TEST_CASE("PulpSampler typed pitch families keep simultaneous note clocks independent",
    ...
    for (const auto family : {audio::SampleHeritagePitchFamily::VariableClock,
        ...
        const auto latency = static_cast<std::size_t>(
            fixture.processor.latency_samples());
...
TEST_CASE("PulpSampler fixed-clock pitch families match their sample-index oracles",
    ...
    const auto drop_profile = typed_pitch_artifact_profile(
        audio::SampleHeritagePitchFamily::DropRepeat);
    ...
    const auto linear_profile = typed_pitch_artifact_profile(
        audio::SampleHeritagePitchFamily::EarlyLinear);
