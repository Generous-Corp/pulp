...
fn dispatch_start_passes_only_method_and_params() {
    ...
    assert_eq!(calls[0].2.as_deref(), Some("instance-a"));
    let rendered = String::from_utf8(output).unwrap();
    assert!(rendered.contains("tracing started"));
    assert!(rendered.contains("stop with: pulp trace stop --instance instance-a"));
...
fn dispatch_stop_prints_pftrace_path() {
    ...
    assert_eq!(talker.calls.borrow()[0].2.as_deref(), Some("instance-a"));
    assert_eq!(
