...
fn kit_and_content_help_flags_fall_through_to_pulp_cpp() {
    ...
    for command in ["kit", "content"] {
        for help_flag in ["--help", "-h"] {
            ...
            assert_eq!(
                code, 42,
                "expected stub exit 42 for {command} {help_flag}, got {code}"
            );
            let stdout = String::from_utf8(output.stdout).expect("utf8");
            assert!(
                stdout.contains(command) && stdout.contains(help_flag),
                "stub should echo delegated {command} {help_flag} argv; got: {stdout:?}"
            );
        }
    }
}
