...
| `project` (singular) | **Ported** | Phase 6b base plus the later upgrade/bump safety-rail slices. All flags now ported: positional, `--to`, `--all`, `--dry-run`, `--force-dirty`, `--allow-downgrade`, **`--allow-cli-skew`** (Slice A, refuse newer-than-CLI), **`--allow-redundant`** (Slice B, fail-open `origin/main` probe), `--verify-builds`. Standalone-mode bump (Slice C): `pulp.toml sdk_version` is source of truth, `find_package(Pulp X.Y.Z)` mirrored, `project(VERSION)` preserved as product version. Source-checkout refusal (Slice A) blocks running inside the Pulp framework tree. Undo records explicit `UndoEdit` list (Slice D) with backward-compat read of legacy single-edit batches. Migration-note rendering still stubbed with pointer to C++ binary (`cmd_project.cpp` links `migration_runtime.cpp`). **Slice C2 deferred:** managed-cache `sdk_path` rewrite needs the standalone-SDK resolution helpers from `cli_common.cpp` to land first. |
...
| `status` | **Ported** | Phase 6e — `GitProbe` trait drives `git -C <root>` branch + commit, standalone SDK detail (version, path-hint readiness, local + download caches, checkout hint), source-tree walk (`.cpp`/`.mm` + `.hpp`/`.h` counts, `test/*.cpp` count, `examples/*` dir count), plugin-format availability (VST3/AU/CLAP/AAX) with marker-file AAX SDK discovery plus host-OS gate. |
...
| `config` | **Ported** | Phase 6e — bare / `help` / `--help` / `-h` print usage + exit 0; `update.mode` changes clear `$PULP_HOME/update-snooze` best-effort for update-mode parity. |
...
| `search` | **Ported-partial** | Phase 6c — local-registry fuzzy search + JSON. `--refresh` recognized but **no-op** (the C++ path shells out to `curl`; Rust stub prints "no packages found" when refresh is requested on an empty registry). |
...
| 2026-04-24 | Project safety-rail slices | 7fbd7db1... | `git log abe2b07a..7fbd7db1 -- tools/cli/cmd_project.cpp tools/cli/project_bump.cpp tools/cli/project_bump.hpp tools/cli/cli_common.cpp tools/cli/cli_common.hpp` — diff = 7fbd7db1 itself ("feat: harden project SDK bump flow", +1082 −171 lines). | **Ports the upgrade + project-bump safety-rail spec.** **Slice A:** source-checkout refusal (`is_pulp_source_checkout` — CMakeLists + core/ + tools/cli + tools/shipyard.toml all present), `--allow-cli-skew` gate refusing target newer than installed CLI. **Slice B:** `--allow-redundant` flag + fail-open `origin/main` redundant-pin probe (`probe_origin_main_pin`, `git_show`, `parse_toml_string_value` helpers in `cmd::project`). **Slice C:** standalone-mode bump (`is_standalone_project`, `bump_one_standalone`) — pulp.toml `sdk_version` source-of-truth, `find_package(Pulp X.Y.Z)` mirror, `project(VERSION)` preserved. New `bump.rs` helpers: `find_find_package_pulp_version`, `find_toml_pin_site`. Atomic apply with rollback on mirror failure. **Slice D:** `UndoEdit` struct, `UndoEntry.{notes, edits}`, three new `PinKind` variants (`PulpTomlSdkVersion`, `PulpTomlSdkPath`, `CMakeFindPackagePulpVersion`); legacy single-edit `bump-undo-*.json` files still deserialize via `#[serde(default)]`. **Tests:** 30+ new (10 in `bump.rs`, 9 standalone-mode + 7 source-checkout/skew/redundant/edits in `cmd::project`). **Deferred to Slice C2:** conservative `sdk_path` managed-cache rewrite (needs shared standalone-SDK resolution helpers from `cli_common.cpp` to land first). Adds `tools/cli/cli_common.cpp`, `cli_common.hpp` to the watched files list (already implicitly mirrored as needed; codifying explicitly now). |
