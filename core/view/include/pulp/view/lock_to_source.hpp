...
/// Lock-to-source support for generated JS/TSX import artifacts.
///
...
///
/// Scope discipline: this path only handles generated import artifacts.
/// Live-runtime JSX patching (`jsx_lock.hpp`) and DESIGN.md token export
/// (`token_lock.hpp`) are separate mechanisms.
///
...
// remains a separate formatting pass.
...
///
/// `insert_after_anchor_id` carries the requested insertion slot: the anchor of
/// the sibling the moved block should physically follow under the new parent, or
/// empty to land as the new parent's first child. An empty / unresolved value
/// falls back to first-child so callers that do not set it are unaffected.
...
/// Heuristic guard for the generated-file boundary. Pulp's `generate_pulp_js`
/// emits `// Generated by Pulp import-design from <source>` as the first line;
/// an `// @generated` marker is also accepted for files that adopt the
/// conventional banner. Returns true when `source` looks like a Pulp-generated
/// import artifact safe to rewrite.
