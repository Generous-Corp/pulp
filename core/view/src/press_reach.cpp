...
bool ancestors_interactive(const View& view, const View& root) {
    for (const View* v = view.parent(); v != nullptr; v = v->parent()) {
        ...
        if (v->pointer_events() == View::PointerEvents::none) return false;
        if (v == &root) break;
    }
    return true;
}
...
std::string name_of(const View* view) {
    if (view == nullptr) return "(nothing)";
...
void walk(View& view, View& root, const PressReachOptions& options,
...
PressReach reach_of_press(View& root, Point root_pt, PressChannel channel) {
    ...
    for (View* v = out.hit; v != nullptr; v = v->parent()) {
        const bool carries = channel == PressChannel::click
                                 ? static_cast<bool>(v->on_click)
...
std::optional<Rect> rect_in_root(const View& view, View& root) {
    ...
    for (const View* node = &view; node != nullptr; node = node->parent()) {
        if (node == &root) { saw_root = true; break; }
        x += node->bounds().x;
        y += node->bounds().y;
        ...
        if (const auto* scroll = dynamic_cast<const ScrollView*>(node->parent()))
            { x -= scroll->scroll_x(); y -= scroll->scroll_y(); }
    }
    if (!saw_root) return std::nullopt;  // not in this tree
    ...
    if (candidate.width > 0.0f && candidate.height > 0.0f) {
        ...
        const Point local =
            point_to_local(centre, const_cast<View*>(&view), &root);
        if (!view.local_bounds().contains(local)) return std::nullopt;
    }
    return candidate;
}
...
void probe(View& view, View& root, PressChannel channel,
           const PressReachOptions& options, PressReachAudit& audit) {
    if (!advertises(view, channel)) return;
    ...
    if (painted->width < options.min_dimension ||
        painted->height < options.min_dimension) {
        return;
    }
    ...
    if (painted->width <= 0.0f || painted->height <= 0.0f) {
        ...
        finding.reason =
            "paints a zero-area box (" + std::to_string(painted->width) + "x" +
    ...
    const Point centre{painted->x + painted->width * 0.5f,
                       painted->y + painted->height * 0.5f};
    ...
    if (!is_self_or_descendant(reach.hit, &view)) {
        ...
        finding.reason = "a press at the centre of the rect it paints resolves "
                         "to " + finding.reached_id + ", which is outside its subtree";
        audit.unreachable.push_back(std::move(finding));
        return;
    }
    ...
    if (reach.handler == nullptr) {
        ...
        finding.reached_id = name_of(reach.hit);
        finding.reason =
            std::string("the press lands inside its subtree, on ") +
...
void walk(View& view, View& root, const PressReachOptions& options,
          PressReachAudit& audit) {
    if (options.audit_click) probe(view, root, PressChannel::click, options, audit);
    ...
    for (std::size_t i = 0; i < view.child_count(); ++i)
        if (auto* child = view.child_at(i)) walk(*child, root, options, audit);
}
...
std::string format_press_reach_audit(const PressReachAudit& audit) {
    ...
    for (const auto& f : audit.unreachable) {
        std::snprintf(line, sizeof(line),
                      "UNREACHABLE %-13s %s paints (%.1f,%.1f %.1fx%.1f); %s\n",
                      press_channel_name(f.channel), f.id.c_str(), f.painted.x,
                      f.painted.y, f.painted.width, f.painted.height,
                      f.reason.c_str());
        out += line;
    }
    ...
    std::snprintf(line, sizeof(line),
                  ...
                  audit.examined, audit.skipped_not_interactive,
                  audit.skipped_offscreen, audit.skipped_indeterminate,
                  audit.unreachable.size());
    out += line;
    return out;
}
