...
std::string js_single_quote_escape(const std::string& s) {
    ...
    for (unsigned char c : s) {
        switch (c) {
            ...
            // JS source must not contain literal U+2028 / U+2029 in string
            // literals (ECMA-262). They appear in editor.html-style content
            // (line/paragraph separator characters). Emit as \u escapes.
            default:
                if (c < 0x20) {
                    char buf[8];
...
static void generate_node(std::ostringstream& ss, const IRNode& node,
    ...
    // Text content (pulp #81 — escape for JS single-quoted string literal)
