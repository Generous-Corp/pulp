...
void WidgetBridge::register_svg_api() {
    ...
    register_bridge_function(api, "setSvgFill", [this](choc::javascript::ArgumentList args) {
        auto id = args.get<std::string>(0, "");
    ...
    register_bridge_function(api, "setSvgStroke", [this](choc::javascript::ArgumentList args) {
        auto id = args.get<std::string>(0, "");
        auto hex = args.get<std::string>(1, "");
        const bool clear = hex.empty() || hex == "none";
        if (auto* w = dynamic_cast<SvgPathWidget*>(widget(id))) {
