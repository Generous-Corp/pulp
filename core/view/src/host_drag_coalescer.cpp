...
void HostDragCoalescer::accumulate_movement_(const PointerSample& sample) {
    ...
    if (!sample.is_motion())
        return;
    if (!sample.pointer.has_movement_delta)
        return;
...
void HostDragCoalescer::apply_accumulated_movement_(std::vector<PointerSample>& samples) {
    for (auto& s : samples) {
        if (!s.is_motion())
            continue;
...
std::size_t HostDragCoalescer::deliver_all_(std::vector<PointerSample>& samples,
    ...
    for (const auto& s : samples) {
        ...
        PULP_TRACE_COUNTER("state", "delivered_drag_samples", stats_.delivered_samples);
        if (deliver)
            deliver(s);
    }
    return samples.size();
...
void HostDragCoalescer::set_frame_driver_running(bool running, const Deliver& deliver) {
    if (frame_driver_running_ == running)
        return;
...
HostDragCoalescer::SubmitResult HostDragCoalescer::submit(const PointerSample& sample,
                                                          const Deliver& deliver) {
    ++stats_.raw_samples;
    ...
    if (!due.empty())
        result.delivered = deliver_all_(due, deliver);
    accumulate_movement_(sample);
    stats_.merged_samples = coalescer_.total_merged();
