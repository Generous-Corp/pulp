...
std::vector<GridStyle::NamedArea> GridStyle::parse_template_areas(const std::string& css) {
    ...
    //   or "\"header header\" \"main aside\""  (double-quoted; equally valid CSS)
    // Each quoted segment is one row; cells are space-separated.
    ...
    std::vector<std::vector<std::string>> rows;
    {
        ...
        // Walk quoted runs — accept either single OR double quotes (both are
        // valid CSS for grid-template-areas; double-quoted is the more common
        // form when authors embed CSS strings inside JS templated styles).
        size_t i = 0;
        while (i < s.size()) {
            if (s[i] == '\'' || s[i] == '"') {
                char quote = s[i];
