...
///
/// ## Formant preservation — HONEST GAP, shipped as a reserved no-op
///
/// `set_formant_preserve` exists, is **off by default, and does nothing at
/// all** in this build — a test asserts that toggling it produces bit-identical
/// output. The dual-tap crossfade method resamples the waveform, so it moves
...
/// structurally cannot do it. A flag that is honestly inert and tested as inert
/// is better than one that half works.
///
...
public:
    ...
    /// **RESERVED — a no-op in this build.** Stored so the parameter exists and
    /// round-trips, and deliberately consulted by nothing: a test asserts that
    /// toggling it produces bit-identical output. See the file banner for why
    /// the crossfade-delay tier structurally cannot preserve formants.
    void set_formant_preserve(bool on) { formant_preserve_ = on; }
    bool formant_preserve() const { return formant_preserve_; }
