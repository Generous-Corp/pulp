...
///
/// The controls that matter are the ratio and the index envelope, not the
/// waveforms. The **ratio** decides *where* the partials land — integer ratios
/// stay harmonic and read as a tone, ratios near a simple fraction beat, and
/// irrational-ish ratios clang. The **index** decides *how many* there are, and
/// because it is enveloped separately from amplitude the spectrum collapses
/// while the note is still sounding: bright at the strike, simple in the tail.
/// That decoupling is what makes FM read as a struck object rather than as a
/// sustained synth tone, and it is the same decoupling the tom and the zap
/// rely on in their own domains.
///
    ...
    void render_add(float* out, int num_samples) override {
        for (int i = 0; i < num_samples; ++i) {
            ...
            const double carrier_hz =
                std::min(tune_hz_ * std::exp2(applied_bend_ * pitch), 0.49 * sample_rate());
            ...
            const double modulator =
                std::sin(2.0 * 3.14159265358979323846 * modulator_phase_ +
                         feedback_ * kFeedbackDepth * modulator_z_);
            modulator_z_ = modulator;

            carrier_phase_ += carrier_hz / sample_rate();
            if (carrier_phase_ >= 1.0) carrier_phase_ -= std::floor(carrier_phase_);
            const double carrier =
                std::sin(2.0 * 3.14159265358979323846 * carrier_phase_ + index * modulator);
            ...
            out[i] += static_cast<float>(
                output_.process(static_cast<float>(body + strike)) * velocity_gain_);
        }
    }
    ...
    void render_add(float* out, int num_samples) override {
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            for (std::size_t op = 0; op < operator_count; ++op) {
                ...
                current[op] = std::sin(2.0 * 3.14159265358979323846 * phases_[op] +
                                       modulation) *
