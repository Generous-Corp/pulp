...
bool id_less(const Clip& lhs, const Clip& rhs) noexcept { return lhs.id() < rhs.id(); }
...
runtime::Result<NoteContent, ModelError> NoteContent::create(std::vector<NoteEvent> notes) {
    ...
    if (const auto duplicate = first_duplicate(notes, [](const NoteEvent& note) { return note.id; }))
...
Clip::create_absolute(ItemId id, timebase::SamplePosition start, std::uint64_t sample_count,
    ...
    if (sample_count == 0 ||
        ...
        start.value > std::numeric_limits<std::int64_t>::max() -
...
ItemId Clip::id() const noexcept { return data_->id; }
...
const ClipTimeRange& Clip::time_range() const noexcept { return data_->range; }
...
timebase::TickPosition Clip::end() const noexcept { return start() + duration(); }
...
const ClipContent& Clip::content() const noexcept { return data_->content; }
...
struct ClipIndexNode {
    ...
    ClipIndexNode(Clip value, std::shared_ptr<const ClipIndexNode> lhs,
        ...
        : clip(std::move(value)), left(std::move(lhs)), right(std::move(rhs)),
          height(node_height), count(node_count) {
        ++g_live_index_nodes;
        ++g_created_index_nodes;
    }
    ~ClipIndexNode() { --g_live_index_nodes; }
};
...
std::uint8_t height(const NodePtr& node) noexcept { return node ? node->height : 0; }
std::size_t count(const NodePtr& node) noexcept { return node ? node->count : 0; }
...
NodePtr node(Clip clip, NodePtr left = {}, NodePtr right = {}) {
    ...
    return std::make_shared<const ClipIndexNode>(
        std::move(clip), std::move(left), std::move(right),
...
NodePtr insert(NodePtr root, Clip clip, Less less, bool& duplicate) {
    ...
    if (less(clip, root->clip))
        return balance(root->clip, insert(root->left, std::move(clip), less, duplicate), root->right);
    if (less(root->clip, clip))
        return balance(root->clip, root->left, insert(root->right, std::move(clip), less, duplicate));
    duplicate = true;
    return root;
}
...
runtime::Result<Track, ModelError> Track::create(ItemId id, std::string name,
    ...
    if (!clips.empty()) {
        ...
        for (const auto& clip : clips) {
            ...
            if (anchor == ClipTimeAnchor::Absolute &&
                clip.absolute_sample_rate() != absolute_rate)
...
ItemId Track::id() const noexcept { return data_->id; }
const std::string& Track::name() const noexcept { return data_->name; }
...
const Clip& Track::ClipView::Iterator::operator*() const noexcept { return select(root_, index_); }
const Clip* Track::ClipView::Iterator::operator->() const noexcept { return &select(root_, index_); }
...
Sequence::create(ItemId id, std::string name,
    ...
    std::sort(by_id.begin(), by_id.end(), [](const auto& lhs, const auto& rhs) {
        return lhs.first < rhs.first;
    });
    const auto duplicate = std::adjacent_find(by_id.begin(), by_id.end(), [](const auto& lhs,
                                                                             const auto& rhs) {
        return lhs.first == rhs.first;
    });
...
const Track* Sequence::find_track(ItemId id) const noexcept {
    const auto found = std::lower_bound(data_->track_id_index.begin(), data_->track_id_index.end(), id,
                                        [](const auto& entry, ItemId wanted) {
...
runtime::Result<Project, ModelError> Project::create(ProjectInput input) {
    ...
    const auto root = std::lower_bound(input.sequences.begin(), input.sequences.end(),
                                       input.root_sequence_id,
                                       [](const Sequence& sequence, ItemId id) {
            ...
            for (const auto& clip : track.clips()) {
                if (const auto* media = std::get_if<MediaRef>(&clip.content())) {
                    ...
                    if (found == input.assets.end() || found->id != media->asset_id)
                        return fail<Project>(ModelErrorCode::MissingAsset, clip.id(), media->asset_id);
    ...
    return runtime::Result<Project, ModelError>(runtime::Ok(Project(std::make_shared<const Data>(
        Data{input.id, std::move(input.name), input.next_item_id, input.root_sequence_id,
             std::move(input.assets), std::move(input.sequences)}))));
}

ItemId Project::id() const noexcept { return data_->id; }
const std::string& Project::name() const noexcept { return data_->name; }
std::uint64_t Project::next_item_id() const noexcept { return data_->next_item_id; }
ItemId Project::root_sequence_id() const noexcept { return data_->root_sequence_id; }
std::span<const MediaAsset> Project::assets() const noexcept { return data_->assets; }
std::span<const Sequence> Project::sequences() const noexcept { return data_->sequences; }
const MediaAsset* Project::find_asset(ItemId id) const noexcept {
    const auto found = std::lower_bound(data_->assets.begin(), data_->assets.end(), id,
                                        [](const MediaAsset& asset, ItemId wanted) {
                                            return asset.id < wanted;
                                        });
    return found != data_->assets.end() && found->id == id ? &*found : nullptr;
}
const Sequence* Project::find_sequence(ItemId id) const noexcept {
    const auto found = std::lower_bound(data_->sequences.begin(), data_->sequences.end(), id,
                                        [](const Sequence& sequence, ItemId wanted) {
...
std::optional<ItemId> IdRemapTable::find(ItemId old_id) const noexcept {
    const auto found = std::lower_bound(entries_.begin(), entries_.end(), old_id,
                                        [](const auto& entry, ItemId wanted) {
                                            return entry.first < wanted;
                                        });
...
std::optional<ModelError> finish_table(IdRemapTable& table) {
    ...
    const auto duplicate = std::adjacent_find(entries.begin(), entries.end(),
...
runtime::Result<Clip, ModelError> rebuild_clip(const Clip& clip, const IdRemapTable& table,
    ...
    if (clip.time_anchor() == ClipTimeAnchor::Musical)
        return Clip::create(*table.find(clip.id()), clip.start(), clip.duration(), std::move(content));
...
runtime::Result<Sequence, ModelError> rebuild_sequence(const Sequence& sequence,
...
runtime::Result<RemappedClip, ModelError> remap_ids(const Clip& clip,
    ...
    if (const auto table_error = finish_table(table))
        return fail<RemappedClip>(table_error->code, table_error->item,
...
runtime::Result<RemappedTrack, ModelError> remap_ids(const Track& track,
    ...
    if (error)
        return fail<RemappedTrack>(error->code, error->item, error->related_item);
    if (const auto table_error = finish_table(table))
        return fail<RemappedTrack>(table_error->code, table_error->item,
                                   table_error->related_item);
    auto rebuilt = rebuild_track(track, table, external);
    if (!rebuilt)
...
runtime::Result<RemappedProject, ModelError> remap_ids(const Project& project,
    ...
    struct Context { const IdRemapTable* table; } context{&table};
    const ExternalIdFixup internal{
        &context,
        [](void* raw, ItemId id) noexcept -> runtime::Result<ItemId, ModelError> {
    ...
    auto rebuilt = Project::create(ProjectInput{*table.find(project.id()), project.name(),
                                                 allocator.next_value(),
                                                 *table.find(project.root_sequence_id()),
                                                 std::move(assets), std::move(sequences)});
    if (!rebuilt)
        return fail<RemappedProject>(rebuilt.error().code, rebuilt.error().item,
                                     rebuilt.error().related_item);
