...
std::optional<ConflictCode> target_error(const Project& project, ItemId item,
    ...
    // Parent is canonical; the coordinate caches must agree with the expected
    // identity too. The caller supplies expected.parent_id via immediate_parent_id.
    if (location->parent_id != expected.parent_id ||
        location->sequence_id != expected.sequence_id ||
        location->track_id != expected.track_id || location->clip_id != expected.clip_id)
