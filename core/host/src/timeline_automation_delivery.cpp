...
// on a claimed node's maximum per-block automation burst fitting its
// fixed-capacity queue in a single append. Couple the two ceilings so a future
// edit cannot raise one without the other and silently reintroduce the drop.
static_assert(
    playback::AutomationPlaybackLimits::kMaximumEventsPerDevicePerBlock <=
        state::ParameterEventQueue::kCapacity,
    "exact automation delivery relies on a claimed node's max per-block burst "
    "fitting its fixed-capacity queue in one append; raise kCapacity if you "
    "raise the event ceiling");
...
TimelineAutomationDeliveryResult TimelineAutomationDelivery::deliver(
    ...
    for (const auto& batch : batches) {
        ...
        for (const auto& event : batch.events) {
            ...
            // Deliver at the ramp START and preserve its duration so the hosted
            // adapter glides across the ramp. Delivering the endpoint with a
            // zero duration would collapse the glide into an in-block step.
            if (!queue.push(
                    {event.param_id,
                     static_cast<std::int32_t>(event.sample_offset), event.value,
    ...
    for (const auto& batch : batches) {
        ...
        const auto mapping_index = static_cast<std::size_t>(mapping - mappings_.begin());
        if (!snapshot.inject_exact_parameter_events(mapping->plugin_node,
                                              *queues_[mapping_index],
                                              SnapshotParameterIngressPasskey{})) {
            ...
            for (std::size_t index = 0; index < published_count; ++index) {
                ...
                cleanup_ok = snapshot.inject_exact_parameter_events(
...
bool TimelineAutomationDelivery::clear(
    ...
    for (std::size_t index = 0; index < published_count_; ++index) {
        ...
        cleared = snapshot.inject_exact_parameter_events(
