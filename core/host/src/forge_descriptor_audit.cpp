...
const char* forge_audit_fault_name(ForgeAuditFault fault) {
    ...
    case ForgeAuditFault::invalid_curve:
...
audit_forge_descriptor(const ForgeNodeDescriptor& descriptor,
    ...
    for (const auto& param : descriptor.params) {
        ...
        const bool discrete = param.kind != ForgeParamKind::continuous;
        if (discrete && param.choices.empty())
            note(out, ForgeAuditFault::missing_choices, node, param.key,
                 "discrete parameter declares no named states or bits");
        if (!discrete && !param.choices.empty())
            note(out, ForgeAuditFault::choices_on_continuous, node, param.key,
                 "continuous parameter declares named states");

        if (found != baked_by_id.end()) {
            if (param.curve == ForgeParamCurve::logarithmic && found->second->min_value <= 0.0f) {
                note(out, ForgeAuditFault::invalid_curve, node, param.key,
                     "logarithmic mapping requires a positive baked minimum, got " +
                ...
                for (const auto& mode : choice.realization_modes) {
                    if (modes.find(mode) == modes.end())
                        note(out, ForgeAuditFault::unknown_realization, node, param.key,
                             "state '" + choice.token + "' is restricted to realization '" +
                                 std::string(mode) + "', which the node does not declare");
                }
            }
        }
...
std::vector<ForgeAuditFinding>
audit_forge_catalog(const std::vector<ForgeNodeDescriptor>& descriptors,
