#pragma once

#include <pulp/host/forge_drum_catalog_contract.hpp>

namespace pulp::host::forge_drum::detail {

struct DrumInstance {
    DrumInstance(EngineId selected, std::vector<state::ParamID> voice_ids)
        : id(selected), voice(signal::drum::create_engine(id)),
          voice_param_ids(std::move(voice_ids)) {
        applied_params.fill(std::numeric_limits<float>::quiet_NaN());
        observed_params.fill(std::numeric_limits<float>::quiet_NaN());
    }
    EngineId id;
    std::unique_ptr<signal::drum::Voice> voice;
    std::vector<state::ParamID> voice_param_ids;
    std::array<float, 300> applied_params;
    std::array<float, 300> observed_params;
    bool trigger_high = false;
    bool choke_high = false;
    int cymbal_hit_life = 1;
    int fm_transient = -1;
};

inline bool changed(DrumInstance& instance, state::ParamID id, float value) noexcept {
    auto& applied = instance.applied_params[static_cast<std::size_t>(id)];
    if (applied == value)
        return false;
    applied = value;
    return true;
}

inline bool voice_values_changed(DrumInstance& instance, const BakedParamView& params,
                                 std::int32_t offset) noexcept {
    bool any_changed = false;
    for (const state::ParamID id : instance.voice_param_ids) {
        const float value = params.value_at(id, offset);
        auto& observed = instance.observed_params[static_cast<std::size_t>(id)];
        if (observed != value) {
            observed = value;
            any_changed = true;
        }
    }
    return any_changed;
}

class FiniteParamView final : public BakedParamView {
  public:
    explicit FiniteParamView(const BakedParamView& source) : source_(source) {}

    float value_at(state::ParamID id, int32_t sample_offset) const override {
        const float value = source_.value_at(id, sample_offset);
        return std::isfinite(value) ? value : 0.0f;
    }

    float value(state::ParamID id) const override {
        const float result = source_.value(id);
        return std::isfinite(result) ? result : 0.0f;
    }

  private:
    const BakedParamView& source_;
};

inline void add(CustomNodeType& type, state::ParamID id, float minimum, float maximum,
                float initial) {
    type.baked_params.push_back({id, minimum, maximum, initial});
}

inline const char* type_id(EngineId id) noexcept {
    switch (id) {
    case EngineId::kick_oscillator:
        return kKickOscillatorTypeId;
    case EngineId::kick_resonant:
        return kKickResonantTypeId;
    case EngineId::kick_circuit:
        return kKickCircuitTypeId;
    case EngineId::snare:
        return kSnareTypeId;
    case EngineId::hat:
        return kHatTypeId;
    case EngineId::clap:
        return kClapTypeId;
    case EngineId::tom_generic:
        return kTomGenericTypeId;
    case EngineId::tom_simmons:
        return kTomSimmonsTypeId;
    case EngineId::cymbal_comb:
        return kCymbalTypeId;
    case EngineId::membrane_modal:
        return kMembraneTypeId;
    case EngineId::string_karplus_strong:
        return kStringTypeId;
    case EngineId::zap_cz:
        return kZapTypeId;
    case EngineId::fm2:
        return kFm2TypeId;
    case EngineId::fm6:
        return kFm6TypeId;
    case EngineId::fm8:
        return kFm8TypeId;
    case EngineId::dx7_msfa:
        return "";
    }
    return "";
}

inline signal::drum::OutputStage* output_stage(DrumInstance& instance) noexcept {
    using namespace signal::drum;
    switch (instance.id) {
    case EngineId::kick_oscillator:
    case EngineId::kick_resonant:
    case EngineId::kick_circuit:
        return &static_cast<KickVoice&>(*instance.voice).output();
    case EngineId::snare:
        return &static_cast<SnareVoice&>(*instance.voice).output();
    case EngineId::hat:
        return &static_cast<HatVoice&>(*instance.voice).output();
    case EngineId::clap:
        return &static_cast<ClapVoice&>(*instance.voice).output();
    case EngineId::tom_generic:
    case EngineId::tom_simmons:
        return &static_cast<TomVoice&>(*instance.voice).output();
    case EngineId::cymbal_comb:
        return &static_cast<CymbalVoice&>(*instance.voice).output();
    case EngineId::membrane_modal:
        return &static_cast<MembraneVoice&>(*instance.voice).output();
    case EngineId::string_karplus_strong:
        return &static_cast<StringVoice&>(*instance.voice).output();
    case EngineId::zap_cz:
        return &static_cast<ZapVoice&>(*instance.voice).output();
    case EngineId::fm2:
        return &static_cast<FmDrumVoice&>(*instance.voice).output();
    case EngineId::fm6:
        return &static_cast<Fm6DrumVoice&>(*instance.voice).output();
    case EngineId::fm8:
        return &static_cast<Fm8DrumVoice&>(*instance.voice).output();
    case EngineId::dx7_msfa:
        return nullptr;
    }
    return nullptr;
}

inline signal::drum::VelocityResponse default_velocity_response(EngineId id) noexcept {
    signal::drum::VelocityResponse response;
    response.level_db = 15.0f;
    switch (id) {
    case EngineId::kick_oscillator:
    case EngineId::kick_resonant:
    case EngineId::kick_circuit:
        response = {14.0f, 0.15f, 1.5f, 0.0f};
        break;
    case EngineId::snare:
        response = {16.0f, 0.08f, 1.2f, 0.25f};
        break;
    case EngineId::hat:
        response = {16.0f, 0.0f, 0.8f, 0.0f};
        break;
    case EngineId::clap:
        response = {15.0f, 0.0f, 0.7f, 0.0f};
        break;
    case EngineId::tom_generic:
    case EngineId::tom_simmons:
        response = {15.0f, 1.5f, 1.0f, 0.0f};
        break;
    case EngineId::cymbal_comb:
        response = {15.0f, 0.0f, 1.2f, 0.0f};
        break;
    case EngineId::membrane_modal:
        response = {16.0f, 0.15f, 1.6f, 0.0f};
        break;
    case EngineId::string_karplus_strong:
        response = {16.0f, 0.05f, 2.0f, 0.0f};
        break;
    case EngineId::zap_cz:
        response = {15.0f, 1.0f, 0.8f, 0.0f};
        break;
    case EngineId::fm2:
        response = {15.0f, 0.1f, 1.0f, 0.0f};
        break;
    case EngineId::fm6:
    case EngineId::fm8:
        response = {15.0f, 0.0f, 1.0f, 0.0f};
        break;
    case EngineId::dx7_msfa:
        break;
    }
    return response;
}

inline void add_common_params(CustomNodeType& type, EngineId id) {
    add(type, kTrigger, 0.0f, 1.0f, 0.0f);
    add(type, kVelocity, 0.0f, 1.0f, 1.0f);
    add(type, kChoke, 0.0f, 1.0f, 0.0f);
    add(type, kChokeMs, 0.01f, 100.0f, 4.0f);
    const auto response = default_velocity_response(id);
    add(type, kVelocityLevelDb, 0.0f, 48.0f, response.level_db);
    add(type, kVelocityBendOctaves, -6.0f, 6.0f, response.bend_octaves);
    add(type, kVelocityBrightnessOctaves, -6.0f, 6.0f, response.brightness_octaves);
    add(type, kVelocityNoiseBalance, 0.0f, 1.0f, response.noise_balance);
    add(type, kOutputDrive, 0.0f, 1.0f, 0.0f);
    add(type, kOutputFold, 0.0f, 1.0f, 0.0f);
    add(type, kOutputLevel, 0.0f, 4.0f, 1.0f);
    add(type, kOutputAhdEnabled, 0.0f, 1.0f, 0.0f);
    add(type, kOutputAttackMs, 0.0f, 2000.0f, 0.0f);
    add(type, kOutputHoldMs, 0.0f, 2000.0f, 0.0f);
    add(type, kOutputDecayMs, 0.01f, 8000.0f, 1000.0f);
    add(type, kOutputBits, 1.0f, 24.0f, 24.0f);
    add(type, kOutputHoldRateHz, 1.0f, std::numeric_limits<float>::max(),
        std::numeric_limits<float>::max());
    add(type, kOutputJitter, 0.0f, 1.0f, 0.0f);
    add(type, kOutputSmoothing, 0.0f, 1.0f, 0.0f);
    add(type, kOutputDeadZone, 0.0f, 0.9f, 0.0f);
}

inline void add_gate_params(CustomNodeType& type) {
    add(type, kGateRiseMs, 0.01f, 2000, 2);
    add(type, kGateFallMs, 0.01f, 8000, 200);
    add(type, kGateColour, 0, 1, 0.5f);
    add(type, kGateClosedHz, 10, 20000, 60);
    add(type, kGateOpenHz, 10, 20000, 12000);
    add(type, kGateGainExponent, 0.25f, 4, 1.5f);
}

inline void add_lofi_params(CustomNodeType& type, state::ParamID base) {
    add(type, base, 1, 24, 24);
    add(type, base + 1, 1, std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
    add(type, base + 2, 0, 1, 0);
    add(type, base + 3, 0, 1, 0);
    add(type, base + 4, 0, 0.9f, 0);
}

inline void add_voice_params(CustomNodeType& type, EngineId id) {
    switch (id) {
    case EngineId::kick_oscillator:
    case EngineId::kick_resonant:
    case EngineId::kick_circuit:
        add(type, kTuneHz, 20, 400, 55);
        add(type, kDecay, 10, 4000, 400);
        add(type, kPitchSweepOctaves, 0, 6, 2);
        add(type, kPitchSweepMs, 0.5f, 500, 30);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0, 2, 0.3f);      // click level
        add(type, kControl1, 20, 20000, 4000); // click tone
        add(type, kControl2, 0.05f, 200, 2);   // click decay
        add(type, kControl3, 0, 2, 0);         // noise level
        add(type, kControl4, 0.5f, 4000, 60);  // noise decay
        add(type, kControl5, 0, 2, 0);         // sub level
        if (id == EngineId::kick_oscillator) {
            add(type, kControl6, 0, 1, 0);      // triangle
            add(type, kControl7, 0, 8, 0);      // FM amount
            add(type, kControl8, 0.25f, 16, 1); // FM ratio
        }
        if (id == EngineId::kick_circuit) {
            add(type, kControl6, 0, 1, 0.85f); // feedback
            add(type, kControl7, 0, 1, 0.3f);  // drive
            add(type, kControl8, 0, 50, 4);    // attack
            add(type, kControl9, 0.2f, 20, 2); // pulse
            add(type, kControl10, 0, 1, 1);    // sigh
            add(type, kControl11, 0, 2, 2);    // hit life
            add(type, kCircuitC41, 1.0e-10f, 1.0e-6f, 15.0e-9f);
            add(type, kCircuitC42, 1.0e-10f, 1.0e-6f, 15.0e-9f);
            add(type, kCircuitR161, 100, 1.0e7f, 1.0e6f);
            add(type, kCircuitR165, 100, 1.0e7f, 47.0e3f);
            add(type, kCircuitR166, 100, 1.0e7f, 6.8e3f);
            add(type, kCircuitR167, 100, 1.0e7f, 1.0e6f);
            add(type, kCircuitR170, 100, 1.0e7f, 470.0e3f);
        }
        break;
    case EngineId::snare:
        add(type, kTuneHz, 60, 800, 180);
        add(type, kDecay, 5, 2000, 120);
        add(type, kPitchSweepOctaves, 0, 4, 0.5f);
        add(type, kPitchSweepMs, 0.5f, 300, 25);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 1, 4, 1.6f);
        add(type, kControl1, 0, 2, 0.5f);
        add(type, kControl2, 0, 8, 0);
        add(type, kControl3, 0, 1, 0);
        add(type, kControl4, 0, 2, 0.6f);
        add(type, kControl5, 5, 3000, 180);
        add(type, kControl6, 100, 18000, 3000);
        add(type, kControl7, 0.5f, 12, 1);
        add(type, kControl8, -4, 4, 0);
        add(type, kControl9, 0, 1, 0);
        add(type, kControl10, 5, 400, 45);
        add(type, kControl11, 0, 2, 0.4f);
        add(type, kControl12, 20, 20000, 6000);
        add(type, kControl13, 0.05f, 200, 4);
        add(type, kControl14, 0, 2, 0);
        add(type, kControl15, 1, 30, 12);
        add_lofi_params(type, kToneLofiBits);
        add_lofi_params(type, kNoiseLofiBits);
        break;
    case EngineId::hat:
        add(type, kTuneHz, 40, 4000, 320);
        add(type, kDecay, 5, 8000, 60);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0, 1, 1);
        add(type, kControl1, 0, 1, 0.8f);
        add(type, kControl2, 0, 1, 0);
        add(type, kControl3, 0.25f, 16, 1.4f);
        add(type, kControl4, 200, 18000, 7000);
        add(type, kControl5, 0.5f, 12, 1.2f);
        add(type, kControl6, 0, 1, 0);
        break;
    case EngineId::clap:
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 1, 8, 4);
        add(type, kControl1, 1, 60, 11);
        add(type, kControl2, 0.5f, 60, 6);
        add(type, kControl3, 0.2f, 1.5f, 0.82f);
        add(type, kControl4, 0, 1, 0.35f);
        add(type, kControl5, 0, 1, 0);
        add(type, kControl6, 0, 1, 1);
        add(type, kControl7, 0, 2, 0.35f);
        add(type, kControl8, 10, 3000, 180);
        add(type, kControl9, 200, 16000, 1400);
        add(type, kControl10, 0.5f, 12, 1.4f);
        add(type, kControl11, 0, 2, 0);
        add(type, kControl12, 40, 2000, 300);
        break;
    case EngineId::tom_generic:
    case EngineId::tom_simmons:
        add(type, kTuneHz, 30, 1200, id == EngineId::tom_generic ? 130 : 75);
        add(type, kDecay, 10, 4000, id == EngineId::tom_generic ? 450 : 700);
        add(type, kPitchSweepOctaves, 0, 6, id == EngineId::tom_generic ? .35f : .8f);
        add(type, kPitchSweepMs, 0.5f, 500, id == EngineId::tom_generic ? 30 : 45);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0, 1, 0);
        add(type, kControl1, 0, 1, id == EngineId::tom_generic ? .25f : .08f);
        add(type, kControl2, 50, 16000, id == EngineId::tom_generic ? 2200 : 1100);
        add(type, kControl3, 0, 1, .25f);
        add(type, kControl4, 0, 2, id == EngineId::tom_generic ? .35f : .30f);
        add(type, kControl5, 20, 20000, 4000);
        add(type, kControl6, 0.05f, 200, 2);
        break;
    case EngineId::membrane_modal:
        add(type, kTuneHz, 20, 2000, 90);
        add(type, kDecay, 20, 8000, 700);
        add(type, kControl0, 0, 1, 1);
        add(type, kControl1, 0, 1, 0);
        add(type, kControl2, 0, 1, 0.5f);
        add(type, kControl3, 0, 1, 0.5f);
        add(type, kControl4, 0.02f, 0.5f, 0.28f);
        add(type, kControl5, 0, 1, 0.1f);
        add(type, kControl6, 0.1f, 50, 1.5f);
        add(type, kControl7, 100, 18000, 6000);
        add(type, kControl8, 0, 1, 0);
        add(type, kControl9, 0, 2, 0);
        add(type, kControl10, 0, 2, 0);
        add(type, kControl11, 1, 500, 20);
        add(type, kControl12, 0, 2, 0);
        add(type, kControl13, 0.1f, 50, 2);
        add_gate_params(type);
        break;
    case EngineId::cymbal_comb:
        add(type, kTuneHz, 40, 2000, 320);
        add(type, kDecay, 50, 8000, 1800);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0.5f, 1, 0.93f);
        add(type, kControl1, -18, 18, 0);
        add(type, kControl2, 0, 1, 0.35f);
        add(type, kControl3, 0, 18, 4);
        add(type, kControl4, 0, 8000, 500);
        add(type, kControl5, 0, 1, 1);
        add(type, kControl6, 0, 1, 0.4f);
        add(type, kControl7, -400, 400, 45);
        add(type, kControl8, 0, 2, 0.7f);
        add(type, kControl9, 0, 2, 0.3f);
        add(type, kControl10, 0.5f, 200, 6);
        add(type, kControl11, 500, 20000, 12000);
        add(type, kControl12, 20, 2000, 300);
        add(type, kControl14, 0, 2, 1);
        break;
    case EngineId::string_karplus_strong:
        add(type, kTuneHz, 30, 4000, 220);
        add(type, kDecay, 0.05f, 20, 2);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0, 1, 0.3f);
        add(type, kControl1, 0, 1, 0);
        add(type, kControl2, 0, 0.5f, 0.25f);
        add(type, kControl3, 0.1f, 50, 1);
        add(type, kControl4, 200, 18000, 1800);
        add(type, kControl5, 0, 0.99f, 0);
        add(type, kControl6, 0, 1, 0);
        add(type, kControl7, 0, 3, 0);
        add(type, kControl8, 0, 1, 0);
        add(type, kControl9, 0.125f, 16, 2);
        add(type, kControl10, 0, 2, 0.25f);
        add(type, kControl11, 0, 1, 0);
        add_gate_params(type);
        break;
    case EngineId::zap_cz:
        add(type, kTuneHz, 20, 4000, 220);
        add(type, kDecay, 10, 4000, 350);
        add(type, kPitchSweepOctaves, 0, 6, 2.5f);
        add(type, kPitchSweepMs, 0.5f, 500, 35);
        add(type, kControl0, 0, 4, 2);
        add(type, kControl1, 0, 1, 0.8f);
        add(type, kControl2, 0.5f, 500, 60);
        add(type, kControl3, 1, 32, 12);
        add(type, kControl4, 0, 100, 9);
        add(type, kControl5, 0, 1, 0);
        add(type, kControl6, 0.25f, 16, 1.5f);
        add_gate_params(type);
        break;
    case EngineId::fm2:
        add(type, kTuneHz, 20, 4000, 110);
        add(type, kDecay, 10, 4000, 400);
        add(type, kPitchSweepOctaves, 0, 6, 0);
        add(type, kPitchSweepMs, 0.5f, 500, 30);
        add(type, kNoiseColor, 0, 4, 0);
        add(type, kControl0, 0.1f, 24, 1.41f);
        add(type, kControl1, 0, 24, 6);
        add(type, kControl2, 0.5f, 2000, 40);
        add(type, kControl3, 0, 1, 0);
        add(type, kControl4, 0, 25, 0);
        add(type, kControl5, 0, 25, 0);
        add(type, kControl6, 0, 1, 0);
        add(type, kControl7, 0, 1, 0);
        add(type, kControl8, 0.5f, 2000, 40);
        add(type, kControl9, 0.5f, 2000, 40);
        add(type, kControl10, 0.05f, 40, 5);
        add(type, kControl11, 0, 2, 0);
        add(type, kControl12, 0, 4000, 0);
        add(type, kControl13, 0, 4000, 0);
        add(type, kControl14, 0, 1, 0);
        add(type, kControl15, -1, 23, -1);
        add(type, kControl16, 0, 2, 0);
        add(type, kControl17, 0.5f, 500, 10);
        add(type, kControl18, 40, 18000, 12000);
        add(type, kControl19, 0.5f, 12, 0.8f);
        add(type, kControl20, 0, 1, 0);
        add(type, kControl21, 0, 2, 0.2f);
        add(type, kControl22, 20, 20000, 6000);
        break;
    case EngineId::fm6:
    case EngineId::fm8: {
        const int count = id == EngineId::fm6 ? 6 : 8;
        add(type, kTuneHz, 20, 4000, 110);
        add(type, kControl0, 0, id == EngineId::fm6 ? 31 : 15, id == EngineId::fm6 ? 4 : 9);
        add(type, kControl1, 0, 12, 3);
        add(type, kControl2, 40, 18000, id == EngineId::fm6 ? 3000 : 2000);
        add(type, kControl3, 0.5f, 12, id == EngineId::fm6 ? 0.9f : 1);
        if (id == EngineId::fm6) {
            add(type, kPitchSweepOctaves, -6, 6, 0);
            add(type, kPitchSweepMs, 0.5f, 1000, 40);
            add(type, kControl4, 0, 1, 0);
        } else {
            add(type, kNoiseColor, 0, 4, 0);
            add(type, kControl4, -1, 23, -1);
            add(type, kControl5, 0, 2, 0);
            add(type, kControl6, 0.5f, 500, 10);
            add(type, kControl7, 0, 2, 0);
        }
        constexpr float ratios6[] = {1, 1, 2, 3, 4, 7};
        constexpr float levels6[] = {1, .8f, .7f, .6f, .5f, .4f};
        constexpr float decays6[] = {400, 260, 180, 120, 80, 50};
        constexpr float ratios8[] = {1, 1.41f, 2, 3.17f, 4, 5.63f, 7, 9.41f};
        constexpr float levels8[] = {1, .8f, .7f, .6f, .5f, .4f, .35f, .3f};
        constexpr float decays8[] = {400, 220, 160, 110, 80, 60, 45, 30};
        for (int op = 0; op < count; ++op) {
            add(type, kOperatorRatioBase + op, 0.1f, 32,
                id == EngineId::fm6 ? ratios6[op] : ratios8[op]);
            add(type, kOperatorLevelBase + op, 0, 1,
                id == EngineId::fm6 ? levels6[op] : levels8[op]);
            add(type, kOperatorDecayBase + op, 1, 4000,
                id == EngineId::fm6 ? decays6[op] : decays8[op]);
            if (id == EngineId::fm8) {
                add(type, kOperatorFeedbackBase + op, 0, 1, 0);
                add(type, kOperatorWaveBase + op, 0, 25, 0);
            }
        }
        break;
    }
    case EngineId::dx7_msfa:
        break;
    }
}

inline void apply_common(DrumInstance& instance, const BakedParamView& params, std::int32_t offset);
inline void apply_voice(DrumInstance& instance, const BakedParamView& params, std::int32_t offset);

} // namespace pulp::host::forge_drum::detail

#include <pulp/host/detail/forge_drum_catalog_processing.hpp>

namespace pulp::host::forge_drum::detail {

inline CustomNodeType make_drum_node_impl(EngineId id) {
    const auto metadata =
        std::find_if(signal::drum::engine_registry.begin(), signal::drum::engine_registry.end(),
                     [id](const auto& item) { return item.id == id; });
    CustomNodeType type;
    if (metadata == signal::drum::engine_registry.end() || !metadata->available)
        return type;
    type.type_id = type_id(id);
    type.version = 1;
    type.num_input_ports = 0;
    type.num_output_ports = 2;
    type.default_name = std::string(metadata->display_name);
    type.lowerable = true;
    type.destroy = [](void* p) { delete static_cast<DrumInstance*>(p); };
    type.prepare = [](void* p, double sample_rate, int /*max_block*/) {
        auto& instance = *static_cast<DrumInstance*>(p);
        instance.voice->set_output_oversampling(signal::drum::OutputOversampling::bypass);
        instance.voice->prepare(sample_rate);
    };
    type.reset = [](void* p) {
        auto& instance = *static_cast<DrumInstance*>(p);
        instance.voice->reset();
        instance.trigger_high = false;
        instance.choke_high = false;
    };
    add_common_params(type, id);
    add_voice_params(type, id);
    std::vector<state::ParamID> voice_param_ids;
    for (const auto& parameter : type.baked_params) {
        if ((parameter.id >= kTuneHz && parameter.id <= kNoiseColor) || parameter.id >= kGateRiseMs)
            voice_param_ids.push_back(parameter.id);
    }
    type.create = [id, voice_param_ids]() -> void* {
        return new DrumInstance(id, voice_param_ids);
    };
    type.process_instance_baked_param = [](void* opaque, audio::BufferView<float>& output,
                                           const audio::BufferView<const float>& /*input*/,
                                           int num_samples, const BakedParamView& params) {
        auto& instance = *static_cast<DrumInstance*>(opaque);
        const FiniteParamView finite_params(params);
        float* left = output.channel_ptr(0);
        float* right = output.channel_ptr(1);
        for (int sample = 0; sample < num_samples; ++sample) {
            const auto offset = static_cast<std::int32_t>(sample);
            apply_common(instance, finite_params, offset);
            if (voice_values_changed(instance, finite_params, offset))
                apply_voice(instance, finite_params, offset);
            const bool trigger = finite_params.value_at(kTrigger, offset) >= 0.5f;
            if (trigger && !instance.trigger_high)
                instance.voice->note_on(finite_params.value_at(kVelocity, offset));
            instance.trigger_high = trigger;
            const bool choke = finite_params.value_at(kChoke, offset) >= 0.5f;
            if (choke && !instance.choke_high)
                instance.voice->choke(finite_params.value_at(kChokeMs, offset));
            instance.choke_high = choke;
            left[static_cast<std::size_t>(sample)] = 0.0f;
            right[static_cast<std::size_t>(sample)] = 0.0f;
            instance.voice->process_stereo(left + sample, right + sample, 1);
        }
    };
    return type;
}

} // namespace pulp::host::forge_drum::detail
