...
// The product-owned source generated by PulpInspectorShipping.cmake defines
// this symbol. Referencing it from the composition root makes direct linkage
// of the exported inspector archive fail at link time: every runnable endpoint
// must carry the exact adjacent manifest and capability markers generated by
// pulp_add_plugin(... SHIP_INSPECTOR ...).
...
StandaloneInspectorRuntime::create(StandaloneApp& app, Processor& processor, ViewBridge& bridge,
                                   ...
                                   bool runtime_eval_enabled) {
    pulp_inspector_shipping_declaration_v1();
        ...
        for (const auto& capability : requested_capabilities) {
            ...
            if (!strict_custom_profile && parsed_capability &&
                !standalone_inspector_capability_available(
                    *parsed_capability, standalone_capture_available(root, window),
                    runtime_eval_enabled))
                continue;
