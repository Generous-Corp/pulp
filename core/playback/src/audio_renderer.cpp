...
template <typename T>
runtime::Result<T, AudioRendererError> fail(AudioRendererErrorCode code,
                                            timeline::ItemId item = {},
                                            timeline::ItemId related = {},
...
float source_sample(const audio::AudioFileData& source, std::size_t output_channel,
                    std::size_t output_channels, std::uint64_t frame,
                    std::uint64_t next_frame, float fraction) noexcept {
...
std::optional<std::int64_t> scale_position(std::int64_t value,
                                           timebase::RationalRate from,
                                           timebase::RationalRate to) noexcept {
    const auto scaled = static_cast<long double>(value) * to.as_long_double() /
        from.as_long_double();
...
std::optional<std::uint64_t> scale_duration_ceil(std::uint64_t value,
                                                 timebase::RationalRate from,
                                                 timebase::RationalRate to) noexcept {
    const auto scaled = static_cast<long double>(value) * to.as_long_double() /
        from.as_long_double();
...
bool valid_transport(const TransportSnapshot& transport, std::size_t output_frames,
    ...
    if (transport.frame_count != output_frames || transport.frame_count == 0 ||
        ...
        transport.range_count > transport.ranges.size() ||
        transport.tempo_map != &tempo_map ||
...
void render_track(const AudioTrackRendererProgram& track, const TransportRange& range,
    ...
    for (; clip != clips.end() && clip->timeline_start < range_end; ++clip) {
        const auto media_end = clip->timeline_start + static_cast<std::int64_t>(
        ...
        if (overlap_end <= overlap_start)
            continue;
        const auto relative_start = static_cast<std::uint64_t>(
        ...
        for (std::size_t channel = 0; channel < output.num_channels(); ++channel) {
            ...
            for (std::size_t frame = 0; frame < count; ++frame) {
                ...
                const auto source_position = static_cast<long double>(relative) *
                    static_cast<long double>(clip->source_frames_per_timeline_frame);
                const auto source_offset = std::min(
...
DecodedAudioAssetPool::create(std::vector<DecodedAudioAsset> assets,
    ...
    return runtime::Ok(std::shared_ptr<const DecodedAudioAssetPool>(
        new DecodedAudioAssetPool(std::move(assets))));
}
...
DecodedAudioAssetPool::decode_wav(timeline::ItemId id, std::span<const std::uint8_t> bytes,
    ...
    return runtime::Ok(DecodedAudioAsset{
        id, std::make_shared<const audio::AudioFileData>(std::move(*decoded))});
}

const DecodedAudioAsset* DecodedAudioAssetPool::find(timeline::ItemId id) const noexcept {
    const auto found = std::lower_bound(assets_.begin(), assets_.end(), id,
        [](const DecodedAudioAsset& asset, timeline::ItemId wanted) {
...
compile_audio_clip_program(const timeline::Clip& clip, const timeline::Project& project,
    ...
    if (!media)
        return fail<AudioClipRendererProgram>(AudioRendererErrorCode::InvalidClipRange,
                                              clip.id());
    ...
    if (!valid_audio(*decoded->audio, limits))
        return fail<AudioClipRendererProgram>(AudioRendererErrorCode::InvalidAsset,
                                              clip.id(), media->asset_id);
    ...
    const auto renderable_frames = scale_duration_ceil(media->frame_count,
    ...
    if (clip.time_anchor() == timeline::ClipTimeAnchor::Musical) {
        ...
        const auto fade_in_tick = clip.start() + timebase::TickDuration{
            static_cast<std::int64_t>(playback.fade_in_duration)};
        const auto fade_out_tick = clip.end() - timebase::TickDuration{
            static_cast<std::int64_t>(playback.fade_out_duration)};
        ...
        if (clip.absolute_sample_rate().as_long_double() > 768'000.0L)
            return fail<AudioClipRendererProgram>(AudioRendererErrorCode::UnsupportedSampleRate,
                                                  clip.id(), media->asset_id);
        const auto projected_start = scale_position(clip.absolute_start().value,
                                                     clip.absolute_sample_rate(), timeline_rate);
        const auto projected_end = scale_position(clip.absolute_end().value,
                                                   clip.absolute_sample_rate(), timeline_rate);
        const auto fade_in_position = clip.absolute_start().value +
            static_cast<std::int64_t>(playback.fade_in_duration);
        const auto fade_out_position = clip.absolute_end().value -
            static_cast<std::int64_t>(playback.fade_out_duration);
        const auto projected_fade_in_end = scale_position(
            fade_in_position, clip.absolute_sample_rate(), timeline_rate);
        const auto projected_fade_out_start = scale_position(
            fade_out_position, clip.absolute_sample_rate(), timeline_rate);
        if (!projected_start || !projected_end || !projected_fade_in_end ||
            !projected_fade_out_start || *projected_end < *projected_start ||
            *projected_fade_in_end < *projected_start ||
            *projected_fade_out_start > *projected_end)
            return fail<AudioClipRendererProgram>(AudioRendererErrorCode::UnsupportedSampleRate,
                                                  clip.id(), media->asset_id);
    ...
    return runtime::Ok(AudioClipRendererProgram{
        clip.id(), media->asset_id, decoded->audio, timeline_start, timeline_frames,
        source_start, media->frame_count, *renderable_frames,
...
link_audio_track_program(timeline::ItemId track_id,
    ...
    if (clips.size() > limits.max_clips)
        return fail<std::shared_ptr<const AudioTrackRendererProgram>>(
            AudioRendererErrorCode::CapacityExceeded, track_id, {}, clips.size(),
            limits.max_clips);
    ...
    for (std::size_t index = 1; index < clips.size(); ++index) {
        if (clips[index - 1].id == clips[index].id)
            return fail<std::shared_ptr<const AudioTrackRendererProgram>>(
                AudioRendererErrorCode::InvalidIdentity, clips[index].id);
    }
...
AudioRenderStatus ArrangementAudioRenderer::process(const PlaybackProgram& program,
    ...
    limits.max_block_frames = std::min(limits.max_block_frames,
                                       compiled_limits.max_block_frames);
    limits.max_asset_frames = std::min(limits.max_asset_frames,
                                       compiled_limits.max_asset_frames);
    ...
    for (const auto& track : program.tracks()) {
        ...
        const auto clip_count = static_cast<std::uint64_t>(
            track->audio_program()->clips().size());
