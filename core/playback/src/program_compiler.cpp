...
class ProgramCompilerTask final : public CompileTask {
  ...
  private:
    ...
    std::vector<ControllerProgramEvent> current_controller_events_;
    ...
    while (std::chrono::steady_clock::now() < budget.deadline && work < budget.max_work_units) {
        ...
        if (stage_ == Stage::CompileTracks) {
            ...
            if (!clip_started_) {
                if (role == detail::ClipContentRole::Notes) {
                    ...
                    // nested sequence. Its controller lanes are lowered beside
                    // its notes, so a clip that authors both compiles to a
                        ...
                        for (const auto& point : lane.points) {
                            if (current_controller_events_.size() >= max_controller_events)
                                return fail({CompileErrorCode::MidiExpressionLaneBudgetExceeded,
                                             lane.id, request_->document_revision});
                            // Points are authored relative to the clip; the
                            // program is absolute, and the sample is what the
                            // renderer schedules against.
                            const auto tick =
                                clip.start() + timebase::TickDuration{point.position.value};
                            current_controller_events_.push_back(
                                {request_->tempo_map->ticks_to_samples(tick), tick, clip.id(),
                                 lane.id, point.id, lane.address, point.value,
                                 point.chased ? ControllerProgramEventOrigin::Chased
                                                        : ControllerProgramEventOrigin::Authored});
                        }
                    }
                }
        ...
        if (stage_ == Stage::FinalizeTrack) {
            ...
            tracks_.push_back(std::shared_ptr<const TrackProgram>(new TrackProgram(
                ...
                std::move(current_controller_events_), std::move(current_note_modifiers_),
