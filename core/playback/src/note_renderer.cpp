...
bool ArrangementNoteRenderer::prepare(std::size_t maximum_events_per_block) {
    if (maximum_events_per_block == 0) return false;
...
bool ArrangementNoteRenderer::append_physical(midi::MidiEvent midi1,
                                               midi::UmpPacket midi2,
...
bool ArrangementNoteRenderer::emit(const NoteProgramEvent& event,
    ...
    auto midi1 = event.kind == NoteProgramEventKind::Off
        ? midi::MidiEvent::note_off(event.channel, event.pitch)
        : midi::MidiEvent::note_on(event.channel, event.pitch,
                                   midi1_velocity(event.velocity));
    ...
    if (!append_physical(std::move(midi1), std::move(midi2), sample_offset)) return false;
    if (event.kind == NoteProgramEventKind::Off)
        active = 0;
    else
        active = 1;
...
NoteRenderResult ArrangementNoteRenderer::process(
    const PlaybackProgramBlock& block, const TransportSnapshot& transport) noexcept {
    ...
    if (!valid_ranges(transport)) {
        ...
        return {dropped_events_ == 0 ? NoteRenderCode::InvalidTransport
                ...
                ShellAdoptionResult::Missing,
                static_cast<std::uint32_t>(output_.size()),
                dropped_events_};
    }
    ...
    if (!view.program) {
        ...
        result.code = view.adoption == ShellAdoptionResult::Rejected
            ? NoteRenderCode::AdoptionRejected : NoteRenderCode::MissingProgram;
        result.emitted_events = static_cast<std::uint32_t>(output_.size());
        result.dropped_events = dropped_events_;
        if (result.dropped_events != 0) result.code = NoteRenderCode::OutputOverflow;
        return result;
    }

    const bool block_sequence_reset = has_block_index_ &&
        transport.block_index != last_block_index_ + 1;
    last_block_index_ = transport.block_index;
    has_block_index_ = true;
    if (pending_flush_ || view.adoption == ShellAdoptionResult::Adopted ||
        block_sequence_reset) {
        if (!flush(0)) {
    ...
    if (!transport.is_playing) {
        if (!flush(0)) result.code = NoteRenderCode::OutputOverflow;
        result.emitted_events = static_cast<std::uint32_t>(output_.size());
        result.dropped_events = dropped_events_;
        update_carry_state(transport, 0);
        return result;
    ...
    for (std::uint8_t range_index = 0; range_index < transport.range_count; ++range_index) {
        ...
        for (; cursor != events.end(); ++cursor) {
            ...
            if (!emit(*cursor, block_offset)) {
                if (!state_overflow_) pending_flush_ = true;
                break;
            }
            last_cursor = static_cast<std::int64_t>((cursor - events.begin()) + 1);
        }
        if (pending_flush_) break;
    }
