...
export function applyTypographyProp(
    ...
    switch (key) {
        ...
        // calls `setTextShadow` defensively; the bridge does not register the
        // aggregate shadow function yet, so the missing call is no-op-safe
        // through `call`. Each per-attribute setter writes one slot in
