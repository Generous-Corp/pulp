...
- Refill: opened #1238 from
  ...
  limited to `test/test_phase9_widgets.cpp`.
...
- Local-only progress: `pulp-docs-generate-coverage-643` refreshed
  cleanly at `a063d891` with 15 docs-generator tests,
  `tools/docs_generate.py check`, `tools/check-docs.sh`, focused 100%
  coverage for `tools/docs_generate.py`, and diff checks green.
- Local-only progress: `pulp-mkdocs-hooks-coverage-643` refreshed cleanly
  at `478a2d88` with 10 hook tests, focused 100% coverage for
  `tools/mkdocs_hooks.py`, sync checks, and diff checks green.
...
- Local-only progress: `pulp-phase3-app-framework-edges-493` refreshed
  at `d2267257` with targeted diff-cover mirror passing, direct
  `pulp-test-app-framework "[view]"` passing 101 assertions in 27 test
  cases, and clean status. The machine still has only about 2.4 GiB free,
  so broad coverage/full CI was not run locally, but this focused tranche is
  queue-ready.
...
| --- | --- | --- | --- | --- | --- |
| `feature/phase3-docs-generate-coverage-643` | `a063d891` | #643 tooling tranche for `tools/docs_generate.py` paths | `tools/scripts/test_docs_generate.py` | Rebased onto current `origin/main` at `8a9eadc4`; `python3 tools/scripts/test_docs_generate.py` reports 15 tests; `python3 tools/docs_generate.py check` reports `docs-generate: OK`; `tools/check-docs.sh` passed with existing warning-only messages; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_docs_generate.py` passed and reported 100% for `tools/docs_generate.py`; diff-cover exited 0 with no reportable covered source lines because the branch only changes the test file; `git diff --check origin/main...HEAD` and tracked status clean. Branch is local-only and not pushed for this refreshed head. | Ready to queue when a slot opens; push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `feature/mkdocs-hooks-coverage-643` | `478a2d88` | #643 tooling tranche for `tools/mkdocs_hooks.py` paths | `tools/scripts/test_mkdocs_hooks.py` | Rebased onto current `origin/main` at `8a9eadc4`; `python3 tools/scripts/test_mkdocs_hooks.py` reports 10 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_mkdocs_hooks.py` passed and reported 100% for `tools/mkdocs_hooks.py` with 55 statements, 0 misses, 24 branches, and 0 partials; skill-sync, docs-sync, version-bump, and compat-sync reports passed; `git diff --check origin/main...HEAD`, `git diff --exit-code`, and tracked status clean. Branch is local-only and not pushed for this refreshed head. | Ready to queue when a slot opens; push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-app-framework-edges-493` | `d2267257` | #493 view tranche for `core/view/src/app_framework.cpp` shortcut, key mapping, menu, toolbar, and settings edges | `test/test_app_framework.cpp` | Rebased cleanly onto `origin/main` at `69bbe8a4`; targeted mirror `PATH="$(dirname "$(xcrun -f llvm-cov)"):$PATH" tools/scripts/local_diff_cover.sh pulp-test-app-framework` passed the diff-cover threshold and reported no lines with coverage information because the branch only changes test code; the unrelated all-`ctest` tail was stopped when it reached iOS configure while disk was dropping. Final-head direct validation passed: `cmake --build build-cov --target pulp-test-app-framework` and `LLVM_PROFILE_FILE=... build-cov/test/pulp-test-app-framework "[view]"` reported 101 assertions in 27 test cases. Final status clean and ahead 1; only about 2.4 GiB local disk free, so broad coverage/full CI was not run locally. | Ready to queue when a slot opens; rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`. |
