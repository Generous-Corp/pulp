...
- GitHub Actions pressure: #1231, #1238, #1239, #1240, and #1241 have PR-event
...
- Local-only progress: `pulp-mkdocs-hooks-coverage-643` refreshed cleanly
  at `d51f599e` with 10 hook tests, focused 100% coverage for
...
| `feature/mkdocs-hooks-coverage-643` | `d51f599e` | #643 tooling tranche for `tools/mkdocs_hooks.py` paths | `tools/scripts/test_mkdocs_hooks.py` | Rebased cleanly onto `origin/main` at `f6e092ba`; `python3 tools/scripts/test_mkdocs_hooks.py` reports 10 tests; `uv run --with 'coverage>=7.10' python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_mkdocs_hooks.py` passed and reported 100% for `tools/mkdocs_hooks.py` with 55 statements, 0 misses, 24 branches, and 0 partials; tracked status clean with diff limited to `tools/scripts/test_mkdocs_hooks.py`. System Python lacked `coverage.py >= 7.10`, so the successful coverage run used `uv`. | Ready to queue when a slot opens; push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-app-framework-edges-493` | `f5a44955` | #493 view tranche for `core/view/src/app_framework.cpp` shortcut, key mapping, menu, toolbar, and settings edges | `test/test_app_framework.cpp` | Rebased cleanly onto current `origin/main` at `f6e092ba` after #1237. Earlier validation on the previous head passed the targeted diff-cover mirror and direct `pulp-test-app-framework "[view]"` with 101 assertions in 27 test cases, but the latest rebased head could not be revalidated quickly because both `build/` and `build-cov/` had been removed from this worktree. Current diff is limited to `test/test_app_framework.cpp`; tracked status clean. | Hold local-only until the focused app-framework target is rebuilt and rerun on this latest head; then rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`. |
...
| `local/phase3-asset-manager-coverage-493` | `9bd0fea1` | #493 view tranche for `core/view/src/asset_manager.cpp` file-backed shader/blob cache and cached `@2x` image behavior | `test/test_asset_manager.cpp` | #1125 is merged as `f1db8c29`; this branch was rebased cleanly onto `origin/main` at `f6e092ba` and remains useful as a follow-up. Conflict resolution kept the merged #1125 tests and added only the extra local coverage. Validation passed: `cmake -S . -B build-asset -DCMAKE_BUILD_TYPE=Debug -DPULP_ENABLE_GPU=OFF -DPULP_BUILD_EXAMPLES=OFF -DFETCHCONTENT_SOURCE_DIR_MBEDTLS=/Users/danielraffel/Library/Caches/Pulp/fetchcontent-src/mbedtls-v3.6.2-tar`; `cmake --build build-asset --target pulp-test-asset-manager -j$(sysctl -n hw.ncpu)`; `./build-asset/test/pulp-test-asset-manager` passed 75 assertions in 29 test cases; `ctest --test-dir build-asset -R "AssetManager|asset-manager|asset_manager" --output-on-failure` passed 24/24; `git diff --check origin/main..HEAD` passed. Final status clean and ahead 1. | Ready to queue when a slot opens; rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`. |
