...
| --- | --- | --- | --- | --- | --- |
| `local/phase3-docs-generate-coverage-643` | `bedd2ea0` | #643 tooling tranche for `tools/docs_generate.py` paths | `tools/scripts/test_docs_generate.py` | Refreshed against current `origin/main` at `e60cc129`; `python3 tools/scripts/test_docs_generate.py` reports 13 tests; `python3 tools/scripts/test_run_python_coverage.py` reports 23 tests; isolated-venv focused `run_python_coverage.py --pattern 'tools/scripts/test_docs_generate.py'` passed 13 tests and generated summary/html/Cobertura outputs; skill-sync report; version-bump report; `git diff --check`. System `python3` lacked `coverage.py >= 7.10`, so the worker used an isolated temporary venv for focused coverage. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-list-limitations-coverage-643` | `1d26d06f` | #643 tooling tranche for `tools/list_limitations.py` paths | `tools/scripts/test_list_limitations.py` | Refreshed against current `origin/main`; `python3 tools/scripts/test_list_limitations.py` reports 8 tests; `python3 tools/test_list_limitations.py` reports 6 tests; `python3 tools/scripts/test_run_python_coverage.py` reports 23 tests; CI-style local venv focused `run_python_coverage.py --pattern tools/scripts/test_list_limitations.py` passed 8 tests and reported 98% for `tools/list_limitations.py`; skill-sync report; version-bump report; `git diff --check`. System `python3` lacked `coverage.py >= 7.10`, so the worker created `build-coverage/python-venv` for focused coverage. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
...
| `local/phase3-check-docs-consistency-coverage-643` | `2e61b545` | #643 tooling tranche for `tools/check-docs-consistency.py` paths | `tools/scripts/test_check_docs_consistency.py` | Refreshed against current `origin/main` at `e60cc129`; `python3 tools/scripts/test_check_docs_consistency.py` reports 9 tests; `python3 tools/scripts/test_run_python_coverage.py` reports 23 tests; focused `run_python_coverage.py --pattern tools/scripts/test_check_docs_consistency.py` passed 9 tests and reported 98% for `tools/check-docs-consistency.py`; skill-sync report; version-bump report; `git diff --check`. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
