...
Last live check: 2026-05-01 19:12:21 EDT.
...
- GitHub Actions pressure: #1227-#1229 have PR-event Build/Coverage
  checks active or queued on Namespace-backed lanes and no failed checks.
  Main changed after #1226 merged, so re-check mergeability for #1227-#1229
...
- Local-only progress: `pulp-skill-sync-extra-643` advanced to
  `9d7f83be327b` with remaining helper, reporting, and entrypoint
...
- Local-only progress: `pulp-lcov-cobertura-extra-643` advanced to
  `000ee1cc1d7d` with zero-hit line, demangler, missing `c++filt`, and
...
| `local/phase3-cmajor-external-extra-643` | `57879ed2` | #643 tooling tranche for `tools/scripts/cmajor_external.py` edges | `tools/scripts/test_cmajor_external_extra.py` | Rebased cleanly onto `origin/main` at `2623a4b4`; `python3 tools/scripts/test_cmajor_external.py` passed with 4 tests; `python3 tools/scripts/test_cmajor_external_extra.py` passed with 9 tests; focused coverage passed and reported 100% for `tools/scripts/cmajor_external.py` with 119 statements, 0 misses, 32 branches, and 0 partials; `python3 tools/scripts/cmajor_external.py doctor --patch examples/cmajor-gain/CmajorGain.cmajorpatch` passed and reported expected missing external `cmaj`; dry-run `generate --target cpp` passed; skill-sync, docs-sync, CLI sync, coverage-tier, and diff checks passed; final tracked status clean. Temporary coverage venv was outside the repo. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-resolve-runs-on-extra-643` | `ac4e9157` | #643 tooling tranche for `tools/scripts/resolve_runs_on.py` edges | `tools/scripts/test_resolve_runs_on_extra.py` | Rebased cleanly onto `origin/main` at `26155037`; `python3 tools/scripts/test_resolve_runs_on.py` passed with 18 tests; `python3 tools/scripts/test_resolve_runs_on_extra.py` passed with 2 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_resolve_runs_on.py --pattern tools/scripts/test_resolve_runs_on_extra.py` passed and reported 100% for `tools/scripts/resolve_runs_on.py` with 94 statements, 0 misses, 40 branches, and 0 partials; docs-sync report passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-merge-cobertura-extra-643` | `ac4552ec` | #643 tooling tranche for `tools/scripts/merge_cobertura.py` edges | `tools/scripts/test_merge_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `26155037`; `python3 tools/scripts/test_merge_cobertura.py` passed with 14 tests; `python3 tools/scripts/test_merge_cobertura_extra.py` passed with 3 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_merge_cobertura.py --pattern tools/scripts/test_merge_cobertura_extra.py` passed and reported 100% for `tools/scripts/merge_cobertura.py` with 121 statements, 0 misses, 48 branches, and 0 partials; CLI sync passed with existing warnings only; skill-sync report passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-lcov-cobertura-extra-643` | `000ee1cc` | #643 tooling tranche for `tools/scripts/lcov_cobertura.py` zero-hit, demangler, missing-tool, and entrypoint edges | `tools/scripts/test_lcov_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `26155037`; `python3 tools/scripts/test_lcov_cobertura.py` passed with 8 tests; `python3 tools/scripts/test_lcov_cobertura_extra.py` passed with 11 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_lcov_cobertura.py --pattern tools/scripts/test_lcov_cobertura_extra.py` passed and reported 100% for `tools/scripts/lcov_cobertura.py` with 243 statements, 0 misses, 74 branches, and 0 partials; docs-sync and skill-sync reports passed; CLI sync passed with existing warnings only; `git diff --check origin/main...HEAD` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
