...
| `local/phase3-mkdocs-hooks-coverage-643` | `8f45836f` | #643 tooling tranche for `tools/mkdocs_hooks.py` paths | `tools/scripts/test_mkdocs_hooks.py` | Refreshed against current `origin/main` at `9c43e9fe`; `python3 tools/scripts/test_mkdocs_hooks.py` reports 6 tests; `python3 tools/scripts/test_run_python_coverage.py` reports 23 tests; focused `run_python_coverage.py --pattern tools/scripts/test_mkdocs_hooks.py` passed 6 tests and reported 96% for `tools/mkdocs_hooks.py`; skill-sync report; version-bump report; `git diff --check`. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
