...
Last live check: 2026-05-01 20:15:20 EDT.
...
- GitHub Actions pressure: #1231, #1233, and #1234 have PR-event
  Build/Coverage checks active or queued on Namespace-backed lanes and no
  failed checks. #1231 is closest to merge-ready at the latest check.
- Local-only progress: `pulp-check-docs-status-extra-643` refreshed cleanly
  at `e749e572` with 10 format-validation tests, 36 related coverage/docs
  tests, and 100% target coverage for `tools/check_format_validation.py`.
...
| `local/phase3-check-docs-status-extra-643` | `e749e572` | #643 tooling tranche for `tools/check_format_validation.py` paths | `tools/test_check_format_validation.py`, `tools/scripts/run_python_coverage.py`, `tools/scripts/test_run_python_coverage.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `python3 -m unittest tools/test_check_format_validation.py tools/test_check_status_ladder.py tools/scripts/test_run_python_coverage.py` passed with 46 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/test_check_format_validation.py` passed and reported 100% for `tools/check_format_validation.py` with 83 statements, 0 misses, 38 branches, and 0 partials; docs-sync report passed via the existing coverage.md bypass trailer; `PYTHONPATH=tools/scripts python3 -m unittest tools/scripts/test_docs_sync_check.py tools/scripts/test_docs_sync_check_extra.py` passed with 28 tests; the combined focused regression command passed with 63 tests; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. `check_format_validation.py --mode report` still exits 1 on 4 existing production_validated gaps and `check_status_ladder.py --mode report` still exits 1 on 18 existing status-ladder violations, both unrelated to this tranche. | Hold local-only while the current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `feature/phase3-resolve-runs-on-extra-643` | `42f24313` | #643 tooling tranche for `tools/scripts/resolve_runs_on.py` edges | `tools/scripts/test_resolve_runs_on_extra.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `python3 tools/scripts/test_resolve_runs_on.py` passed with 18 tests; `python3 tools/scripts/test_resolve_runs_on_extra.py` passed with 2 tests; `uv run --with 'coverage>=7.10' python3 tools/scripts/run_python_coverage.py --pattern tools/scripts/test_resolve_runs_on.py --pattern tools/scripts/test_resolve_runs_on_extra.py` passed and reported 100% for `tools/scripts/resolve_runs_on.py` with 94 statements, 0 misses, 40 branches, and 0 partials; docs-sync report passed; `git diff --check origin/main...HEAD` passed; `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows` created #1234, then exited with no local targets remaining as expected for the Namespace-only route; #1234 is labeled `codecov` and linked from #641/#643. The focused coverage run emitted one no-data warning before the first test output, but completed with exit code 0 and produced the target 100% row. | Queued: monitor #1234 and merge once required gates are green. |
| `local/phase3-merge-cobertura-extra-643` | `f40c0485` | #643 tooling tranche for `tools/scripts/merge_cobertura.py` edges | `tools/scripts/test_merge_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `.venv/bin/python tools/scripts/test_merge_cobertura.py` passed with 14 tests; `.venv/bin/python tools/scripts/test_merge_cobertura_extra.py` passed with 3 tests; `.venv/bin/python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_merge_cobertura.py --pattern tools/scripts/test_merge_cobertura_extra.py` passed and reported 100% for `tools/scripts/merge_cobertura.py` with 121 statements, 0 misses, 48 branches, and 0 partials; docs-sync report passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. The focused coverage run intentionally touched only the merge-cobertura test patterns, so unrelated Python tooling rows show 0% in the aggregate report. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-lcov-cobertura-extra-643` | `6ec8d60e` | #643 tooling tranche for `tools/scripts/lcov_cobertura.py` zero-hit, demangler, missing-tool, and entrypoint edges | `tools/scripts/test_lcov_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `uv run python3 tools/scripts/test_lcov_cobertura.py` passed with 8 tests; `uv run python3 tools/scripts/test_lcov_cobertura_extra.py` passed with 11 tests; `uv run --with 'coverage>=7.10' --with PyYAML python3 tools/scripts/run_python_coverage.py --pattern tools/scripts/test_lcov_cobertura.py --pattern tools/scripts/test_lcov_cobertura_extra.py` passed and reported 100% for `tools/scripts/lcov_cobertura.py` with 243 statements, 0 misses, 74 branches, and 0 partials; docs-sync report passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. System Python lacked coverage, so `uv run` was used. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-pulp-sandbox-extra-643` | `1d107aa7` | #643 tooling tranche for `tools/sandbox-e2e/pulp_sandbox.py` paths | `tools/sandbox-e2e/test_pulp_sandbox_unit.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `uv run --with pytest --with coverage pytest tools/sandbox-e2e/test_pulp_sandbox_unit.py -q` passed with 17 tests; shared `run_python_coverage.py --pattern tools/sandbox-e2e/test_pulp_sandbox_unit.py` exits with the expected `matched tests are outside the configured Python coverage surfaces`; direct `COVERAGE_FILE=build-coverage/python/.coverage.sandbox uv run --with pytest --with coverage coverage run --include='tools/sandbox-e2e/pulp_sandbox.py' -m pytest tools/sandbox-e2e/test_pulp_sandbox_unit.py -q` plus coverage report passed and reported 100% for `tools/sandbox-e2e/pulp_sandbox.py` with 184 statements and 0 misses; version-bump, skill-sync, docs-sync, compat-sync, and diff checks passed; final tracked status clean. System Python lacked pytest and coverage, so validation used `uv run`. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-freshness-extra-643` | `1f0507eb` | #643 tooling tranche for `tools/packages/freshness_check.py` edges | `tools/packages/test_freshness_check_extra.py` | Rebased cleanly onto `origin/main` at `d1750daa`; `python3 -m unittest tools/packages/test_freshness_check_extra.py -v` passed with 9 tests; `python3 -m unittest tools/packages/test_package_validation_tools.py -v` passed with 14 tests; direct focused coverage over the relevant package tests reported 100% for `tools/packages/freshness_check.py` with 109 statements, 0 misses, 44 branches, and 0 partials; `python3 tools/packages/validate_registry.py` passed functionally with 36 packages, 0 errors, and 12 warnings when `jsonschema` was unavailable; skill-sync and docs-sync reports passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. The repo `run_python_coverage.py` currently rejects package tests as outside selected surfaces, so direct focused coverage was used. Strict license checking still exposes existing registry policy findings unrelated to this tranche. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
