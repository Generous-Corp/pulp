...
| `local/phase3-list-limitations-coverage-643` | `1dc74d13` | #643 tooling tranche for `tools/list_limitations.py` paths plus shared coverage-runner behavior needed by this tranche | `tools/test_list_limitations.py`, `tools/scripts/run_python_coverage.py` | Refreshed against current `origin/main` at `7b78de80`; `python3 -m unittest tools.test_list_limitations tools.scripts.test_run_python_coverage` reports 31 tests; temp-venv focused `run_python_coverage.py --pattern tools/test_list_limitations.py` passed and reported 98% for `tools/list_limitations.py`; skill-sync report; version-bump report; `git diff --check`; final status clean. System `python3` lacked coverage, so the worker used a temporary venv outside the repo and removed it afterward. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-check-format-validation-coverage-643` | `d17aba5a` | #643 tooling tranche for `tools/check_format_validation.py` paths | `tools/scripts/test_check_format_validation.py` | Refreshed against current `origin/main`; `python3 tools/scripts/test_check_format_validation.py` reports 9 tests; temp-venv branch coverage over the same test passed and reported 97% for `tools/check_format_validation.py`; skill-sync report; version-bump report; `git diff --check origin/main...HEAD`; final status clean. System `python3` lacked coverage, so the worker used a temporary venv outside the repo and removed it afterward. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-bench-diff-coverage-643` | `17a19979` | #643 tooling tranche for `tools/scripts/bench_diff.py` paths | `tools/scripts/test_bench_diff.py` | Refreshed against current `origin/main` at `8f038e9b`; `python3 tools/scripts/test_bench_diff.py` reports 8 tests; temp-venv focused `run_python_coverage.py --pattern tools/scripts/test_bench_diff.py` passed and reported 98% for `tools/scripts/bench_diff.py`; skill-sync report; version-bump report; `git diff --check`; final status clean. System `python3` lacked coverage, so the focused run used a temporary venv outside the repo and removed it afterward. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-check-docs-consistency-coverage-643` | `c0232634` | #643 tooling tranche for `tools/check-docs-consistency.py` paths | `tools/scripts/test_check_docs_consistency.py` | Refreshed against current `origin/main`; `python3 tools/scripts/test_check_docs_consistency.py` reports 9 tests; temp-venv coverage over the same test passed and reported 99% for `tools/check-docs-consistency.py`; skill-sync report; version-bump report; `git diff --check origin/main...HEAD`; `git diff --check`; final status clean. System `python3` lacked coverage, so the worker used a temporary venv outside the repo and removed it afterward. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
...
| `local/phase3-run-swift-coverage-extra-643` | `925faad7` | #643 tooling tranche for `tools/scripts/run_swift_coverage.py` paths | `tools/scripts/test_run_swift_coverage_extra.py` | Refreshed against current `origin/main`; `python3 -m unittest tools.scripts.test_run_swift_coverage tools.scripts.test_run_swift_coverage_extra` reports 14 tests; temp-venv coverage over both test files passed and reported 97% for `tools/scripts/run_swift_coverage.py` and 98% focused total; skill-sync report; version-bump report; `git diff --check`; final status clean. System `python3` lacked coverage, so the worker used a temporary venv outside the repo and removed it afterward. | When capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-android-target-coverage-643` | `b8ce172b` | #643 tooling tranche for `tools/local-ci/android_target.py` paths | `tools/scripts/test_android_target.py` | Refreshed against current `origin/main`; `python3 -m unittest tools/scripts/test_android_target.py` reports 16 tests; temp-venv focused `run_python_coverage.py --pattern tools/scripts/test_android_target.py` passed and reported 100% for `tools/local-ci/android_target.py`; skill-sync report; version-bump report; `git diff --check origin/main...HEAD`; final status clean. System `python3` lacked coverage, so the focused run used a temporary venv outside the repo and removed it afterward; total report was low because the runner source set includes many unrelated Python tooling files. | When capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-validate-hosts-coverage-643` | `ba74fda9` | #643 tooling tranche for `tools/deps/validate_hosts.py` paths | `tools/scripts/test_validate_hosts.py` | Refreshed against current `origin/main` at `8f038e9b`; `python3 tools/scripts/test_validate_hosts.py` reports 9 tests and only mocked/local commands; temp-venv focused `run_python_coverage.py --pattern tools/scripts/test_validate_hosts.py` passed and reported 100% for `tools/deps/validate_hosts.py`; skill-sync report; version-bump report; `git diff --check`; final status clean. No SSH, VM, or remote validation work was run. System `python3` lacked coverage, so the focused run used a temporary venv outside the repo and removed it afterward; total report was low because the runner source set includes many unrelated Python tooling files. | When capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
...
| `local/phase3-docs-sync-check-extra-643` | `d8811061` | #643 tooling tranche for `tools/scripts/docs_sync_check.py` paths | `tools/scripts/test_docs_sync_check_extra.py` | Refreshed against current `origin/main`; `python3 tools/scripts/test_docs_sync_check.py` reports 15 tests; `python3 tools/scripts/test_docs_sync_check_extra.py` reports 12 tests; temp-venv focused `run_python_coverage.py --pattern tools/scripts/test_docs_sync_check.py --pattern tools/scripts/test_docs_sync_check_extra.py` passed and reported 99% for `tools/scripts/docs_sync_check.py`; skill-sync report; version-bump report; `git diff --check`; final status clean. System `python3` lacked supported coverage, so the focused run used a temporary venv outside the repo and removed it afterward. | When capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
| `local/phase3-coverage-tier-check-extra-643` | `b6536d70` | #643 tooling tranche for `tools/scripts/coverage_tier_check.py` paths | `tools/scripts/test_coverage_tier_check_extra.py` | Refreshed against current `origin/main` at `b9ad4002`; temp-venv `python tools/scripts/test_coverage_tier_check.py` reports 15 tests; temp-venv `python tools/scripts/test_coverage_tier_check_extra.py` reports 17 tests; temp-venv branch coverage over the extra tests reported 96% for `tools/scripts/coverage_tier_check.py` and 97% focused total; skill-sync report; version-bump report; `git diff --check`; final status clean. System `python3` lacked coverage, so the focused run used a temporary venv outside the repo and removed it afterward. | When capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build <branch> --require-sha HEAD`. |
