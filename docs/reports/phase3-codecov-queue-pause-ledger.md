...
Last live check: 2026-05-01 19:00:01 EDT.
...
- GitHub Actions pressure: #1226-#1227 have PR-event Build/Coverage
  checks active or queued on Namespace-backed lanes and no failed checks.
  Main changed after #1224/#1225 merged, so re-check mergeability for #1226/#1227
  before merging any further PRs.
- Local-only progress: `pulp-check-docs-status-extra-643` refreshed cleanly
  at `1e79eafd` with 10 format-validation tests, 23 coverage-runner tests,
  and 100% target coverage for `tools/check_format_validation.py`.
- Local-only progress: `pulp-audit-top-level-coverage-643` refreshed cleanly
  ...
  `tools/audit.py`.
- Local-only progress: `pulp-validate-hosts-coverage-643` refreshed cleanly
  at `204dfef5` with 9 tests and 100% target coverage for
  `tools/deps/validate_hosts.py`; no real SSH or remote hosts were used.
- Local-only progress: `pulp-build-migration-index-extra-643` refreshed
  cleanly at `000e6b33` with 7 base tests plus 10 extra tests and 100%
  target coverage for `tools/scripts/build_migration_index.py`.
- Local-only progress: `pulp-ruleset-drift-config-643` refreshed cleanly at
  `84dcd2bc`; keep it behind measured-source tranches because it is a
...
- Local-only progress: `pulp-check-docs-status-extra-643` now provides the
  ...
  `1d7ea74d`, with 28 focused tests and 100% target coverage after adding
  orphan production-list and entrypoint coverage. This supersedes the older
  `local/phase3-check-format-validation-coverage-643` caveated row.
...
- Local-only progress: `pulp-docs-generate-coverage-643` is refreshed
  against current `origin/main` at `0c38fa1ae018` as a single local
...
- Local-only progress: `pulp-mkdocs-hooks-coverage-643` is refreshed
  against current `origin/main` at `e93948bb107e` as a single local
  feature commit and locally validated with 100% target coverage.
...
- Local-only progress: `pulp-validate-hosts-coverage-643` is refreshed
  against current `origin/main` at `204dfef53ca9` as a single local
  feature commit and locally validated with 100% target coverage.
  ...
  passed at 100%; docs-sync, coverage-tier, CLI sync, and diff checks
...
- Local-only progress: `pulp-build-migration-index-extra-643` is
  refreshed against current `origin/main` at `000e6b33bb26` as a single
...
- Local-only progress: `pulp-skill-sync-extra-643` advanced to
  `0b2a0a9dc7de` with remaining helper, reporting, and entrypoint
  coverage for `tools/scripts/skill_sync_check.py`. Validation:
  `python3 -m unittest tools/scripts/test_skill_sync_check.py
  tools/scripts/test_skill_sync_check_extra.py` reported 24 tests,
  `git diff --check` passed, and venv-backed focused coverage reported
  100% for `tools/scripts/skill_sync_check.py`.
- Local-only progress: `pulp-lcov-cobertura-extra-643` advanced to
  `de77ccf3d2c1` with zero-hit line, demangler, missing `c++filt`, and
  entrypoint coverage for `tools/scripts/lcov_cobertura.py`. Validation:
  `python3 tools/scripts/test_lcov_cobertura_extra.py` passed and
  throwaway-venv focused coverage over `test_lcov_cobertura*.py`
  reported 100% line/branch coverage for
...
- Local-only progress: `pulp-validate-registry-extra-643` refreshed to
  `ad8cfbcc` with registry validator edge coverage for
...
| --- | --- | --- | --- | --- | --- |
| `feature/phase3-docs-generate-coverage-643` | `0c38fa1a` | #643 tooling tranche for `tools/docs_generate.py` paths | `tools/scripts/test_docs_generate.py` | Rebased onto current `origin/main`; `python3 tools/scripts/test_docs_generate.py` reports 15 tests; `python3 tools/docs_generate.py check` reports `docs-generate: OK`; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_docs_generate.py` passed and reported 100% for `tools/docs_generate.py`; `git diff --check` clean. Branch is local-only and not pushed for this refreshed head. | Hold local-only while Namespace pressure is high; when capacity returns, push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `feature/phase3-list-limitations-coverage-643` | `fc21f4ed` | #643 tooling tranche for `tools/list_limitations.py` paths plus shared coverage-runner behavior needed by this tranche | `tools/test_list_limitations.py`, `tools/scripts/run_python_coverage.py` | Rebased cleanly onto current `origin/main`; `python3 -m unittest tools/test_list_limitations.py` reports 10 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/test_list_limitations.py` passed and reported 100% for `tools/list_limitations.py`; direct venv-backed coverage report over `tools/test_list_limitations.py` also reported 100% for `tools/list_limitations.py` with 88 statements, 0 misses, 36 branches, and 0 partials; skill-sync report and version-bump report both reported no action needed; `git diff --check origin/main...HEAD` and `git diff --check` passed; final tracked status clean and ahead 2; `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows` created #1226, then exited with no local targets remaining as expected for the Namespace-only route; #1226 is labeled `codecov` and linked from #641/#643. | Queued: monitor #1226 and merge once required gates are green. |
| `local/phase3-check-docs-status-extra-643` | `1e79eafd` | #643 tooling tranche for `tools/check_format_validation.py` paths | `tools/test_check_format_validation.py`, `tools/scripts/run_python_coverage.py`, `tools/scripts/test_run_python_coverage.py` | Rebased cleanly onto `origin/main` at `30fc6ec7`; `python3 tools/test_check_format_validation.py` passed with 10 tests; `python3 tools/scripts/test_run_python_coverage.py` passed with 23 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/test_check_format_validation.py` passed and reported 100% for `tools/check_format_validation.py` with 83 statements, 0 misses, 38 branches, and 0 partials; `python3 tools/check_format_validation.py --mode=warn` passed with 4 expected missing production validation entries; `./tools/check-docs.sh` passed with 49 existing warnings; `git diff --check origin/main...HEAD` passed; `python3 tools/scripts/docs_sync_check.py --base origin/main` passed via commit trailer; final tracked status clean and ahead 1. This supersedes older duplicate branch `local/phase3-check-format-validation-coverage-643` at `389c6963`. | Hold local-only while the current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `feature/mkdocs-hooks-coverage-643` | `e93948bb` | #643 tooling tranche for `tools/mkdocs_hooks.py` paths | `tools/scripts/test_mkdocs_hooks.py` | Rebased onto current `origin/main`; `python3 tools/scripts/test_mkdocs_hooks.py` reports 10 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_mkdocs_hooks.py` passed and reported 100% for `tools/mkdocs_hooks.py` with 55 statements, 0 misses, 24 branches, and 0 partials; `git diff --check` and `git diff --check HEAD~1..HEAD` clean. Branch is local-only and not pushed for this refreshed head. | Hold local-only while Namespace pressure is high; when capacity returns, push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-audit-top-level-coverage-643` | `339b79c8` | #643 tooling tranche for `tools/audit.py` paths | `tools/scripts/test_audit_top_level.py` | Rebased cleanly onto `origin/main` at `71267a71`; `python3 tools/scripts/test_audit_top_level.py` passed with 9 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_audit_top_level.py` passed and reported 100% for `tools/audit.py` with 64 statements, 0 misses, 34 branches, and 0 partials; docs-sync and skill-sync reports passed with no mapped paths touched; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `feature/phase3-android-target-coverage-643` | `92135ab5` | #643 tooling tranche for `tools/local-ci/android_target.py` paths | `tools/scripts/test_android_target.py` | Rebased cleanly onto `origin/main` at `71267a71`; `python3 tools/scripts/test_android_target.py` passed with 16 tests; `uv run --with 'coverage>=7.10' python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_android_target.py` passed and reported 100% for `tools/local-ci/android_target.py` with 69 statements, 0 misses, 28 branches, and 0 partials; `git diff --check origin/main...HEAD`, `git diff --exit-code`, and `git diff --cached --exit-code` passed; `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows` created #1227, then exited with no local targets remaining as expected for the Namespace-only route; #1227 is labeled `codecov` and linked from #641/#643. | Queued: monitor #1227 and merge once required gates are green. |
| `local/phase3-validate-hosts-coverage-643` | `204dfef5` | #643 tooling tranche for `tools/deps/validate_hosts.py` paths | `tools/scripts/test_validate_hosts.py` | Rebased cleanly onto `origin/main` at `71267a71`; `python3 tools/scripts/test_validate_hosts.py` passed with 9 tests and only mocked local/SSH command construction; focused coverage passed and reported 100% for `tools/deps/validate_hosts.py` with 61 statements, 0 misses, 14 branches, and 0 partials; docs-sync report passed; coverage-tier check against focused Cobertura XML passed; `python3 tools/scripts/cli_sync_check.py --help` executed checks and passed with existing warning output; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. No real SSH, VM, or remote validation was run. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-build-migration-index-extra-643` | `000e6b33` | #643 tooling tranche for `tools/scripts/build_migration_index.py` paths | `tools/scripts/test_build_migration_index_extra.py` | Rebased cleanly onto `origin/main` at `71267a71`; `python3 tools/scripts/test_build_migration_index.py` passed with 7 tests; `python3 tools/scripts/test_build_migration_index_extra.py` passed with 10 tests; focused coverage reported 100% for `tools/scripts/build_migration_index.py` with 179 statements, 0 misses, 88 branches, and 0 partials; skill-sync and docs-sync reports passed with no mapped paths touched; temp codegen smoke with `--docs-dir docs/migrations --out <tmp>` passed and produced non-empty 77-line output; `git diff --check origin/main...HEAD` and `git diff --check` passed; final tracked status clean and ahead 1. No tracked generated `tools/cli/migration_index.cpp` exists in this worktree, so validation covered temp codegen output only. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-skill-sync-extra-643` | `cf472bd8` | #643 tooling tranche for `tools/scripts/skill_sync_check.py` paths, reporting edges, and script entrypoint | `tools/scripts/test_skill_sync_check_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_skill_sync_check_extra.py` reports 20 tests; `python3 tools/scripts/test_gates.py` reports 39 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_skill_sync_check_extra.py` passed and reported 100% for `tools/scripts/skill_sync_check.py` with 239 statements, 0 misses, 106 branches, and 0 partials; skill-sync report and version-bump report passed with no action needed; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. Local `./build/tools/cli/pulp version check` was unavailable because this worktree has no built CLI. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-cmajor-external-extra-643` | `3da17b45` | #643 tooling tranche for `tools/scripts/cmajor_external.py` edges | `tools/scripts/test_cmajor_external_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_cmajor_external.py` reports 4 tests; `python3 tools/scripts/test_cmajor_external_extra.py` reports 9 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_cmajor_external.py --pattern tools/scripts/test_cmajor_external_extra.py` passed and reported 100% for `tools/scripts/cmajor_external.py` with 119 statements, 0 misses, 32 branches, and 0 partials; `python3 tools/scripts/cmajor_external.py doctor --patch examples/cmajor-gain/CmajorGain.cmajorpatch` passed and reported missing external `cmaj` without failing; dry-run `generate --target cpp` passed; skill-sync report and version-bump report passed with no action needed; `git diff --check origin/main...HEAD` passed; final tracked status clean. Temporary coverage venv/artifacts and `__pycache__` were removed after validation. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-resolve-runs-on-extra-643` | `9cc8a91e` | #643 tooling tranche for `tools/scripts/resolve_runs_on.py` edges | `tools/scripts/test_resolve_runs_on_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_resolve_runs_on.py` and `python3 tools/scripts/test_resolve_runs_on_extra.py` passed with 18 base tests plus 2 extra tests; system `python3 tools/scripts/run_python_coverage.py` failed because system Python lacks coverage.py >= 7.10, then venv-backed `run_python_coverage.py --pattern tools/scripts/test_resolve_runs_on.py --pattern tools/scripts/test_resolve_runs_on_extra.py` passed and reported 100% for `tools/scripts/resolve_runs_on.py` with 94 statements, 0 misses, 40 branches, and 0 partials; skill-sync report and version-bump report passed with no action needed; `git diff --check` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-merge-cobertura-extra-643` | `1e93e56a` | #643 tooling tranche for `tools/scripts/merge_cobertura.py` edges | `tools/scripts/test_merge_cobertura_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_merge_cobertura.py` and `python3 tools/scripts/test_merge_cobertura_extra.py` passed with 14 base tests plus 3 extra tests; system `python3 tools/scripts/run_python_coverage.py` failed because coverage.py >= 7.10 was unavailable, then venv-backed `run_python_coverage.py --pattern tools/scripts/test_merge_cobertura.py --pattern tools/scripts/test_merge_cobertura_extra.py` passed and reported 100% for `tools/scripts/merge_cobertura.py` with 121 statements, 0 misses, 48 branches, and 0 partials; skill-sync report and version-bump report passed with no action needed; `git diff --check origin/main...HEAD`, `git diff --exit-code`, and `git diff --cached --exit-code` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-lcov-cobertura-extra-643` | `0b0e7c3f` | #643 tooling tranche for `tools/scripts/lcov_cobertura.py` zero-hit, demangler, missing-tool, and entrypoint edges | `tools/scripts/test_lcov_cobertura_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_lcov_cobertura.py` passed with 8 tests; `python3 tools/scripts/test_lcov_cobertura_extra.py` passed with 11 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_lcov_cobertura.py --pattern tools/scripts/test_lcov_cobertura_extra.py` passed and reported 100% for `tools/scripts/lcov_cobertura.py` with 243 statements, 0 misses, 74 branches, and 0 partials; skill-sync, version-bump, docs-sync, and compat-sync reports passed with no action needed; `git diff --check origin/main...HEAD` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-freshness-extra-643` | `c2284762` | #643 tooling tranche for `tools/packages/freshness_check.py` edges plus shared coverage-runner behavior needed by this tranche | `tools/packages/test_freshness_check_extra.py`, `tools/scripts/run_python_coverage.py`, `tools/scripts/test_run_python_coverage.py` | Rebased cleanly onto current `origin/main`; `python3 -m unittest tools/packages/test_package_validation_tools.py tools/packages/test_freshness_check_extra.py` reports 22 tests; `python3 -m unittest tools/scripts/test_run_python_coverage.py tools/scripts/test_run_python_coverage_extra.py` reports 31 tests; venv-backed `run_python_coverage.py --pattern tools/test_check_status_ladder.py --pattern tools/packages/test_package_validation_tools.py --pattern tools/packages/test_freshness_check_extra.py` passed and reported 100% for `tools/packages/freshness_check.py` with 109 statements, 0 misses, 44 branches, and 0 partials; `python3 tools/packages/validate_registry.py` passed with 36 packages, 0 errors, and 12 warnings; strict license validation still fails on existing registry license errors unrelated to this tranche; `git diff --check origin/main...HEAD`, `git diff --exit-code`, and `git diff --cached --exit-code` passed; final tracked status clean and ahead 1. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-validate-registry-extra-643` | `ad8cfbcc` | #643 tooling tranche for `tools/packages/validate_registry.py` edges | `tools/scripts/test_validate_registry_extra.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_validate_registry_extra.py` reports 8 tests; `python3 tools/packages/test_package_validation_tools.py` reports 14 tests; venv-backed `tools/scripts/run_python_coverage.py --pattern tools/scripts/test_validate_registry_extra.py --pattern tools/packages/test_package_validation_tools.py` passed and reported 100% for `tools/packages/validate_registry.py`; direct single-process coverage over `test_validate_registry_extra.py` reported 99% because the script entrypoint is exercised through a subprocess test; non-strict `python3 tools/packages/validate_registry.py` passed with 36 packages, 0 errors, and 12 warnings; strict `python3 tools/packages/validate_registry.py --strict --check-licenses` exits 1 on existing registry license findings (`36 packages validated, 8 errors, 14 warnings`); skill-sync report and version-bump report both reported no action needed; `git diff --check origin/main...HEAD` and `git diff --check` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-workflow-lint-coverage-643` | `2d162c97` | #643 static workflow guard tranche for `.github/workflows/workflow-lint.yml` invariants | `tools/scripts/test_workflow_lint.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_workflow_lint.py` reports 7 tests; `python3 tools/scripts/test_release_workflow_test_step.py` reports 5 tests; `yamllint --no-warnings -d 'relaxed' .github/workflows/` passed; structural PyYAML parse passed across 28 workflow files; `actionlint -shellcheck= -pyflakes= .github/workflows/workflow-lint.yml` passed; skill-sync report and version-bump report passed with no action needed; `PULP_SKIP_DIFF_COVER=1 tools/scripts/local_diff_cover.sh` passed with static/test-only skip; focused `run_python_coverage.py --pattern tools/scripts/test_workflow_lint.py` ran 7 tests but did not produce a measured source report because this tranche only adds a static guard test file; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-codecov-config-edges-643` | `599cf8c7` | #643 static coverage-config guard tranche for `codecov.yml` invariants | `tools/scripts/test_codecov_config.py` | Rebased cleanly onto current `origin/main`; `python3 tools/scripts/test_codecov_config.py` reports 13 tests; `python3 tools/scripts/test_run_python_coverage.py` reports 23 tests; `python3 tools/scripts/test_run_python_coverage_extra.py` reports 8 tests; venv-backed `run_python_coverage.py --pattern tools/scripts/test_codecov_config.py` passed, with no direct measured source target because this is a test-only/static config guard; skill-sync, docs-sync, compat-sync, version-bump, and `test_gates.py` checks passed; `git diff --check origin/main...HEAD` passed; final tracked status clean and ahead 1. No local VM, SSH, push, or PR work was run. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-ruleset-drift-config-643` | `84dcd2bc` | #643 static ruleset-drift guard tranche for branch-protection JSON/workflow invariants | `tools/scripts/test_ruleset_drift_config.py` | Rebased cleanly onto `origin/main` at `30fc6ec7`; `python3 tools/scripts/test_ruleset_drift_config.py` passed with 6 tests; `python3 -m py_compile tools/scripts/test_ruleset_drift_config.py` passed; skill-sync, version-bump, docs-sync, and compat-sync reports passed; supporting guard tests `test_codecov_config.py`, `test_local_diff_cover.py`, `test_coverage_diff_comment.py`, and `test_coverage_tier_check.py` passed; `git diff --check origin/main...HEAD` passed; final status clean and ahead 1. The official `run_python_coverage.py` surface intentionally excludes `tools/scripts/test_*.py`, so this static guard has no non-test source to report in that focused runner. | Hold behind measured-source tranches; when capacity returns, rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-theme-io-validation-493` | `9b99ed90` | #493 view tranche for `core/view/src/theme.cpp` and `core/view/include/pulp/view/theme.hpp` validation/file edge paths | `test/test_theme.cpp` | Refreshed against current `origin/main` with a clean rebase; `cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug`; `cmake --build build --target pulp-test-theme pulp-test-theme-contrast pulp-test-theme-presets -j8`; `build/test/pulp-test-theme` passed 82 assertions in 17 test cases; `ctest --test-dir build -R 'Theme\|theme' --output-on-failure` passed 20/20; `git diff --check origin/main...HEAD` and `git diff --check` clean; final status clean and ahead 1. Existing unrelated build warnings remain. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-app-framework-edges-493` | `ffe4774a` | #493 view tranche for `core/view/src/app_framework.cpp` shortcut, key mapping, menu, toolbar, and settings edges | `test/test_app_framework.cpp` | Refreshed against current `origin/main` with a clean rebase; `cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug`; `cmake --build build --target pulp-test-app-framework -j$(sysctl -n hw.ncpu)`; `./build/test/pulp-test-app-framework` passed 101 assertions in 27 test cases; `ctest --test-dir build -R 'app-framework\|KeyShortcut\|KeyMapping\|MenuBar\|NativeToolbar\|AppSettings' --output-on-failure` passed 27/27; `git diff --check` clean; final status clean and ahead 1. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
