...
`fb0274fe2 test(runtime): cover dismissed moved scope guard`, touching
...
`feature/phase3-codecov-runtime-state-batch-752`:
`b83216558 test(runtime): cover temporary file self move`, touching
...
`feature/phase3-codecov-runtime-state-batch-752`:
`40f4b33fd test(runtime): cover malformed raw deflate streams`, touching
...
2026-05-18 22:17 PDT: added eighth committed tranche to
`feature/phase3-codecov-runtime-state-batch-752`:
`37872fa56 test(runtime): cover expression identifier edges`, touching
`test/test_runtime_utils.cpp`. Scope: deterministic expression evaluator
coverage for underscore/digit variable identifiers, replacing a
registered unary function, and preserving custom functions after variable
clearing. Local validation passed:
`cmake --build build --target pulp-test-runtime-utils`,
`./build/test/pulp-test-runtime-utils "ExpressionEvaluator handles
identifier edges and function replacement"`,
...
2026-05-18 22:32 PDT: added tenth committed tranche to
`feature/phase3-codecov-runtime-state-batch-752`:
`f419d2899 test(runtime): cover child process working directory`,
touching `test/test_runtime_utils.cpp`. Scope: deterministic
`run_process` coverage for applying the requested working directory,
using a temp directory and verifying a child-created marker file lands in
that directory. Local validation passed:
`cmake --build build --target pulp-test-runtime-utils`,
...
2026-05-18 22:42 PDT: added twelfth committed tranche to
`feature/phase3-codecov-runtime-state-batch-752`:
`b15b29c27 test(runtime): cover process running query`, touching
`test/test_runtime_utils.cpp`. Scope: deterministic `is_process_running`
coverage for the current process PID, with platform-specific PID lookup
guarded in the test. Local validation passed:
`cmake --build build --target pulp-test-runtime-utils`,
`./build/test/pulp-test-runtime-utils "is_process_running recognizes the
current process"`, `./build/test/pulp-test-runtime-utils
"[runtime][child_process][coverage][phase3]"`, and `git diff --check`.
PR state: not pushed; keep accumulating runtime coverage commits toward
the larger batch. No Namespace dispatch.

2026-05-18 22:43 PDT: added thirteenth committed tranche to
`feature/phase3-codecov-runtime-state-batch-752`:
`2a1bbc125 test(runtime): cover missing process query`, touching
`test/test_runtime_utils.cpp`. Scope: deterministic `is_process_running`
coverage for the false path using an impossible high PID, complementing
the current-process true-path assertion. Local validation passed:
`cmake --build build --target pulp-test-runtime-utils`,
`./build/test/pulp-test-runtime-utils "is_process_running rejects an
impossible high pid"`, `./build/test/pulp-test-runtime-utils
"[runtime][child_process][coverage][phase3]"`, and `git diff --check`.
PR state: not pushed; keep accumulating runtime coverage commits toward
the larger batch. No Namespace dispatch.
...
2026-05-19 00:25 PDT: added twenty-ninth committed tranche to
`feature/phase3-codecov-runtime-state-batch-752`:
`e98c5be73 test(state): cover listener token self move assignment`,
touching `test/test_state.cpp`. Scope: deterministic `ListenerToken`
coverage for the self move-assignment branch preserving the existing
subscription and token id. Local validation passed:
`cmake --build build --target pulp-test-state`,
`./build/test/pulp-test-state "ListenerToken self move assignment
preserves subscription"`, `./build/test/pulp-test-state
...
`test/test_cli_run_options.cpp`. Scope: deterministic `pulp run`
parser coverage for `--` separator behavior with known flags and
multiple positional pass-through tokens. Local validation passed:
`cmake --build build --target pulp-test-cli-run-options`,
`build/test/pulp-test-cli-run-options` (81 assertions in 15 test
...
preference for `index.html` when `code.html` is absent, and unquoted
script attribute scraping. Local validation passed:
`cmake --build build --target pulp-test-cli-import-detect`,
...
mismatch build-dir fallback. Local validation passed:
`cmake --build build --target pulp-test-cli-design-binding`,
`build/test/pulp-test-cli-design-binding` (43 assertions in 9 test
cases), and `git diff --check`. PR state: not pushed; keep
accumulating related coverage commits toward the larger batch. No
Namespace dispatch.
...
unknown license verdict labels, semver prerelease/minor-constraint
edges, and case/empty query search behavior. Local validation passed:
`cmake --build build --target pulp-test-cli-package-registry`,
`build/test/pulp-test-cli-package-registry` (138 assertions in 10
test cases), and `git diff --check`. Local validation caught one
...
counters, and ANSI color rendering. Local validation passed:
`cmake --build build --target pulp-test-cli-validator-discovery`,
`build/test/pulp-test-cli-validator-discovery` (83 assertions in 13
test cases), and `git diff --check`. PR state: not pushed; keep
accumulating related coverage commits toward the larger batch. No
Namespace dispatch.
...
`ShapedRun` / `ClusterEntry` / `LineBreakOpportunity` value behavior.
Validation passed locally: rebuilt `pulp-test-font-options`, ran
`./build/test/pulp-test-font-options "[canvas][font]"` (12 cases / 135
assertions), and `git diff --check` passed. Still held locally for the next
large batch PR.
...
2026-05-18 00:04 PDT: rebased held batch
...
override. Local validation passed after rebase: built
`pulp-test-font-options` and `pulp-test-widgets`, ran
`./build/test/pulp-test-font-options "[canvas][font]"` (12 cases / 135
assertions), ran
...
2026-05-18 23:18 PDT: added `84a399e07` (`test(runtime): cover dynamic
library self move`) to `feature/phase3-codecov-runtime-state-batch-751`. This
covers the `DynamicLibrary::operator=(DynamicLibrary&&)` self-assignment guard
with an open system library handle, verifying the handle remains open and
symbol lookup still succeeds. Focused local validation passed: `cmake --build
build --target pulp-test-runtime-utils`, `./build/test/pulp-test-runtime-utils
"DynamicLibrary self move assignment preserves open handle"` (5 assertions / 1
case), `./build/test/pulp-test-runtime-utils
"[runtime][dynlib][coverage][phase3]"` (23 assertions / 5 cases), and `git
diff --check`. The new batch now has 9 local coverage commits; continue
accumulating before opening a PR.
...
2026-05-18 21:08 PDT: added `21c8fbecd` (`test(host): cover graph nonpositive
process blocks`) to the held `feature/phase3-codecov-host-cache-batch-750`
batch. This covers the `SignalGraph::process()` guard for zero and negative
block sizes, asserting output is left untouched rather than being zeroed or
processed. Focused local validation passed: `cmake --build build --target
pulp-test-host`, `./build/test/pulp-test-host "SignalGraph process ignores
non-positive block sizes"` (4 assertions / 1 case),
`./build/test/pulp-test-host "[host][graph][coverage][phase3]"` (77 assertions
...
from current `origin/main` (`8f4674828`) and added `c21f013e1`
...
2026-05-19 01:15 PDT: added `c3ae018be` (`test(runtime): cover json rpc
...
2026-05-19 01:20 PDT: added `c2c6648cd` (`test(runtime): cover default
...
2026-05-19 01:25 PDT: added `16ccbc8fd` (`test(runtime): cover json rpc
...
`feature/phase3-codecov-runtime-state-batch-752` with `bc0b94353` after one new
...
rerunning the affected target serially because parallel CMake regeneration
temporarily collided in FetchContent: `cmake --build build --target
pulp-test-font-options -j4`, `./build/test/pulp-test-font-options` (138
assertions / 13 cases), and `git diff --check`. The batch is current with
`origin/main`.

2026-05-19 00:06 PDT: dropped local duplicate `000d614a5`
(`test(platform): cover progress parser colon payloads`) before PR prep because
`test/test_progress_parser.cpp` already covered empty event types and
colon-rich payloads. The main merge replayed cleanly from `0886f20ab` to
`bc0b94353`; the branch remained current with `origin/main`.

2026-05-19 00:08 PDT: added `bc7ae7643` (`test(platform): cover partial stderr
lines`) to `feature/phase3-codecov-runtime-state-batch-752`. This covers the
`ChildProcess` line-buffer contract for stderr output that has no trailing
newline, complementing the existing partial-stdout coverage and asserting no
incomplete stderr line callback fires while captured stderr is preserved.
Focused local validation passed: `cmake --build build --target
pulp-test-child-process -j4`, `./build/test/pulp-test-child-process "line
callback buffers partial stderr without trailing newline"` (3 assertions / 1
