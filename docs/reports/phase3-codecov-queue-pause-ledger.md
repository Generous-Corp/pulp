...
Last live check: 2026-05-01 20:24:26 EDT.
...
- Refill: opened #1235 from
  ...
  `tools/packages/freshness_check.py`. PR-event checks are re-running on
  the corrected head.
- GitHub Actions pressure: #1231, #1233, #1234, and #1235 have PR-event
...
- Local-only progress: `pulp-phase3-theme-io-validation-493` refreshed
  cleanly at `4d34c030` with direct theme, theme-contrast, and
  theme-presets binaries plus 22 matching CTest cases green.
...
- Local-only progress: `pulp-lcov-cobertura-extra-643` refreshed cleanly
  at `5742da60` with 19 total tests and 100% target coverage for
  `tools/scripts/lcov_cobertura.py`.
- Local-only progress: `pulp-phase3-app-framework-edges-493` refreshed
  at `cdbc5a11` with app-framework build/direct/CTest checks green; local
  diff coverage still needs rerun on a volume with enough free space before
  queueing.
...
| `local/phase3-merge-cobertura-extra-643` | `29cb5a51` | #643 tooling tranche for `tools/scripts/merge_cobertura.py` edges | `tools/scripts/test_merge_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `8c1c8088`; `.venv/bin/python tools/scripts/test_merge_cobertura.py` passed with 14 tests; `.venv/bin/python tools/scripts/test_merge_cobertura_extra.py` passed with 3 tests; `.venv/bin/python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_merge_cobertura.py --pattern tools/scripts/test_merge_cobertura_extra.py` passed and reported 100% for `tools/scripts/merge_cobertura.py`; docs-sync, skill-sync, version-bump, and CLI sync reports passed; `git diff --check origin/main...HEAD` and `git diff --check` passed; final tracked status clean and ahead 1. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-lcov-cobertura-extra-643` | `5742da60` | #643 tooling tranche for `tools/scripts/lcov_cobertura.py` zero-hit, demangler, missing-tool, and entrypoint edges | `tools/scripts/test_lcov_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `8c1c8088`; `uv run python3 tools/scripts/test_lcov_cobertura.py` passed with 8 tests; `uv run python3 tools/scripts/test_lcov_cobertura_extra.py` passed with 11 tests; `uv run --with 'coverage>=7.10' --with PyYAML python3 tools/scripts/run_python_coverage.py --pattern tools/scripts/test_lcov_cobertura.py --pattern tools/scripts/test_lcov_cobertura_extra.py` passed and reported 100% for `tools/scripts/lcov_cobertura.py`; docs-sync, skill-sync, version-bump, and CLI sync reports passed; `git diff --check origin/main...HEAD` and `git diff --check` passed; final tracked status clean and ahead 1. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-theme-io-validation-493` | `4d34c030` | #493 view tranche for `core/view/src/theme.cpp` and `core/view/include/pulp/view/theme.hpp` validation/file edge paths | `test/test_theme.cpp` | Rebased cleanly onto `origin/main` at `8c1c8088`; `cmake --build build --target pulp-test-theme pulp-test-theme-contrast pulp-test-theme-presets -j$(sysctl -n hw.ncpu)` passed; `./build/test/pulp-test-theme` passed 82 assertions in 17 test cases; `./build/test/pulp-test-theme-contrast` passed 36 assertions in 20 test cases; `./build/test/pulp-test-theme-presets` passed 861 assertions in 9 test cases; `ctest --test-dir build -R 'Color from hex\|Theme\|theme\|Motion\|Dark theme\|Light theme\|Pro audio\|Auto-fix\|Derived' --output-on-failure` passed 22/22; skill-sync and version-bump reports passed; `git diff --check origin/main...HEAD` and `git diff --check` passed; final status clean and ahead 1. | Hold local-only while Namespace pressure is high; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-app-framework-edges-493` | `cdbc5a11` | #493 view tranche for `core/view/src/app_framework.cpp` shortcut, key mapping, menu, toolbar, and settings edges | `test/test_app_framework.cpp` | Rebased cleanly onto `origin/main` at `8c1c8088`; `cmake --build build --target pulp-test-app-framework -j$(sysctl -n hw.ncpu)` passed; `./build/test/pulp-test-app-framework` passed 101 assertions in 27 test cases; `ctest --test-dir build -R 'KeyShortcut\|KeyMapping\|MenuBar\|NativeToolbar\|AppSettings' --output-on-failure` passed 27/27; skill-sync and version-bump reports passed; `git diff --check origin/main...HEAD` passed; final status clean and ahead 1. Local `tools/scripts/local_diff_cover.sh pulp-test-app-framework` could not complete because the local volume had only about 183 MiB free and CMake failed cloning `mbedtls` into `build-cov/`; the partial `build-cov/` directory was removed. | Hold local-only until targeted diff coverage is rerun on a machine/volume with enough free space; then rename/push as a feature branch and run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`. |
