...
one signed release-bot commit whose parent is the immutable event base, the
fixed branch/title/message marker, and a candidate tree byte-identical to
...
detailed pull record; mixed groups do not qualify. Missing base code, GitHub API
or signature errors, stale topology, pagination ambiguity, or any additional
byte retains ordinary validation. This optimization changes only PR and
single-member merge-group admission: version/skill enforcement and both Vellum
gates still run, and pushes to main, releases, scheduled work, and manual
