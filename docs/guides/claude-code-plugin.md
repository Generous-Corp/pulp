...
| Development Inspector | Installed `pulp_inspect_*` clients discover and authenticate to explicitly inspector-enabled standalone sessions. `pulp_motion_*` remains on that legacy development path; `pulp_trace_start` / `pulp_trace_stop` use canonical broker-authorized capability control with no Inspector or raw host/port fallback. Offline trace query remains usable without a live session. |
