...
| **V8** | BSD-3-Clause | Optional JS engine backend, selected with `PULP_JS_ENGINE=v8` (default is QuickJS; JSC is opt-in on Apple with `PULP_JS_ENGINE=jsc`). Sealed prebuilt `libv8` (bundles ICU/zlib/Abseil internally — see NOTICE.md); fetched per-platform via `tools/scripts/fetch_v8_for_release.py`. V8 is unavailable on iOS (JIT is forbidden) | [github.com/danielraffel/v8-builder](https://github.com/danielraffel/v8-builder) |
