...
Pulp is released under the **MIT License**. You can use it for any purpose — commercial, personal, educational — with no royalties, revenue thresholds, or copyleft obligations. See [LICENSE.md](https://github.com/Generous-Corp/pulp/blob/main/LICENSE.md) for the full text.
...
Tables are sorted alphabetically (case-insensitive) by name. Entries here must stay in sync with [DEPENDENCIES.md](https://github.com/Generous-Corp/pulp/blob/main/DEPENDENCIES.md), [NOTICE.md](https://github.com/Generous-Corp/pulp/blob/main/NOTICE.md), and [tools/deps/manifest.json](https://github.com/Generous-Corp/pulp/blob/main/tools/deps/manifest.json) (the machine-readable source of truth).
...
Optional third-party integrations are off by default and follow one of two
distribution models. Some depend on **proprietary, developer-supplied SDKs**
that Pulp never bundles, fetches, exports, or redistributes. Others are
**permissively licensed dependencies** that Pulp fetches via FetchContent
**only when a developer opts in**, and therefore — unlike the proprietary
SDKs — do carry `NOTICE.md` and `DEPENDENCIES.md` entries. Both kinds stay out
of Pulp's default MIT dependency chain until you enable them.

### Developer-Supplied Vendor SDKs

Separately licensed SDKs that Pulp does not bundle, fetch, export, or
redistribute. They are outside Pulp's MIT dependency chain, are supported only
through explicit opt-in local configuration, and are intentionally absent from
`NOTICE.md`.
...
Permissively licensed integrations that Pulp fetches via FetchContent
**only when the developer opts in** (for example with `pulp add`). Because Pulp
redistributes them through the build when enabled, they **do** get `NOTICE.md`
attribution and a `DEPENDENCIES.md` row — the divergence from the
developer-supplied SDKs above.
...
| **MTS-ESP** ([ODDSound/MTS-ESP](https://github.com/ODDSound/MTS-ESP)) | 0BSD | Session-wide microtuning client API, including MTS-ESP master queries, MTS SysEx fallback, note filtering, and multi-channel tuning tables | Source-only client fetched via FetchContent on opt-in (`PULP_ENABLE_MTS_ESP=ON` or `pulp add mts-esp`). The client dynamically loads an installed libMTS shared library at runtime when present and falls back locally when no master is installed. Installed SDKs may ship Pulp-owned wrapper glue for `pulp_enable_midi_tuning_provider()`, but not the ODDSound client source unless the developer opts in. |
...
| [ctx](https://ctx.graphics/) | ISC | A vector UI **protocol**, not just a rasterizer: ctx treats a drawing as a recordable command stream that can be written out, diffed and replayed (`ctx_render_stream` / `ctx_parse`), carrying relative units inline (`100%`, `1@` for em). Pulp took the serialising half only — a text form of a `RecordingCanvas` drawlist, which is position- AND material-aware and so covers the two blind spots every pixel scorer here documents — and deliberately not the parsing half, because Pulp already has two round-trippable draw formats in SKP capture and the canvas2d API the ops were authored in, and a third nobody authors in would be dead weight. Separately, ctx's `deps/` is where we found libDaltonLens (below); ctx vendors it without wiring it into the renderer, so the credit there is for carrying the dependency at all, not for a CVD feature. Pulp implements this independently; no ctx source is copied or vendored. Thanks to [Øyvind Kolås](https://pippin.gimp.org/). |
...
| [libDaltonLens](https://daltonlens.org/) | Public domain | The colour-vision-deficiency model behind Pulp's palette check: Viénot 1999 for protanopia and deuteranopia as a 3x3 matrix applied in **linear** RGB (applying it to gamma-encoded values looks plausible and is wrong, the same trap as shading in sRGB), with severity as a linear blend toward the unimpaired colour. Pulp deliberately does **not** implement tritanopia: libDaltonLens documents its own Viénot tritan matrix as inaccurate and directs callers to Brettel 1997, and a simulation that is quietly wrong is worse than one that is absent — it would let a palette pass a check that never actually looked. Pulp re-implements the published matrices in Python; no libDaltonLens source is vendored. |
...
| [PreText](https://github.com/chenglou/pretext) | MIT | Measure-once, reflow-forever text layout. Pulp's `pulp::canvas::TextShaper` / `PreparedText` adopt PreText's two-phase model directly: `prepare()` shapes multilingual text once through Skia/HarfBuzz and caches segment widths plus real font metrics, and `layout()` reflows arithmetically at any width with no font calls — so wrapped height, virtualization, and width-only relayout stay cheap, and streamed or resized text does not shift. Pulp re-implements the pattern from scratch in C++20; no PreText source is copied or vendored. We learned the *shape* — thanks to [chenglou](https://github.com/chenglou) for articulating it so clearly. |
| [Redraw](https://redraw.dev/) | Proprietary (technical preview; no public source) | A wide set of front-end capability ideas, and one reframing: treat expressive rendering as a **substrate authors reach for**, not a catalogue of effects we ship. Redraw's model — every shape is a signed distance field evaluated at fragment time, with distance, arc length, tangent and cross-stroke position handed to author-written GPU functions — is what makes variable-width and geometry-aware strokes, colour that follows a path, analytic feathering that stays correct at the 1-3px radii where a raster blur is only softening an already-antialiased edge, and shape operators that compose a compound silhouette so translucent paints never double-blend and a stroke follows the combined outline. It also sharpened two things Pulp already had and under-used: `sdFlatArc` computes arc length and cross-stroke distance and threw both away, and bevel/material shading existed but was reachable only for the text atlas. Pulp implements this independently in SkSL over Skia — no Redraw source is copied or vendored; the techniques have well-known public formulations. Thanks to [William Candillon](https://wcandillon.dev/), also a co-creator of react-native-skia, which separately informed `@pulp/react`'s reconciler. |
