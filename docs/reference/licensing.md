...
| [ctx](https://ctx.graphics/) | ISC | A vector UI **protocol**, not just a rasterizer. Two ideas Pulp did not have, both read from `ctx.h`: (1) **per-tile hashing as a first-class context** — `ctx_hasher_new()` replays a drawlist into a context that hashes each tile instead of rasterizing it, so damage detection and a cheap, position-aware visual golden fall out of one mechanism, on a different axis from pixel comparison, which is position- and material-blind by construction; and (2) a **round-trippable textual draw-command stream** (`ctx_render_stream` / `ctx_parse`) carrying relative units inline (`100%`, `1@` for em), which makes a render diffable and streamable rather than only viewable. Separately, ctx's `deps/` is where we found libDaltonLens (below) — ctx vendors it without wiring it into the renderer, so the credit there is for carrying the dependency at all, not for a CVD feature. Pulp implements these independently; no ctx source is copied or vendored. Thanks to [Øyvind Kolås](https://pippin.gimp.org/). |
