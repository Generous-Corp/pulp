...
| Sample publication and storage | `published_sample_store.hpp`, `sample_slot_bank.hpp`, `sample_slot_materializer.hpp`, `sample_pool.hpp`, `sample_asset.hpp`, `sample_stream_window.hpp`, `sample_stream_scheduler.hpp`, `sample_stream_service.hpp`, `sample_stream_voice_reader.hpp` | Publish captured/imported/rendered samples through generation-safe views, fixed slot banks, immutable sampler assets, stable sample IDs, resident streaming pages, bounded urgency-ordered commands, a checked-budget shared-cache service core, and allocation-free forward streamed voice reads |
