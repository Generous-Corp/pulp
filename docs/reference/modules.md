...
| Sample publication and storage | `published_sample_store.hpp`, `sample_slot_bank.hpp`, `sample_slot_materializer.hpp`, `sample_pool.hpp`, `sample_stream_window.hpp`, `sample_stream_scheduler.hpp` | Publish captured/imported/rendered samples through generation-safe views, fixed slot banks, stable sample IDs, resident streaming pages, and bounded urgency-ordered page requests |
