...
| Sample publication and storage | `published_sample_store.hpp`, `sample_slot_bank.hpp`, `sample_slot_materializer.hpp`, `sample_pool.hpp`, `sample_asset.hpp`, `sample_stream_window.hpp`, `sample_stream_scheduler.hpp`, `sample_stream_service.hpp`, `sample_stream_decode_pool.hpp`, `sample_stream_voice_reader.hpp`, `sample_stream_loop_voice_reader.hpp` | Publish captured/imported/rendered samples through generation-safe views, fixed slot banks, immutable sampler assets, stable sample IDs, resident streaming pages, bounded urgency-ordered commands, a fixed-scratch decode worker pool, a checked-budget shared-cache service core, and allocation-free forward/reverse one-shot and crossfade-loop streamed reads |
