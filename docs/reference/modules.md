...
| Sample publication and storage | `published_sample_store.hpp`, `sample_slot_bank.hpp`, `sample_slot_materializer.hpp`, `sample_pool.hpp`, `sample_stream_window.hpp`, `sample_stream_scheduler.hpp`, `sample_stream_service.hpp` | Publish captured/imported/rendered samples through generation-safe views, fixed slot banks, stable sample IDs, resident streaming pages, bounded urgency-ordered requests, and a checked-budget shared-cache service core |
| Sequential streaming source | `streaming_sample_source.hpp`, `streaming_sample_source_file.hpp` | Play a preload head plus background-filled SPSC tail; WAV and uncompressed AIFF use retained mapped ranged reads while fallback codec capability remains explicit |
| Stream starvation envelope | `sample_starvation_envelope.hpp` | Fade valid low-water audio to silence and current-timeline recovery audio back in with explicit predicted/emergency telemetry; missing frames remain silent rather than replaying stale data |
