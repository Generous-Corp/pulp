...
| `sample_interpolation.hpp` and sinc kernels | Reconstruct samples at playback-rate positions and protect high-rate playback | Heritage converter/hold/reconstruction blocks model a machine-domain signal path. They are complementary, not alternate names for playback interpolation |
