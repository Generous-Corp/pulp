...
authority-bearing session admission still fails closed. Ephemeral socket and
liveness files are isolated from owner-private durable receipts and artifacts
under `~/.pulp/state/control-broker/v1`; service stop or removal leaves that
durable state intact. Darwin CLI installs place the broker beside `pulp` and `pulp-cpp` and
