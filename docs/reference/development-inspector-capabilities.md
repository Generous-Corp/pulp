...
| CLI | `pulp run --inspect[=PROFILE]` selects the explicit runtime profile; installed `pulp` resolves its sibling `pulp-cpp`; profiles/list/capabilities/doctor and typed parameter/MIDI/transport mutations have stable JSON; every live operation uses exact session/instance/publication targeting through the shared client | Telemetry subscription lands in the next phase |
| MCP | Installed `pulp-mcp` uses the same in-process authenticated client as the CLI and exposes typed parameter, MIDI, and transport tools; inspector dispatch does not shell through `pulp inspect` or require source-build paths | Telemetry subscription lands in the next phase |
...
expires, disconnects, or the session tears down. `Test.setTransport` accepts an
...
Numeric parameter changes remain `state.write`. Transient tweak application,
highlighting, bypass/lock, live constants, editor URL templates, and repaint
flashing remain `authoring.tweaks`. Generic preset load/save, filesystem tweak
