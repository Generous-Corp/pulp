...
| `dev.pulp.ui/input@1` (`ui.input`) | no | yes | Grant-controlled ordinary Standalone composition accepts one bounded pointer, keyboard, focus, or UTF-8 text event for an exact registration/view-generation/node target on the fenced main thread and binds retained state to the exact client/grant/principal owner; product activation still requires the canonical authority-end subscription to invoke owner-specific cleanup |
