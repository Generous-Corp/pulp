...
| `dev.pulp.gpu/health.read@1` (`gpu.health.read`) | yes | yes | Exact-instance sensitive read of a bounded, already-produced first-visible-frame GPU health snapshot; registry and generated-client presence do not imply a live product provider before A2/A2T integration |
