...
pulp build --seed-build        # New worktree: clone + retarget a sibling's warm build dir first (APFS)
pulp build --seed-build=../pulp-main  # Same, naming the donor worktree
...
In a source checkout each `pulp build` is also recorded in Shipyard's metrics store (`shipyard metrics watch --project pulp`) as `local-build/focused` or `local-build/all`, with its wall time, granted `-j`, where that grant came from, and the focused target count, through the same `tools/ci/record_build_metric.sh` recorder the governed build wrapper uses. It is silent when `shipyard` is not installed, adds a few tens of milliseconds, never changes the build's exit status, and `PULP_BUILD_METRICS=0` turns it off.
...
**Warm-seeding a new worktree.** A fresh worktree normally pays a full configure plus every compile and link before its first useful build, even when a sibling worktree a few commits away already holds all of that. On macOS/APFS, `pulp build --seed-build` (or `PULP_SEED_BUILD=1`) runs `tools/scripts/seed_build_dir.py` before the first configure: it clones the donor's Ninja build dir in one `clonefile` call (shared blocks, no data copied), rewrites every text file that names the donor's source or build path, re-emits `.ninja_deps` and rehashes `.ninja_log` so unchanged edges stay clean, deletes any binary that embeds the donor path (a `-D` define carrying the source dir, a build-tree rpath), and gives unchanged clean sources the donor's mtimes. The first build then compiles and links only what differs from the donor; the receipt is `build/.pulp-seed-receipt.json` (`dirty_edges` is Ninja's own plan; `cmake_rerun_pending` names any CMake input that differs from the donor, in which case CMake re-runs first and the count is a lower bound). `--seed-build` picks the eligible sibling worktree (Ninja, built, same build type and `--examples` setting, same volume, not mid-build) closest to `HEAD`; `--seed-build=<worktree>` names one. Every unsupported case (another platform, cross-volume, no warm Ninja donor, build type mismatch, build dir already present) falls back to a plain configure and says so. The donor is only read.
