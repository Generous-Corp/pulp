...
- **Latency changed** — the replacement reports a different processing
  ...
  the same reason.)
- **Over budget / cost unknown** — the graph projects that running the
  replacement would push CPU load past the node's `headroom_threshold`, or
  it has no measured load history to judge the cost from. Because the cost of
  a *different* plugin is unknown, only re-instantiating the **same** plugin
  can go fully gap-free; substituting a different plugin always falls back to
  an eager re-prepare rather than risk an overload on the audio thread.
