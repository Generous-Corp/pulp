...
- Prove the *emitted artifact* renders what the importer claimed — before installing a panel or reporting a score. → `tools/import-validation/verify_rendered_panel.py`
  - ⚠ **Cannot see:** Scores the artifact that ships, not the pipeline that made it — that distinction is the point, since scoring the pipeline is how a panel at 0.13 was certified at 0.98. Refuses to score on a size mismatch (exit 4) rather than comparing the intersection and returning a plausible number. Exit 7 is a harness fault, never reportable as a low score — a stale pulp-screenshot silently no-ops the artifact's theme calls and reads as a catastrophic regression.
