...
- Localize a native panel's pixel differences from its Chromium capture to the topmost responsible nodes. → `tools/import-validation/score_native_panel.py`
  - ⚠ **Cannot see:** Requires NumPy and Pillow, refuses size or colour-calibration mismatches, and measures the supplied Chromium capture rather than independently proving that capture authoritative.
