...
- Ask whether a native design render contains any colour the design never asked for — a value arc, pointer, meter fill or fader in Pulp's palette rather than the design's. → `tools/import-design/design_colour_audit.py`
  - ⚠ **Cannot see:** It compares Pulp's render against Chromium's screenshot of the SAME page, so it measures provenance, not correctness — a stock colour that happens to equal one the design painted is indistinguishable from a derived one. Recolour the design (design_colour_battery test 3) or render two packs (test 4) to separate those. Thresholds sit in a measured gap and the run re-reports that gap as `margins`; a thin margin is a warning, not a pass.
- Run the seven-test acceptance battery proving imported control primitives take their colours from the design and never from Pulp's own palette. → `tools/import-design/design_colour_battery.py`
  - ⚠ **Cannot see:** Each variant is one headless Chromium capture, so a full run is several minutes. Tests 3, 4 and 6 change the design and watch Pulp follow, so each first proves the change reached Chromium — a variant whose reference render is unchanged reports SKIP with the fixture named, never a verdict against the product. Appending an @import to styles.css does NOT work as a variant hook; CSS requires @import before every other rule and the entry sheet ends with a body rule, so the override is silently dropped.
