...
| Surge Synth Team tuning-library | release_1.1.0 (`64a939111891`) | MIT | Optional Scala `.scl` / `.kbm` parser and tuning-table lookup. Fetched source-only via FetchContent only when `PULP_ENABLE_SCALA_TUNING=ON` or when a developer runs `pulp add sst-tuning-library`; Pulp exposes it through `pulp::midi::ScalaTuningProvider` behind the provider-neutral `TuningProvider` API. Not part of the default dependency chain. | pulp-midi | 2026-07-07 |
