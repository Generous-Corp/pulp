...
| V8 | v8-m152-15.2.124.7-cdec2b8282a8 | BSD-3-Clause | JIT JavaScript engine backend (optional, selected with `PULP_JS_ENGINE=v8`; default is QuickJS, with JSC opt-in on Apple). Sealed prebuilt `libv8` (exports only `v8::`/`cppgc::`; bundles ICU/zlib/Abseil internally) via the [danielraffel/v8-builder](https://github.com/danielraffel/v8-builder) milestone-matched m152 release; fetched by `tools/scripts/fetch_v8_for_release.py` into `external/v8-build/`. The active Skia/Dawn provider is m153, so this is an explicitly supported independent mixed-milestone selection until the complete matched V8 m153 release is available and verified by the provider-identity/ODR gate. V8 is unavailable on iOS, where QuickJS and JSC remain supported. See NOTICE.md for bundled ICU/zlib/Abseil attribution. | pulp-view | 2026-08-29 |
