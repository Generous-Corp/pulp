...
| Jost | 3.710 | SIL OFL 1.1 | Embedded UI font (Jost-Regular/Medium/SemiBold/Bold .ttf; SHA-256 `c3143e92…`, `d6ff7726…`, `a63c8d75…`, `3e49280c…`). Bundled so imported designs that specify it are measured and drawn in it rather than in a host substitute ~10% wider per glyph | pulp-view | 2026-08-02 |
...
| Node.js runtime | 22.23.2 | MIT and bundled third-party notices | Official target-architecture executable shipped with `pulp-import-design` so sandboxed plugin hosts do not depend on Homebrew, PATH, or host dylibs. Exact archive SHA-256 pins live in `prepare_node_runtime.py`; the complete upstream `node.LICENSE` ships beside the executable. | design-import | 2026-08-27 |
...
| Brotli | v1.2.0 | MIT | Decoder used by the bundled WOFF2 path so Chromium-captured `@font-face` bytes can be registered by the native renderer. | pulp-canvas | 2026-08-12 |
| WOFF2 | fb9c3379f260 | MIT | Google's WOFF2-to-sfnt decoder. Materialized browser imports register their exact packaged font subsets instead of substituting a system face. | pulp-canvas | 2026-08-12 |
...
| Skia | chrome/m153 | BSD-3-Clause | GPU 2D rendering engine. Pre-built via [danielraffel/skia-builder](https://github.com/danielraffel/skia-builder) fork (adds iOS device/simulator, visionOS, mac-x86_64, and `Skia.xcframework` slices that upstream `olilarkin/skia-builder` doesn't ship). The fork tracks Skia's `chrome/m153` tag and is the active dependency until upstream publishes those slices. Call sites use the `SkShaders::*` gradient namespace + the 3-arg `ParagraphBuilder::make(..., SkUnicode)` signature; m150 also moves `SkRegion::setRects` to an `SkSpan` parameter and changes the `SkStrikeRef` accessor used by the text shaper. The m153 release ships all required platform slices and adds the process-global `SkLogHandler` callback plus Graphite's client-owned `ContextOptions::fExecutor`; adoption of those APIs is separately routed and measured. The bundled skottie + sksg modules (same Skia repo, same BSD-3-Clause license — no separate dependency) are linked only when the opt-in `PULP_LOTTIE` CMake option is enabled and its real link probe succeeds. The `wasm` slice remains Ganesh on WebGL2 (no Dawn or Graphite) and still omits jsonreader/skresources, so Lottie is unavailable there. The wasm slice is validated only against Emscripten 6.0.2 / wasi-sdk 25.0 (`determinism.web_toolchain` in `tools/deps/manifest.json`). | pulp-canvas, pulp-render | 2026-08-29 |
...
| three.js | 077dd13c0e86 | MIT | Pinned native WebGPU runtime for demos, tests, and installed-SDK consumers. Fetched or supplied through `PULP_THREEJS_RUNTIME_DIR` when `PULP_ENABLE_THREEJS_RUNTIME=ON` (the default when GPU support is enabled), then staged under `share/pulp/threejs`. | pulp-render | 2026-08-29 |
...
| V8 | v8-m153-15.3.76.5-26cef0256b0e | BSD-3-Clause | JIT JavaScript engine backend (optional, selected with `PULP_JS_ENGINE=v8`; default is QuickJS, with JSC opt-in on Apple). Sealed prebuilt `libv8` (exports only `v8::`/`cppgc::`; bundles ICU/zlib/Abseil internally) via the [danielraffel/v8-builder](https://github.com/danielraffel/v8-builder) release matched to the active Skia/Dawn chrome/m153 provider generation; fetched by `tools/scripts/fetch_v8_for_release.py` into `external/v8-build/`. Exact provider identity and ODR gates remain mandatory. The m153 Android V8 provider requires API 29+ (general Pulp/QuickJS Android remains API 26); V8 is unavailable on iOS, where QuickJS and JSC remain supported. See NOTICE.md for bundled ICU/zlib/Abseil attribution. | pulp-view | 2026-08-29 |
| Vellum D15 GPU provider | 0.1.7 | MIT plus bundled third-party notices | Opt-in macOS development provider for coordinated Dawn GPU-audio execution. A D15-enabled SDK redistributes `libvellum-gpu.dylib` and the provider's exact LICENSE, NOTICE, and dependency inventory. Production release eligibility remains blocked until Vellum's renderer builder emits and attests an exhaustive transitive source/license manifest. | pulp-gpu-audio (development SDK only) | 2026-09-17 |
