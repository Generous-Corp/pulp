...
| V8 | v8-m152-15.2.124.7-cdec2b8282a8 | BSD-3-Clause | JIT JavaScript engine backend (optional, selected with `PULP_JS_ENGINE=v8`; default is QuickJS, JSC on Apple). Sealed prebuilt `libv8` (exports only `v8::`/`cppgc::`; bundles ICU/zlib/Abseil internally) via the [danielraffel/v8-builder](https://github.com/danielraffel/v8-builder) milestone-matched release for Skia `chrome/m152`; fetched by `tools/scripts/fetch_v8_for_release.py` into `external/v8-build/`. iOS is JSC-only (V8 needs JIT). See NOTICE.md for bundled ICU/zlib/Abseil attribution. | pulp-view | 2026-08-08 |
