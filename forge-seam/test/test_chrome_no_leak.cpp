...
TEST_CASE("an abandoned mention is reconsidered when the library arrives",
          "[mention][catalog]") {
    // THE CORPUS IS NOT FIXED, AND THE SHORT-CIRCUIT ASSUMED IT WAS.
    //
    // A mention that spans a space and matches nothing is given back to the
    // prose, and the token is remembered so the rest of the sentence costs a
    // string compare instead of a search. That is sound only while the library
    // is the one it was decided against -- and the catalogue reloads itself,
    // because on a machine that has never had an index the file arrives about
    // a minute after launch.
    //
    // So this is the ordinary first-run sequence: somebody types a maker's
    // name into an empty library, the index lands carrying that very maker,
    // and the list has to notice.
    std::vector<forge_modular::ModuleEntry> library;
    std::uint64_t generation = 1;

    forge_modular::MentionOverlay overlay;
    auto view = overlay.build();
    overlay.set_source([&](const std::string& q) {
        return forge_modular::search_entries(library, q);
    });
    overlay.set_corpus_generation([&] { return generation; });

    // Nothing is installed and no index has been built yet.
    overlay.handle_text("@Catro Blanco", 13);
    REQUIRE_FALSE(overlay.is_open());

    // Still nothing, and still cheap: within one generation the answer cannot
    // change, so the guard is doing its job here.
    overlay.handle_text("@Catro Blancos", 14);
    CHECK_FALSE(overlay.is_open());

    // The index build finishes. This is one of the two makers the fold rules
    // exist to make nameable, so it is exactly the case that must not be lost.
    library.push_back({"Catro/Blanco", "Rounds", "CatroBlanco/Rounds", false});
    library.push_back({"Catro/Blanco", "Squares", "CatroBlanco/Squares", false});
    ++generation;

    overlay.handle_text("@Catro Blanco", 13);
    CHECK(overlay.is_open());
    REQUIRE_FALSE(overlay.candidates().empty());
    CHECK(overlay.candidates()[0].brand == "Catro/Blanco");

    // And the guard is live again against the NEW library, rather than simply
    // disabled by the first reload.
    overlay.handle_text("@Catro Blanco into a filter", 27);
    CHECK_FALSE(overlay.is_open());
    overlay.handle_text("@Catro Blanco into a filter and", 31);
    CHECK_FALSE(overlay.is_open());
}
...
TEST_CASE("a maker's case is not something anybody has to match either",
          "[mention][catalog]") {
    // CASE-FOLDING ONLY ASCII WAS NOT ENOUGH, AND ONE REAL MAKER PROVED IT.
    //
    // `ÄSK` folded here to "Äsk" -- the A-diaeresis left alone, the ASCII
    // lowercased -- while patch.py's `fold_name` is Unicode-aware in both
    // halves and folded it to "äsk". So "@äsk" named the maker in a prompt and
    // found NOTHING in the list, which is the exact failure this work set out
    // to end, surviving because the sweep that found the others typed every
    // name the way its maker writes it.
    //
    // The other three non-ASCII makers agreed by luck: `Instruō`, `Hügelton
    // Instruments` and `nozoïd` are already lowercase, the one quadrant where
    // the two rules cannot differ.
    //
    // The expected forms below were generated by `patch.py`'s `fold_name`,
    // which is the authority on the other side of the seam, and the same
    // strings are pinned there by `check_fold_agrees_across_the_seam`. Either
    // side drifting fails one of the two.
    using Kind = forge_modular::MentionCandidate::Kind;
    const std::vector<forge_modular::ModuleEntry> lib = {
        {"ÄSK", "Amplifier", "ASK/Amplifier", false},
        {"Instruō", "Tānz", "Instruo/Tanz", false},
        // A DIFFERENT MAKER, one diacritic apart. See the end of this case.
        {"Instruo", "Cesare", "InstruoPlain/Cesare", false},
        {"Hügelton Instruments", "Kickall", "Hugelton/Kickall", false},
        {"nozoïd", "OCS-2", "nozoid/OCS2", false},
        {"ŠKODA", "Octave", "Skoda/Octave", false},
        {"ДЕЛЬТА", "Delay", "Delta/Delay", false},
    };
    struct Case { const char* typed; const char* brand; };
    for (const auto& c : std::vector<Case>{
             // The maker's own spelling, and the one somebody types.
             {"ÄSK", "ÄSK"}, {"äsk", "ÄSK"}, {"Äsk", "ÄSK"}, {"ÄsK", "ÄSK"},
             {"Instruō", "Instruō"}, {"instruō", "Instruō"},
             {"Hügelton", "Hügelton Instruments"},
             {"HÜGELTON", "Hügelton Instruments"},
             {"nozoïd", "nozoïd"}, {"NOZOÏD", "nozoïd"},
             {"ŠKODA", "ŠKODA"}, {"škoda", "ŠKODA"},
             {"ДЕЛЬТА", "ДЕЛЬТА"}, {"дельта", "ДЕЛЬТА"}}) {
        const auto hits = forge_modular::search_entries(lib, c.typed);
        INFO("typed: " << c.typed);
        REQUIRE_FALSE(hits.empty());
        CHECK(hits[0].kind == Kind::brand);
        CHECK(hits[0].brand == c.brand);
    }

    // AND IT IS CASE-FOLDING, NOT TRANSLITERATION. "Instruo" and "Instruō" are
    // two different makers here, and folding the diacritic away would merge
    // them -- a worse failure than the one being fixed, because it is silent
    // and it changes what an already-stored token resolves to.
    //
    // Asserted as a COUNT of maker rows rather than as which one leads: with
    // the diacritic folded away both names become "instruo", both match a
    // whole maker name exactly, and the winner is then whichever the catalogue
    // happens to list first -- which an assertion about hits[0] passes
    // straight through without noticing. That version of this check was
    // written, mutated, and did not fail.
    const auto accented = brands_in(forge_modular::search_entries(lib, "Instruō"));
    REQUIRE(accented.size() == 1);
    CHECK(accented[0].brand == "Instruō");
    const auto plain = brands_in(forge_modular::search_entries(lib, "Instruo"));
    REQUIRE(plain.size() == 1);
    CHECK(plain[0].brand == "Instruo");
}
...
TEST_CASE("a newer release outranks an older installed toolchain",
    ...
    CHECK(forge_modular::compare_stamps("0.13.0-rc10", "0.13.0-rc2") > 0);
    CHECK(forge_modular::compare_stamps("0.13.0-alpha.10",
                                        "0.13.0-alpha.2") > 0);
    CHECK(forge_modular::choose_toolchain(
...
TEST_CASE("Forge Modular renders Stop in the live Build composer and exposes recovery",
          "[build][cancel][recovery][render]") {
    HermeticProjects isolated;
    std::error_code ec;
    const auto dir = std::filesystem::temp_directory_path() /
                     "forge-modular-owned-generation-controls";
    std::filesystem::remove_all(dir, ec);
    std::filesystem::create_directories(dir, ec);
    ...
    REQUIRE(shell.start_build_with("a visible stop regression").empty());
    shell.on_poll();  // deferred Home-row replacement + owned-control sync
    REQUIRE(chrome->mode() == forge::ForgeChrome::Mode::Build);
    REQUIRE(chrome->cancel_action_visible());
    CHECK_FALSE(chrome->failure_actions_visible());

    // Exercise the actual renderer, then inspect only labels beneath visible
    // ancestors. Before this fix the model had already entered Build and this
    // exact tree still painted the generic send arrow; composer_row() saying
    // Stop on the hidden Home screen was not evidence of what the user saw.
    const auto shot = dir / "running.png";
    REQUIRE(pulp::view::render_to_file(
        *view, forge::ForgeChrome::kDesignWidth,
        forge::ForgeChrome::kDesignHeight, shot.string(), 1.0f,
        pulp::view::ScreenshotBackend::skia));
    CHECK(read_all(shot).size() > 20000);

    int visible_stop_labels = 0;
    pulp::view::TextButton* composer_stop = nullptr;
    const std::function<void(pulp::view::View*)> walk =
        [&](pulp::view::View* v) {
            if (!v || !v->visible()) return;
            if (auto* label = dynamic_cast<pulp::view::Label*>(v);
                label && label->text() == "Stop") {
                ++visible_stop_labels;
                if (auto* parent = dynamic_cast<pulp::view::TextButton*>(v->parent()))
                    composer_stop = parent;
            }
            for (std::size_t i = 0; i < v->child_count(); ++i)
                walk(v->child_at(i));
        };
    walk(view.get());
    REQUIRE(visible_stop_labels == 1);
    REQUIRE(composer_stop != nullptr);
    CHECK(composer_stop->access_label() == "Stop generation");
    REQUIRE(composer_stop->on_click);

    composer_stop->on_click();
    CHECK(engine.cancellations == 1);
    CHECK_FALSE(composer_stop->is_enabled());

    { std::ofstream f(log, std::ios::app); f << "generation cancelled by user\n"; }
    shell.on_poll();
    CHECK_FALSE(chrome->cancel_action_visible());
    REQUIRE(chrome->failure_actions_visible());

    // Edit restores the exact retained request without spending another run.
    REQUIRE(chrome->edit_replayable_prompt());
    REQUIRE(chrome->followup_input() != nullptr);
    CHECK(chrome->followup_input()->text() == "a visible stop regression");
    CHECK_FALSE(chrome->failure_actions_visible());
    CHECK(engine.submissions.size() == 1);

    // A second terminal turn exposes Retry, and Retry goes back through the
    // shell's ordinary launch guard rather than bypassing it.
    { std::ofstream f(log, std::ios::trunc); }
    REQUIRE(shell.start_build_with("the edited stop regression").empty());
    shell.on_poll();
    { std::ofstream f(log, std::ios::app); f << "generation cancelled by user\n"; }
    shell.on_poll();
    REQUIRE(chrome->failure_actions_visible());
    { std::ofstream f(log, std::ios::trunc); }
    REQUIRE(chrome->retry_replayable_prompt());
    CHECK(engine.submissions.size() == 3);
    CHECK(engine.submissions.back().first == "the edited stop regression");
    CHECK(chrome->cancel_action_visible());
