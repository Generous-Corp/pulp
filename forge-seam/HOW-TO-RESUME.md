...
git fetch origin
# One command. It applies the chrome patch, copies the shell sources, the
# plugin directory and the tests, and then VERIFIES what it produced. Those
# last steps used to be prose here, and a worktree that skipped them
# configured three products instead of four and built with no chrome coverage
# at all -- without complaining, because `git apply --check` only ever answers
# "does this apply", never "is this everything".
#
# The base is NOT origin/main, which moves and against which the patch will
# not apply. populate.sh reads it from patches/BASE.
git worktree add /tmp/forge-cur "$(cat /Volumes/Workshop/Code/pulp-modular-rack/forge-seam/patches/BASE)"
/Volumes/Workshop/Code/pulp-modular-rack/forge-seam/populate.sh /tmp/forge-cur
cd /tmp/forge-cur
...
./build/forge-test-chrome-no-leak     # expect all green (90 cases)
