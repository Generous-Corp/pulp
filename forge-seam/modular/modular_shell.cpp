...
bool ForgeModularShell::owned_generation_replayable() const {
    if (in_flight_ || last_request_.empty() || !watching_) return false;
    const auto outcome = monitor_.outcome();
    return outcome == BuildOutcome::failed || outcome == BuildOutcome::refused;
}

bool ForgeModularShell::cancel_owned_generation() {
    if (!in_flight_ || !engine_) return false;
    stop_build();
    return true;
}

bool ForgeModularShell::retry_owned_generation() {
    if (!owned_generation_replayable()) return false;
    const std::string prompt = last_request_;
    return start_build_with(prompt).empty();
}

bool ForgeModularShell::edit_owned_generation() {
    if (!owned_generation_replayable()) return false;
    auto* c = chrome();
    if (!c || !c->followup_input()) return false;
    c->enter_build();
    c->followup_input()->set_text(last_request_);
    return true;
}
