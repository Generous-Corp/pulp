...
function(pulp_add_rack_plugin target)
    ...
    set(_rack_target_processor "${CMAKE_SYSTEM_PROCESSOR}")
    set(_rack_universal FALSE)
    if(APPLE AND CMAKE_OSX_ARCHITECTURES)
        set(_rack_target_processor "${CMAKE_OSX_ARCHITECTURES}")
    endif()
    if(_rack_target_processor MATCHES "arm64"
       AND _rack_target_processor MATCHES "x86_64")
        set(_rack_cpus "arm64;x64")
        set(_rack_universal TRUE)
    elseif(_rack_target_processor MATCHES "^(arm64|aarch64)$")
        target_compile_options(${target} PRIVATE -march=armv8-a+fp+simd)
        set(_rack_cpus "arm64")
    elseif(_rack_target_processor MATCHES "^(x86_64|AMD64|amd64)$")
        target_compile_options(${target} PRIVATE -march=nehalem)
        set(_rack_cpus "x64")
    else()
        message(FATAL_ERROR
            "pulp_add_rack_plugin(${target}): unsupported target architecture "
            "'${_rack_target_processor}'. Rack packages must target exactly one "
            "of arm64 or x86_64.")
    endif()
    ...
    set(_rack_packages)
    foreach(_rack_cpu IN LISTS _rack_cpus)
        list(APPEND _rack_packages
            "${CMAKE_BINARY_DIR}/rack/${RACK_SLUG}-${RACK_VERSION}-${_rack_os}-${_rack_cpu}.vcvplugin")
    endforeach()
    set(_rack_known_packages
        "${CMAKE_BINARY_DIR}/rack/${RACK_SLUG}-${RACK_VERSION}-${_rack_os}-arm64.vcvplugin"
        "${CMAKE_BINARY_DIR}/rack/${RACK_SLUG}-${RACK_VERSION}-${_rack_os}-x64.vcvplugin")

    set(_package_inputs ${RACK_SOURCES} ${RACK_LICENSES})
    set(_stage_commands
        COMMAND ${CMAKE_COMMAND} -E rm -f ${_rack_known_packages}
    ...
    if(PULP_ZSTD_EXE)
        set(_package_commands)
        list(LENGTH _rack_packages _rack_package_count)
        math(EXPR _rack_package_last "${_rack_package_count} - 1")
        foreach(_rack_package_index RANGE ${_rack_package_last})
            list(GET _rack_packages ${_rack_package_index} _pkg)
            list(GET _rack_cpus ${_rack_package_index} _rack_cpu)
            set(_tar "${_pkg}.tar")
            if(_rack_universal)
                set(_rack_binary_arch "${_rack_cpu}")
                if(_rack_cpu STREQUAL "x64")
                    set(_rack_binary_arch "x86_64")
                endif()
                list(APPEND _package_commands
                    COMMAND /usr/bin/lipo "$<TARGET_FILE:${target}>"
                            -thin "${_rack_binary_arch}"
                            -output "${_stage}/$<TARGET_FILE_NAME:${target}>")
            else()
                list(APPEND _package_commands
        ...
        add_custom_target(${target}-package ALL
            ${_stage_commands}
            ${_package_commands}
            BYPRODUCTS ${_rack_packages}
            DEPENDS ${target} ${_package_inputs}
            VERBATIM
            COMMENT "Packaging ${RACK_SLUG} ${_rack_os} targets: ${_rack_cpus}")
    elseif(APPLE AND EXISTS "/usr/bin/tar")
        ...
        add_custom_target(${target}-package ALL
            ...
            DEPENDS ${target} ${_package_inputs}
            VERBATIM
            COMMENT "Packaging ${RACK_SLUG} ${_rack_os} targets: ${_rack_cpus} with system tar")
    else()
