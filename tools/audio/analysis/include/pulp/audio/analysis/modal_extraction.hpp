...
///
/// Shipped offline/tool layer only — every entry point analyzes a buffer that
/// has already left the audio thread. Nothing here is realtime-safe, and this
/// target must not be pulled into an audio processor merely because it is
...
// ── Shipped offline compiler ─────────────────────────────────────────────

enum class ModalExtractionError {
    none,
    empty_input,
    input_too_short,
    silent_input,
...
struct ModalExtractionOptions {
    ModeAnalysisOptions analysis{};
    /// Requested input bound. The compiler also enforces an absolute 2^22
    /// sample ceiling and a 2^20 FFT ceiling regardless of caller options.
    int max_input_samples = 1 << 20;
    int max_modes = 64;
    /// Qualified candidates closer than this are one mode. The higher
    /// amplitude*confidence candidate wins; the other remains in the ledger
    /// with `merged == true`. The analyzer's FFT resolution remains a harder
    /// lower bound on separability.
    double min_separation_cents = 25.0;
    double confidence_floor = 0.70;
    double snr_floor_db = -60.0;
    std::string recipe = "modal-extraction-v1";
};

struct ModalExtractionCandidate {
    pulp::signal::ModalMode mode{};
    double confidence = 0.0;
    double prominence_db = 0.0;
...
struct ModalExtractionResult {
    ModalExtractionError error = ModalExtractionError::none;
    bool ok = false;
    std::string message;
    std::string recipe;
    pulp::signal::ModalSpec spec;
    std::vector<ModalExtractionCandidate> candidates;
};

/// Compile a bounded, mono impulse response into the existing ModalSpec data
/// model. All storage and FFT work are offline/control-thread operations.
/// Candidates are ranked by amplitude * confidence, ties by frequency, while
/// the emitted spec is always frequency sorted. The candidate ledger retains
/// accepted and rejected evidence for an auditable result.
