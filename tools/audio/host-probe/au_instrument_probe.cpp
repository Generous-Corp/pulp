...
//
// Usage:
//   au_instrument_probe --name "Reference Instrument" [--out /tmp/reference.wav]
...
void print_usage(const char* program) {
    std::printf(
        "Usage: %s --name NAME [--out PATH] [--note MIDI_NOTE] "
        "[--seconds N] [--hits MS:VEL,...] [--list-params] "
        "[--set-param ID=VALUE] [--allow-silent]\n",
        program);
}
...
int main(int argc, char** argv) {
    ...
    if (want_name.empty()) {
        std::printf("FAIL: --name is required\n");
        print_usage(argv[0]);
        return 1;
    }
