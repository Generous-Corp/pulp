...
from ..dsp import attack_rise as _attack_rise  # shared attack primitive (also compare's transient axis)
from ..dsp import onset_attack_deficit  # shared per-onset deficit (also compare's transient axis)
...
def detect(
    ...
    for ref_t, cand_t in onset_pairs:
        deficit = onset_attack_deficit(reference, candidate, sr, ref_t, cand_t)
        if deficit is None:                       # boundary / silent reference attack — unmeasurable
            continue
