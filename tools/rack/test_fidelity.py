...
def test_the_shipped_portmap_finds_pitch_on_a_fresh_machine() -> int:
    """Tap selection must not depend on a machine-local Rack scan."""
    held_local_path = portmap_seed.LOCAL_PATH
    with tempfile.TemporaryDirectory() as stage:
        portmap_seed.LOCAL_PATH = os.path.join(stage, "missing-portmap.json")
        try:
            _, taps = F.instrument(fixture())
        finally:
            portmap_seed.LOCAL_PATH = held_local_path
    return check(any(t[2] == "pitch" and t[0] == SEQ for t in taps),
                 "the shipped port map identifies the sequencer pitch output",
                 str(taps))


def test_one_probe_keeps_eight_named_series_synchronized() -> int:
    """The acid contract must not lose its later taps to a four-input cap."""
    taps = [(SEQ, i, kind) for i, kind in enumerate((
        "clock", "raw_pitch", "post_slew_pitch", "accent", "slide",
        "cutoff", "filter_audio", "audio"))]
    p, used = F.instrument(fixture(), taps=taps)
    probe = next(m for m in p["modules"] if m["model"] == F.PROBE_MODEL)
    probe_cables = [c for c in p["cables"]
                    if c["inputModuleId"] == probe["id"]]
    bad = check(used == taps,
                "all eight behavior series survive instrumentation", str(used))
    bad += check([c["inputId"] for c in probe_cables] == list(range(8)),
                 "the series occupy one probe clock in declared order",
                 str(probe_cables))
    return bad
...
def main(argv: list[str]) -> int:
    ...
    for fn in (test_a_faithful_load_reads_as_faithful,
               ...
               test_the_shipped_portmap_finds_pitch_on_a_fresh_machine,
               test_one_probe_keeps_eight_named_series_synchronized,
