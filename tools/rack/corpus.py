...
"""Fetch the texts an idiom may cite as `read`, into a directory we never commit.
    ...
    python3 corpus.py --index    # update the machine-local searchable index
    python3 corpus.py --query "sample and hold"  # retrieve bounded passages
...
if __name__ == "__main__":
    if "--index" in sys.argv:
        ingest_local()
        import source_index                    # noqa: PLC0415
        result = source_index.build_index(CORPUS)
        print("indexed "
              f"{result.sources} sources / {result.chunks} passages "
              f"({result.added} added, {result.updated} updated, "
              f"{result.removed} removed, {result.unchanged} unchanged)")
        sys.exit(0)
    if "--query" in sys.argv:
        query_at = sys.argv.index("--query")
        if query_at + 1 >= len(sys.argv):
            print("--query needs search text", file=sys.stderr)
            sys.exit(2)
        import source_index                    # noqa: PLC0415
        sys.exit(source_index.main([
            "--corpus", CORPUS, "query", sys.argv[query_at + 1],
        ]))
