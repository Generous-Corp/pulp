...
def vendor_inventory() -> dict:
    ...
    sys.path.insert(0, HERE)
    import patch as patch_mod
    with open(VENDOR, encoding="utf-8") as source:
...
LINT_CASES = [
    ...
    ("a non-boolean substitution policy",
     lambda i: i["acid-voice"].__setitem__("substitutable", "sometimes"),
     "non-boolean substitutable"),

    ("a non-boolean measured-behaviour policy",
     lambda i: i["acid-voice"].__setitem__(
         "require_measured_behaviour", "eventually"),
     "non-boolean require_measured_behaviour"),

    ("a hard modifier naming no known module role",
     lambda i: i["acid-voice"]["hard_modifiers"][0].__setitem__(
         "role", "magic_box"),
     "unknown role"),

    ("a hard modifier with no exact count",
     lambda i: i["acid-voice"]["hard_modifiers"][0].__setitem__(
         "exactly", "one"),
     "invalid exact count"),

    ("an intrinsic exact count naming no known module role",
     lambda i: i["acid-voice"]["exactly"].__setitem__("magic_box", 1),
     "exact count for unknown role"),

    ("an intrinsic exact count that is not a count",
     lambda i: i["acid-voice"]["exactly"].__setitem__("sequencer", "one"),
     "invalid exact count"),

    ("a distinct-lane group naming no topology requirement",
     lambda i: i["acid-voice"]["distinct_source_lanes"][0][
         "requirements"].append("imaginary-lane"),
     "names unknown requirement"),
...
def check_explicit_acid_contract() -> int:
    """Acid is exact, its count modifier binds, and behavior cannot vanish."""
    ...
    read = idiom_check.resolve_all(prompt, idioms, roles)
    if intent.slug != "acid-voice" or intent.how != "named":
        print(f"  WRONG  explicit acid resolved as {intent}")
        bad += 1
    elif read.alternatives:
        print(f"  WRONG  explicit acid can be substituted by "
              f"{read.alternatives}")
        bad += 1
    elif "sequenced-voice" in read.also:
        print("  WRONG  ordinary sequenced voice is offered as a layer on top "
              "of exact acid")
        bad += 1
    else:
        print("  ok     explicit acid is one non-substitutable contract")

    # Preserve the useful broadening for the generic request that motivated
    # alternatives in the first place.
    generic = idiom_check.resolve_all(
        "a highly melodic patch using cv funk modules", idioms, roles)
    if len(generic.alternatives) < 3:
        print("  WRONG  exact acid handling removed generic melody routes")
        bad += 1
    else:
        print(f"  ok     generic melody still offers "
              f"{len(generic.alternatives)} valid routes")

    acid = idiom_check.synthesize(idioms["acid-voice"], inv, roles)
    ordinary = idiom_check.synthesize(idioms["sequenced-voice"], inv, roles)
    if acid is None or ordinary is None:
        print("  WRONG  the acid/voice control fixtures are not buildable")
        return bad + 1
    diagnosis = patch_mod.diagnose_intent(prompt, ordinary, inv, intent, idioms)
    if diagnosis.built or not diagnosis.topology:
        print("  WRONG  an ordinary sequenced voice passed explicit acid")
        bad += 1
    else:
        print("  ok     a generic sequenced voice cannot pass explicit acid")

    modifiers = idiom_check.bound_modifiers(
        prompt, idioms["acid-voice"])
    if len(modifiers) != 1 or idiom_check.check_modifiers(
            acid, inv, modifiers, roles):
        print("  WRONG  single sequencer did not bind to the valid acid patch")
        bad += 1
    else:
        two = copy.deepcopy(acid)
        sequencer = next(m for m in two["modules"]
                         if idiom_check._module_matches(
                             "sequencer", idiom_check._entry(inv, m), roles))
        duplicate = copy.deepcopy(sequencer)
        duplicate["id"] = max(m["id"] for m in two["modules"]) + 1
        two["modules"].append(duplicate)
        problems = idiom_check.check_modifiers(two, inv, modifiers, roles)
        if not problems or "found 2" not in problems[0]:
            print(f"  WRONG  two sequencers passed single: {problems}")
            bad += 1
        else:
            print("  ok     two sequencers fail the bound single modifier")

    for punctuated in ("make acid with a single sequencer, please",
                       "make acid with one-sequencer."):
        if len(idiom_check.bound_modifiers(
                punctuated, idioms["acid-voice"])) != 1:
            print(f"  WRONG  punctuation hid the hard modifier in "
                  f"{punctuated!r}")
            bad += 1
        else:
            print(f"  ok     punctuation still binds {punctuated!r}")

    # Acid intrinsically means one sequencer and three independent CV lanes;
    # these are not optional consequences of somebody saying "single".
    two = copy.deepcopy(acid)
    sequencer = next(m for m in two["modules"]
                     if idiom_check._module_matches(
                         "sequencer", idiom_check._entry(inv, m), roles))
    ...
    duplicate["id"] = max(m["id"] for m in two["modules"]) + 1
    two["modules"].append(duplicate)
    intrinsic = idiom_check.check(two, inv, idioms["acid-voice"], roles)
    if not any("exactly 1 sequencer" in problem for problem in intrinsic):
        print(f"  WRONG  named acid accepted a second sequencer: {intrinsic}")
        bad += 1
    else:
        print("  ok     named acid rejects a second sequencer intrinsically")

    reqs = {r["id"]: r for r in idioms["acid-voice"]["topology"]}
    no_slide = idiom_check.apply_mistake(
        acid, {"do": "cut", "requirement": "slide-control",
               "_topology": idioms["acid-voice"]["topology"]}, inv, roles)
    no_slide_problems = (idiom_check.check(
        no_slide, inv, idioms["acid-voice"], roles) if no_slide else [])
    if not no_slide or reqs["slide-control"]["describe"] not in \
            no_slide_problems:
        print(f"  WRONG  acid accepted no explicit slide-control lane: "
              f"{no_slide_problems}")
        bad += 1
    else:
        print("  ok     acid rejects a missing slide-control lane")

    shared = copy.deepcopy(acid)
    seq_id = sequencer["id"]
    pitch_cable = next(c for c in shared["cables"]
                       if c["outputModuleId"] == seq_id
                       and c["inputModuleId"] == next(
                           m["id"] for m in shared["modules"]
                           if idiom_check._module_matches(
                               "slew", idiom_check._entry(inv, m), roles))
                       and c["inputId"] == 4)
    for cable in shared["cables"]:
        if cable["outputModuleId"] != seq_id:
            continue
        prole, label = idiom_check._port_info(
            inv, sequencer, "out", cable["outputId"])
        if idiom_check._port_matches("cv_out", prole, label, roles):
            cable["outputId"] = pitch_cable["outputId"]
    shared_problems = idiom_check.check(
        shared, inv, idioms["acid-voice"], roles)
    if not any("three distinct output lanes" in problem
               for problem in shared_problems):
        print(f"  WRONG  acid accepted one CV lane shared by pitch, accent, "
              f"and slide: {shared_problems}")
        bad += 1
    else:
        print("  ok     acid rejects a shared pitch/accent/slide lane")

    understood = idiom_check.reading(prompt, idioms)["known"]
    if "single" not in understood or "acid-voice" not in understood["single"]:
        print("  WRONG  the gate binds single while the request reader says "
              "nothing checks it")
        bad += 1
    else:
        print("  ok     the request reader reports single as a checked word")

    diagnosis = patch_mod.diagnose_intent(prompt, acid, inv, intent, idioms)
    if diagnosis.topology:
        print(f"  WRONG  the positive acid topology fails: "
              f"{diagnosis.topology}")
        bad += 1
    elif not diagnosis.deferred or not all("UNMEASURED" in d
                                           for d in diagnosis.deferred):
        print("  WRONG  topology-only acid is labeled full behavior success")
        bad += 1
    else:
        print(f"  ok     topology holds but {len(diagnosis.deferred)} dynamic "
              "behavior claim(s) fail closed as UNMEASURED")

    text = patch_vocabulary.for_prompt(prompt, idioms)
    if "must contain exactly one sequencer" not in text:
        print("  WRONG  the bound single-sequencer gate never reaches the model")
        bad += 1
    else:
        print("  ok     the exact modifier reaches the model contract")
    return bad
...
def check_named_sounds() -> int:
    ...
    core_alias = {
        "core": {"id": "core", "kind": "settings", "family": "effects",
                 "names": ["core"], "what": "wrong", "why": "wrong"}}
    if knowledge.for_prompt(
            "use Core/AudioInterface2 as the output", core_alias):
        print("  WRONG  a qualified Core module name became a sound recipe")
        bad += 1
    else:
        print("  ok     qualified module identities never become sound recipes")
...
def main() -> int:
    ...
    print("\nan explicitly named acid request stays exact:")
    bad += check_explicit_acid_contract()
    ...
    with open(os.path.join(HERE, "prompt", "patch_contract.md"),
              encoding="utf-8") as source:
    ...
    if "Every identity/index field is a JSON integer" not in contract:
        print("  WRONG  the model is not told Rack identities require JSON integers")
        bad += 1
    else:
        print("  ok     exact Rack JSON integer identity types reach the model")
