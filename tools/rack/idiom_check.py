...
def resolve_all(prompt: str, idioms: dict | None = None,
    ...
    # A named idiom is not a request for anything vaguely adjacent to it.
    # "acid" names slide + accent + the 303 topology; accepting an ordinary
    # sequenced voice because both eventually drive oscillator pitch erases
    # the distinction the user supplied. Alternatives exist for GENERIC
    # requests ("a melodic patch") and nearest-match guidance, where several
    # structures really can answer the same broad intent. An idiom may also
    # declare itself non-substitutable for a strong implied phrase such as
    # "Roland 303 sound".
    may_substitute = (intent.how in ("implied", "nearest") and
                      idioms[intent.slug].get("substitutable", True))
    all_kin = set(alternatives_to(intent.slug, idioms, roles))
    kin = all_kin if may_substitute else set()
    return Reading(
        intent,
        sorted(touched & kin),
        # Structurally alternative idioms do not become composable `also`
        # suggestions merely because this exact request forbids substituting
        # them. An ordinary sequenced voice is neither an acid substitute nor
        # a layer to put on top of acid.
        sorted(s for s in touched - all_kin
               if idioms[s].get("kind") != "fragment"),
        sorted(s for s in touched if idioms[s].get("kind") == "fragment"))


class HardModifier(NamedTuple):
    """A literal request qualifier that becomes a deterministic patch gate."""
    phrase: str
    role: str
    exactly: int
    describe: str
...
def bound_modifiers(prompt: str, idiom: dict) -> list[HardModifier]:
    """Hard modifiers from `prompt` that this idiom promises to enforce.

    These live on the idiom rather than in a global bag of English guesses.
    "single sequencer" is meaningful for the acid topology and must not be
    acknowledged in prose while disappearing from validation. A phrase that
    is not declared remains ordinary prompt text; this function never invents
    a rejecting constraint from resemblance.
    """
    # Match words, not the punctuation somebody happened to put after them.
    # The old whitespace-only normalization missed "single sequencer," and
    # "one-sequencer." -- exactly the ordinary prose forms this gate exists
    # to bind.
    ...
    text = phrase_text(prompt)
    out: list[HardModifier] = []
    for spec in idiom.get("hard_modifiers") or []:
        for phrase in spec.get("phrases") or []:
            normalized = phrase_text(phrase).strip()
            if normalized and phrase_text(phrase) in text:
                out.append(HardModifier(
                    normalized, spec.get("role", "any"),
                    int(spec.get("exactly", 0)),
                    spec.get("describe") or
                    f"needs exactly {spec.get('exactly')} {spec.get('role')} "
                    "module(s)"))
                break
    return out


def check_modifiers(patch: dict, inv: dict, modifiers: list[HardModifier],
                    roles: dict | None = None) -> list[str]:
    """Every bound hard modifier the patch violates, in request language."""
    roles = roles if roles is not None else load_roles()
    problems = []
    for modifier in modifiers:
        have = sum(1 for module in patch.get("modules", [])
                   if _module_matches(modifier.role, _entry(inv, module), roles))
        if have != modifier.exactly:
            problems.append(f"{modifier.describe}; found {have}")
    return problems
...
def reading(prompt: str, idioms: dict | None = None) -> dict:
    ...
    idioms = idioms if idioms is not None else load_idioms()
    asked = _words(prompt)
    resolved = resolve_exact(prompt, idioms)
    known: dict[str, list[str]] = {}
    for slug, idiom in idioms.items():
        words = _words(" ".join(list(idiom.get("names", [])) +
                                list(idiom.get("implies", []))) +
                       " " + slug.replace("-", " "))
        # Bound modifiers are part of the deterministic reading too. Without
        # this, "single sequencer" was enforced by the gate while the log said
        # "nothing checks single" -- exactly the silent-ignore signal this
        # layer is meant to eliminate.
        if slug == resolved and bound_modifiers(prompt, idiom):
            for modifier in idiom.get("hard_modifiers") or []:
                words |= _words(" ".join(modifier.get("phrases") or []))
...
def check(patch: dict, inv: dict, idiom: dict, roles: dict | None = None,
    ...
    # Some techniques need several independent control lanes from one source.
    # Merely satisfying each connection is insufficient: one sequencer CV
    # copied to pitch, accent, and slide-control is still one lane wearing
    # three labels. Prove that the named requirements can be assigned distinct
    # physical output jacks.
    topology = {r.get("id"): r for r in idiom.get("topology", [])}
    for group in idiom.get("distinct_source_lanes") or []:
        requirements = [topology.get(rid)
                        for rid in group.get("requirements") or []]
        lane_sets = [_source_lanes(patch, by_id, inv, roles, req, candidates)
                     if req else set() for req in requirements]

        def assign(index: int, used: set) -> bool:
            if index == len(lane_sets):
                return True
            return any(assign(index + 1, used | {lane})
                       for lane in lane_sets[index] if lane not in used)
    ...
    for role, n in (idiom.get("exactly") or {}).items():
        have = len(candidates(role))
        if have != n:
            problems.append(f"needs exactly {n} {role} module(s), found {have}")

    return problems


def _source_lanes(patch: dict, by_id: dict, inv: dict, roles: dict, req: dict,
                  candidates) -> set[tuple[int, int]]:
    """Physical source output jacks that can satisfy one requirement.

    The source identity survives transparent attenuators/multiples, just as it
    does in `_satisfied`; otherwise an accent routed through a CV mixer would
    appear to originate at the mixer and a shared sequencer lane would evade
    the distinct-lane check.
    """
    froms = candidates(req.get("from_module", "any"))
    tos = candidates(req.get("to_module", "any"))
    transparent = {mid for mid, module in by_id.items()
                   if any(roles["roles"].get(role, {}).get("transparent")
                          and _module_matches(role, _entry(inv, module), roles)
                          for role in roles["roles"])}
    outgoing: dict[int, list] = {}
    for cable in patch.get("cables", []):
        outgoing.setdefault(cable.get("outputModuleId"), []).append(cable)

    queue: list[tuple[dict, tuple[int, int]]] = []
    for src_id in froms:
        src = by_id.get(src_id)
        if src is None:
            continue
        for cable in outgoing.get(src_id, []):
            output_id = cable.get("outputId", 0)
            prole, label = _port_info(inv, src, "out", output_id)
            if _port_matches(req.get("from_port", "any_out"), prole, label,
                             roles) or _uncartographed(inv, src, "out"):
                queue.append((cable, (src_id, output_id)))

    found: set[tuple[int, int]] = set()
    seen = set()
    while queue:
        cable, origin = queue.pop()
        key = (origin, cable.get("outputModuleId"), cable.get("outputId", 0),
        ...
        if dst is None:
            continue
        if dst_id in tos:
            if req.get("same_module") and origin[0] != dst_id:
                pass
            elif req.get("different_module") and origin[0] == dst_id:
                pass
            else:
                prole, label = _port_info(inv, dst, "in",
                                           cable.get("inputId", 0))
                if _port_matches(req.get("to_port", "any_in"), prole, label,
                                 roles) or _uncartographed(inv, dst, "in"):
                    found.add(origin)
...
def library_problems(idioms: dict | None = None,
    ...
    for slug, idiom in sorted(idioms.items()):
        ...
        if not isinstance(idiom.get("substitutable", True), bool):
            bad.append(f"{slug} has non-boolean substitutable metadata")
        if not isinstance(idiom.get("require_measured_behaviour", False), bool):
            bad.append(f"{slug} has non-boolean require_measured_behaviour "
                       "metadata")
        for role, count in (idiom.get("exactly") or {}).items():
            if role not in roles.get("roles", {}):
                bad.append(f"{slug} has an exact count for unknown role "
                           f"{role!r}")
        ...
        for group in idiom.get("distinct_source_lanes") or []:
            ...
            for requirement_id in requirement_ids:
                if requirement_id not in topology_ids:
                    bad.append(f"{slug} distinct-source group names unknown "
        ...
        for modifier in idiom.get("hard_modifiers") or []:
            ...
            exactly = modifier.get("exactly")
            if not phrases or not all(isinstance(p, str) and p.strip()
                                      for p in phrases):
                bad.append(f"{slug} has a hard modifier with no phrases")
            if role not in roles.get("roles", {}):
                bad.append(f"{slug} has a hard modifier for unknown role "
                           f"{role!r}")
            if not isinstance(exactly, int) or isinstance(exactly, bool) \
                    or exactly < 0:
                bad.append(f"{slug} has a hard modifier with invalid exact "
                           f"count {exactly!r}")
            if not modifier.get("describe"):
                bad.append(f"{slug} has a hard modifier with no description")
...
def apply_mistake(patch: dict, mistake: dict, inv: dict, roles: dict) -> dict | None:
    ...
    def matching_cables(requirement_id: str) -> list[dict]:
        ...
        def matches(c) -> bool:
            ...
            if src is None or dst is None:
                return False
            if not _module_matches(req.get("from_module", "any"),
                                   _entry(inv, src), roles):
                return False
            if not _module_matches(req.get("to_module", "any"),
                                   _entry(inv, dst), roles):
                return False
            if req.get("same_module") and src.get("id") != dst.get("id"):
                return False
            srole, slabel = _port_info(inv, src, "out", c.get("outputId", 0))
            drole, dlabel = _port_info(inv, dst, "in", c.get("inputId", 0))
            return (_port_matches(req.get("from_port", "any_out"),
                                  srole, slabel, roles)
                    and _port_matches(req.get("to_port", "any_in"),
                                      drole, dlabel, roles))

        return [c for c in out.get("cables", []) if matches(c)]

    if kind == "cut":
        # Remove the cables satisfying one requirement, by its id.
        target = mistake.get("requirement")
        matched = matching_cables(target)
        before = len(out.get("cables", []))
        out["cables"] = [c for c in out.get("cables", []) if c not in matched]
        if len(out["cables"]) == before:
            return None                  # nothing to cut: not a real control
        return out

    if kind == "share_source_lane":
        # Keep both destination cables but collapse the named lane onto the
        # donor's physical output jack. This is the destructive control for a
        # distinct-source contract: two wires do not make stereo, accent, or
        # slide independent when they are copies of one source lane.
        target = matching_cables(mistake.get("requirement"))
        donor = matching_cables(mistake.get("source_requirement"))
        if not target or not donor:
            return None
        donor_cable = donor[0]
        # Requirements can intentionally have the same role/port shape (left
        # and right both say audio -> output). In that case both match lists
        # contain both fixture cables, so choose a different object rather
        # than accidentally copying a cable onto itself.
        target_cable = next((c for c in reversed(target)
                             if c is not donor_cable), None)
        if target_cable is None:
            return None
    ...
    if kind == "wire":
        ...
        if src is None or dst is None:
            return None
        existing_cables = out.setdefault("cables", [])
        existing_cables.append({
            "id": max((c.get("id", 0) for c in existing_cables
                       if isinstance(c, dict) and type(c.get("id")) is int),
                      default=0) + 1,
...
def synthesize(idiom: dict, inv: dict, roles: dict) -> dict | None:
    ...
    for group in idiom.get("distinct_source_lanes") or []:
        for index, requirement_id in enumerate(group.get("requirements") or []):
            lane_index[requirement_id] = index
    ...
    for req in idiom.get("topology", []):
        ...
        if (dst, in_index) in taken:
            if roles["roles"].get(t_role, {}).get("fan_in") \
                    or t_role == "output":
                # A mixer/multiple accepts repeated channels, while an output
                # interface has parallel physical channels. The fixture gives
                # either another jack of the same kind; the real checker still
                # requires that exact jack to exist in Cartog inventory.
