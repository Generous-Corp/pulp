...
def for_prompt(prompt: str, entries: dict | None = None) -> list[dict]:
    ...
    entries = entries if entries is not None else load()
    # Qualified Rack identities are inventory constraints, not musical words.
    # In particular, Core/AudioInterface2 must not summon a cookbook entry
    # whose alias happens to be the single word "core".
    musical_prompt = re.sub(
        r"(?:@)?[A-Za-z0-9_.+-]+/[A-Za-z0-9_.+-]+", " ", prompt)
    words = set(re.findall(r"[a-z]+", musical_prompt.lower()))
    if not words:
        return []
