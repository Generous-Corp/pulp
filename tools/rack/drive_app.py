...
Usage:
    ...
    drive_app.py random
    drive_app.py status
    drive_app.py open /absolute/path/to/generated.vcv
    drive_app.py quit
"""
...
from rack_open import open_evidence as rack_open_evidence
...
def main(argv: list[str]) -> int:
    ...
    if cmd == "open":
        ...
        if len(argv) < 3 or not os.path.isabs(argv[2]):
            print("usage: drive_app.py open /absolute/path/to/generated.vcv")
            return 2
        expected_patch = os.path.realpath(argv[2])
        try:
            with open(expected_patch) as f:
                expected_modules = len(json.load(f).get("modules", []))
        except (OSError, ValueError, TypeError) as error:
            print(f"FAIL: cannot read the expected Rack patch: {error}")
            return 1
        if expected_modules < 1:
            print("FAIL: the expected Rack patch contains no modules")
            return 1
        focus()
        ...
        for _ in range(40):
            ...
            evidence = rack_open_evidence(
                fresh, expected_patch, expected_modules)
            if evidence:
                for ln in evidence:
                    print("  " + ln.strip())
                print("PASS: Rack opened the artifact")
                return 0
        print("FAIL: Rack never reported loading the exact patch and creating "
              "a module after the click")
        return 1
