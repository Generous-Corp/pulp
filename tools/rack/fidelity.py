...
PROBE_MODEL = "PROBE"
# Acid behavior needs synchronized observations at the clock, both sides of
# the slew, accent and slide lanes, the filter boundary, and the final output.
# Keep one probe and one engine run so those series share a sample clock.
PROBE_INPUTS = 8
...
def read_portmap() -> dict:
    """The shipped Rack knowledge, refined by this machine's local scan."""
    return {"modules": portmap_seed.entries()}
...
def instrument(patch: dict, taps: list[tuple] | None = None
    ...
    if taps is None:
        taps = choose_taps(patch, port_names(read_portmap()))
