...
# The production default belongs in Application Support; a test corpus never
# does. Set this before importing patch.py because its attempt root is resolved
# at import time. A crash may leave a disposable /tmp directory, but can never
# add a fake paid attempt to the user's history.
_TEST_ATTEMPT_ROOT = tempfile.mkdtemp(prefix="forge-test-patch-attempts-")
os.environ["FORGE_ATTEMPT_DIR"] = _TEST_ATTEMPT_ROOT
...
CASES = [
    ...
    ("JSON-real module ids collapse when Rack loads them", {
        "version": "2.6.6",
        "modules": [mod(1.0, "Fundamental", "VCO"),
                    mod(2, "Core", "AudioInterface2", (10, 0))],
        "cables": [cable(1, 1.0, 0, 2, 0)]}, False,
     "module id 1.0 must be a JSON integer"),
    ("JSON-real cable references do not identify Rack modules", {
        "version": "2.6.6",
        "modules": [mod(1, "Fundamental", "VCO"),
                    mod(2, "Core", "AudioInterface2", (10, 0))],
        "cables": [cable(1, 1.0, 0.0, 2.0, 0.0)]}, False,
     "cable outputModuleId 1.0 must be a JSON integer"),
    ("JSON-real parameter ids are not Rack parameter ids", {
        "version": "2.6.6",
        "modules": [mod(1, "Fundamental", "VCO", params=[
                        {"id": 0.0, "value": 0.5}]),
                    mod(2, "Core", "AudioInterface2", (10, 0))],
        "cables": [cable(1, 1, 0, 2, 0)]}, False,
     "parameter id 0.0 must be a JSON integer"),
...
def check_model_failure() -> int:
    ...
    cases = [
        ...
        ("an actionable quota error outranks a harmless stdout warning",
         "Skill descriptions were shortened to fit the context budget.",
         "ERROR: You've hit your usage limit. Purchase more credits.",
         ("out of quota", "Settings > Agents")),
...
def check_attempt_keeping() -> int:
    ...
    implementation = "def _generate(" if "def _generate(" in src else "def generate("
    body = src[src.index(implementation):]
...
def check_buildable_from_parts() -> tuple:
    ...
    src = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "patch.py")).read()
    implementation = ("def _generate(" if "def _generate(" in src
                      else "def generate(")
    body = src[src.index(implementation):]
...
def check_gate_survives_third_party() -> tuple:
    ...
    elif ("engine->addModule" not in up or "m->id = patch_id" not in up or
          "onSampleRateChange" in up or "onAdd" in up):
        bad += 1
        print("  WRONG  the harness imitates lifecycle callbacks instead of "
              "registering the patch ID with Rack's real Engine; modules "
              "using patch storage still have no valid runtime identity")
    elif ("new rack::patch::Manager" not in src or
          "ctx->patch->autosavePath" not in src):
        bad += 1
        print("  WRONG  the Rack context has no isolated Patch Manager, so a "
              "module using patch storage during onAdd aborts the gate")
    else:
        print("  ok     each patch module is registered with the real Engine "
              "and an isolated Patch Manager")
    ...
    pdir = P._plugin_dir()
    inv = P.inventory()
    victims = [(p, m) for p, m in (("Fundamental", "VCO2"),
                                   ("Bogaudio", None), ("CVfunk", "Alloy"),
...
def check_unjudged_patch_is_kept() -> tuple:
    ...
    P.audibility = lambda pch, **_k: (
        P.UNMEASURED, P.gate_crash_report(11, pch))
    ...
    if got is None:
        ...
        print(f"  WRONG  the patch that came back is not the one that was "
              f"built: {got}")
    elif shortfall is None or shortfall.severity != 2:
        bad += 1
        print("  WRONG  an unmeasured patch graduated as a successful build")
    else:
        print("  ok     an unmeasured patch is retained only as unfinished "
              "evidence and cannot graduate")
    return bad, ran
...
def check_named_is_fetched() -> tuple:
    ...
    P.ensure_audibility_sdk = fake_sdk
    toolpaths.missing_prerequisites = lambda *args, **kwargs: []
    try:
        with tempfile.TemporaryDirectory() as out_dir:
            P.main(["patch.py", "build", "@Acme Audio a drone", "--out",
                    os.path.join(out_dir, "fetch-test.vcv")])
    except Stop:
        pass
    except SystemExit:
        pass
    ...
    if "prompt" not in seen:
        ...
        print("  WRONG  generation reached the model before the Rack DSP SDK "
              "acceptance instrument was ready")
...
def check_sdk_resolution() -> tuple:
    ...
    # 3b. Patch generation uses that same installer as a fail-closed preflight,
    # before a model call can spend quota and before UNMEASURED can look like a
    # finished patch. It also refreshes patch.py's gate SDK after the fetch.
    old_settings, old_sdk = P.settings, P.SDK
    real_compiler = fetch_sdk.compiler_missing
    calls = []
    try:
        P.settings = lambda: {"auto_fetch_sdk": True}
        fetch_sdk.installed_at = lambda: None
        fetch_sdk.fetch = lambda quiet=True: (calls.append(("fetch", quiet)),
                                               "/fresh-sdk")[1]
        fetch_sdk.compiler_missing = lambda: None
        said = []
        got = P.ensure_audibility_sdk(said.append)
        if got != "/fresh-sdk" or P.SDK != "/fresh-sdk":
            bad += 1
            print(f"  WRONG  patch preflight did not adopt the fetched SDK: "
                  f"{got!r}, gate={P.SDK!r}")
        elif calls != [("fetch", False)] or not said:
            bad += 1
            print(f"  WRONG  patch preflight was silent or did not use the "
                  f"verified installer: calls={calls}, said={said}")
        else:
            print("  ok     patch preflight fetches and adopts the verified SDK")
    finally:
        P.settings, P.SDK = old_settings, old_sdk
        fetch_sdk.installed_at, fetch_sdk.fetch = real_installed, real_fetch
        fetch_sdk.compiler_missing = real_compiler
...
def check_default_output_write_probe() -> tuple:
    """A normal app build reaches generation before choosing its output name."""
    import toolpaths
    ...
    class ReachedGenerate(Exception):
        pass

    def fake_generate(*args, **kwargs):
        nonlocal reached_generate
        reached_generate = True
        raise ReachedGenerate()

    saved = (P.inventory, P.catalog, P.module_index,
             P.ensure_named_installed, P.preflight, P.generate,
             P.ensure_audibility_sdk,
             toolpaths.missing_prerequisites)
    old_home = os.environ.get("HOME")
    try:
        with tempfile.TemporaryDirectory() as home:
            ...
            P.generate = fake_generate
            P.ensure_audibility_sdk = lambda announce=print: "/sdk"
            toolpaths.missing_prerequisites = lambda *args, **kwargs: []
            try:
                P.main(["patch.py", "build", "a test patch",
                        "--response-file", os.path.join(home, "saved.txt")])
            except ReachedGenerate:
                pass
    except Exception as exc:
        bad += 1
        print(f"  WRONG  default-output build failed before generation: {exc}")
    finally:
        (P.inventory, P.catalog, P.module_index,
         P.ensure_named_installed, P.preflight, P.generate,
         P.ensure_audibility_sdk,
         toolpaths.missing_prerequisites) = saved
        if old_home is None:
    ...
    if not reached_generate:
        bad += 1
        print("  WRONG  default-output build never reached generation")
    elif not bad:
        print("  ok     default-output build reaches generation")
    return bad, ran
...
def check_shipped_generator() -> tuple:
    ...
    probe = ("import sys; sys.path.insert(0, %r); import generate;"
             ...
             "generate.dsp_vocabulary('clock phase')" % here)
    r = subprocess.run([sys.executable, "-c", probe], cwd=here,
                       capture_output=True, text=True)
    failure = r.stdout + r.stderr
    if r.returncode != 0 and any(message in failure for message in (
            "vocabulary came back empty",
            "curated DSP capability selection was not valid JSON")):
...
def main():
    ...
    for _check in (test_handover.check_retry_names_a_real_jack,
                   ...
                   test_handover.check_silent_wrong_acid_aggregates_every_defect,
                   test_handover.check_unmeasured_acid_is_not_retried):
        _b, _r = _check()
        hand_bad += _b
        hand_ran += _r
    ...
    output_bad, output_ran = check_default_output_write_probe()
    ...
    import test_module_activation
    activation_bad, activation_ran = \
        test_module_activation.check_module_activation_contracts()
    import test_long_horizon
    horizon_bad, horizon_ran = test_long_horizon.check_long_horizon_contract()
    import test_deterministic_repair
    repair_bad, repair_ran = \
        test_deterministic_repair.check_deterministic_repairs()
...
if __name__ == "__main__":
    try:
        ...
        shutil.rmtree(_TEST_ATTEMPT_ROOT, ignore_errors=True)
