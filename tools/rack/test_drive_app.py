...
def check_rack_open_evidence() -> int:
    """The GUI-open proof must bind ordered module creation to the exact file."""
    expected = "/tmp/demo.vcv"
    cases = [
        ("exact load followed by its two modules",
         "Loading patch /tmp/demo.vcv\nCreating module Forge A\n"
         "Creating module Core Audio\n", 2, True),
        ("a path with the expected path as a prefix",
         "Loading patch /tmp/demo.vcv.backup\nCreating module Forge A\n", 1,
         False),
        ("module creation before the requested load",
         "Creating module Forge AUTOSAVE\nLoading patch /tmp/demo.vcv\n", 1,
         False),
        ("too few modules created from the requested patch",
         "Loading patch /tmp/demo.vcv\nCreating module Forge A\n", 2, False),
    ]
    bad = 0
    for name, log, count, want in cases:
        got = bool(D.rack_open_evidence(log, expected, count))
        if got == want:
            print(f"  ok     Rack open proof rejects/accepts {name}")
        else:
            print(f"  WRONG  Rack open proof got {got} for {name}; wanted {want}")
            bad += 1
    return bad
...
def check_success_line_agrees() -> int:
    ...
    import re
    bad = 0
    src = Path(HERE, "patch.py").read_text()
    ...
    gen = Path(HERE, "generate.py").read_text()
    ...
    for script in ("prove_idioms.sh", "prove_surfaces.sh"):
        text = Path(HERE, script).read_text()
