...
- **Every identity/index field is a JSON integer, never a decimal or boolean.**
  That includes module, cable, parameter, module-reference, and port ids. Module
  and cable `id` values must also be unique.
...
8. **A melody must be WRITTEN, not implied by the wiring.** When the request
   ...
   A module may instead list a **REQUIRED persistent musical state** form in
   its inventory entry. That form outranks this generic parameter advice:
   write exactly the named `data` authoring object and do not simulate its
   editor buttons with `params`. Forge compiles that small musical object into
   the maker's exact versioned Rack state before lint and DSP validation.
