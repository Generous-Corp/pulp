...
def check_attempts_are_kept_without_being_asked() -> tuple:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(where[:-4] + ".txt", encoding="utf-8") as source:
                reason = source.read()
...
def check_give_up_still_ends_the_run() -> tuple:
    ...
    """
    bad = 0
    with open(P.__file__, encoding="utf-8") as source:
        src = source.read()
    ...
    gen = src[src.index(implementation):src.index("def main(argv)")]
...
class _Intent:
    ...
    def __init__(self, slug, gating=True):
        self.slug = slug
        self.how = "named"
        self.gating = gating
        self.why = ""
...
def check_the_loop_gives_up_holding_a_patch() -> tuple:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            P.lint = lambda p, i: []
            P.claim_idiom = lambda *a, **k: _Intent(
                "sequenced-voice", gating=False)
            P.diagnose_intent = lambda *a, **k: P.IntentDiagnosis(
                None, [describe], [], [], [])
            # Never satisfied: retries=2 must mean one call plus two retries.
            ...
            try:
                ...
                bad += 1
                print(f"  WRONG  a run with three sound patches still ends "
            ...
            if len(seen) != 3 or (shortfall and shortfall.tried != 3):
                bad += 1
                print(f"  WRONG  retries=2 made {len(seen)} calls and reports "
                      f"tried={getattr(shortfall, 'tried', None)}")
            else:
                print("  ok     retries=2 makes and reports exactly 3 calls")
...
def check_silent_wrong_acid_aggregates_every_defect() -> tuple:
    """Named SILENT failures report all evidence without another model call."""
    import tempfile
    import idiom_check as I
    ...
    built = I.synthesize(idioms["sequenced-voice"], inv, roles)
    prompt = "an acid line with a single sequencer"
    seen = []
    bad = 0
    saved = {k: getattr(P, k) for k in
             ("find_claude", "ask_model", "library_brief", "catalog",
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            P.catalog = lambda *a, **k: {}
            P.configure_audio = lambda *a, **k: None
            P.audibility = lambda *a, **k: (
                P.SILENT, "  FAIL listener silent: 0.000 V\n")
            P.reflow = lambda p, i: p
            P.lint = lambda p, i: []

            def fake_model(claude, model_prompt, seconds, tick=8.0):
                seen.append(model_prompt)
                return 0, ("```json patch\n" + json.dumps(built) +
                           "\n```\n```json why\n{}\n```"), ""
            P.ask_model = fake_model
            # Exact named acid remains single-shot with the default zero-retry
            # budget; retries can only be enabled explicitly.
            _, _, shortfall = P.generate(prompt, inv, None)

            detail = "\n".join(shortfall.detail if shortfall else [])
            topology = "slew" in detail or "accent" in detail
            silent = "silent" in detail.lower()
            behavior = "UNMEASURED" in detail
            if len(seen) != 1 or any("REJECTED" in text for text in seen):
                print(f"  WRONG  named acid spent {len(seen)} model calls or "
                      "fabricated prior-attempt retry context")
                bad += 1
            elif not (topology and silent and behavior):
                print("  WRONG  the retained zero-retry shortfall did not "
                      "aggregate topology, behavior, and audio")
                bad += 1
            else:
                print("  ok     default-budget named acid uses one call and "
                      "retains topology, behavior, and audio findings")
            if shortfall is None:
                print("  WRONG  silent generic voice was accepted as acid")
                bad += 1
    finally:
...
def check_unmeasured_acid_is_not_retried() -> tuple:
    """Missing runtime evidence blocks publish, not another model call."""
    import tempfile
    import idiom_check as I
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            P.catalog = lambda *a, **k: {}
            P.configure_audio = lambda *a, **k: None
            P.audibility = lambda *a, **k: (P.AUDIBLE, "audio reached output")
            P.reflow = lambda p, i: p
            P.lint = lambda p, i: []
            ...
            def fake_model(claude, model_prompt, seconds, tick=8.0):
                ...
                return 0, ("```json patch\n" + json.dumps(built) +
                           "\n```\n```json why\n{}\n```"), ""
            P.ask_model = fake_model
            _, _, shortfall = P.generate(prompt, inv, None, retries=4)
            if len(calls) != 1:
                print(f"  WRONG  behavior-UNMEASURED acid spent "
                      f"{len(calls)} model calls")
                bad += 1
            elif shortfall is not None and shortfall.tried != 1:
                print(f"  WRONG  one-call early stop reports tried="
                      f"{shortfall.tried}")
                bad += 1
            elif shortfall is None or not any(
                    "UNMEASURED" in item for item in shortfall.detail):
                print("  WRONG  unmeasured behavior did not retain an honest "
                      "unfinished result")
                bad += 1
            else:
                print("  ok     topology-correct acid stops after one call and "
                      "retains behavior-UNMEASURED as a shortfall")

            # Even a model-actionable topology defect must not start a blind
            # loop when the audio instrument itself did not run.
            built = I.synthesize(idioms["sequenced-voice"], inv, roles)
            calls.clear()
            P.audibility = lambda *a, **k: (
                P.UNMEASURED, "audio harness unavailable")
            _, _, shortfall = P.generate(prompt, inv, None, retries=4)
            if len(calls) != 1 or shortfall is None or shortfall.tried != 1:
                print(f"  WRONG  audio-UNMEASURED acid with wrong topology "
                      f"spent {len(calls)} model calls")
                bad += 1
            else:
                print("  ok     audio-UNMEASURED also stops after one call, "
                      "without hiding its static defects")
    finally:
        for key, value in saved.items():
            setattr(P, key, value)
        P._ATTEMPTS_DIR = saved_dir
...
CHECKS = (check_retry_names_a_real_jack,
          ...
          check_silent_wrong_acid_aggregates_every_defect,
          check_unmeasured_acid_is_not_retried,
