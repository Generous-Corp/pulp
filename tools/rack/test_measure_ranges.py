...
def test_run_rack_consumes_the_typed_launch_result() -> int:
    """The Launch.died field must survive the retry coordinator boundary."""
    class FakeCrash:
        retryable = False

    class RetryableCrash:
        retryable = True

    real_launch = mr.launch_once
    real_sleep = mr.time.sleep
    bad = 0
    try:
        mr.launch_once = lambda *a, **k: mr.Launch(
            "CARTOG wrote the map", "", [], False)
        got = mr.run_rack("/A/Rack", "/p.vcv", 1.0, attempts=1)
        bad += check(got == ("CARTOG wrote the map", ""),
                     "run_rack accepts a successful Launch value",
                     f"got {got!r}")

        calls = []
        mr.launch_once = lambda *a, **k: (
            calls.append(1) or mr.Launch(
                "died", "Rack died before it scanned", [FakeCrash()], True))
        mr.time.sleep = lambda *a, **k: None
        got = mr.run_rack("/A/Rack", "/p.vcv", 1.0, attempts=3)
        bad += check(len(calls) == 1 and got[1] ==
                     "Rack died before it scanned",
                     "a fatal typed crash preserves died and stops immediately",
                     f"calls={len(calls)}, got={got!r}")

        calls.clear()
        mr.launch_once = lambda *a, **k: (
            calls.append(1) or mr.Launch(
                "died before patch load", "transient startup crash",
                [RetryableCrash()], True))
        got = mr.run_rack("/A/Rack", "/p.vcv", 1.0, attempts=5)
        bad += check(len(calls) == mr.ABORT_ATTEMPTS and got[1] ==
                     f"{mr.ABORT_ATTEMPTS} launches died in CoreMIDI before "
                     "reaching the patch",
                     "a retryable typed death uses the bounded abort budget",
                     f"calls={len(calls)}, got={got!r}")

        calls.clear()
        mr.launch_once = lambda *a, **k: (
            calls.append(1) or mr.Launch(
                "Rack loaded the patch", "scan timed out", [], False))
        got = mr.run_rack("/A/Rack", "/p.vcv", 1.0, attempts=2)
        bad += check(len(calls) == 2 and got[1] ==
                     "2 launches, last: scan timed out",
                     "a non-died timeout consumes exactly the requested count",
                     f"calls={len(calls)}, got={got!r}")
    finally:
        mr.launch_once = real_launch
...
def main() -> int:
    ...
    for fn in (test_scanner_is_last, test_portmap_is_a_list,
               ...
               test_run_rack_consumes_the_typed_launch_result,
