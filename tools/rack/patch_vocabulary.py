...
def _sound_guidance_for_prompt(prompt: str, read, idioms: dict) -> list[dict]:
    """Return named-sound settings that do not redefine the claimed idiom.

    Catalogue names include generic synthesis verbs such as ``slide``.  In an
    explicit acid request that word describes the acid topology; it is not a
    request for the catalogue's unrelated R&B Slide instrument recipe.  Keep
    genuinely independent names (``cello drone`` still gets cello settings),
    but let a gating idiom own single-word terms already used by its semantic
    contract.
    """
    import knowledge                                      # noqa: PLC0415
    heard = knowledge.for_prompt(prompt)
    if not heard or not read.primary.gating or not read.primary.slug:
        return heard
    idiom = idioms[read.primary.slug]
    semantic_parts = list(idiom.get("names") or [])
    semantic_parts += list(idiom.get("implies") or [])
    semantic_parts += [idiom.get("is", ""), idiom.get("sounds_right_when", "")]
    semantic_parts += [req.get("describe", "")
                       for req in idiom.get("topology") or []]
    semantic_words = set(re.findall(r"[a-z]+", " ".join(semantic_parts).lower()))
    prompt_words = set(re.findall(r"[a-z]+", prompt.lower()))

    def conflicts(entry: dict) -> bool:
        for name in entry.get("names") or []:
            words = set(re.findall(r"[a-z]+", name.lower()))
            if len(words) == 1 and words <= prompt_words and words <= semantic_words:
                return True
        return False

    return [entry for entry in heard if not conflicts(entry)]
...
def for_prompt(prompt: str, idioms: dict | None = None) -> str:
    ...
    if guidance:
        try:
            import knowledge                             # noqa: PLC0415
            heard = _sound_guidance_for_prompt(prompt, read, idioms)
            if heard:
                named_sounds = (
    ...
    # A modifier in the user's sentence is a contract, not decoration. Keep
    # the deterministic binding beside the topology so the model sees the
    # exact constraint that the validator will enforce.
    from idiom_check import bound_modifiers                 # noqa: PLC0415
    modifiers = bound_modifiers(prompt, idiom)
    if modifiers:
        lines += ["", "The request also imposes these exact constraints:"]
        for modifier in modifiers:
            lines.append(f"  - {modifier.describe}.")
