...
ARCH="${PULP_RACK_PATCH_DECODE_ARCH:-}"

if [ -n "$ARCH" ]; then
  if [ "$(uname -s)" != "Darwin" ]; then
    echo "Requested decoder architecture is supported only on macOS" >&2
    exit 2
  fi
  case "$ARCH" in
    arm64|x86_64) ;;
    *) echo "Unsupported decoder architecture: $ARCH" >&2; exit 2 ;;
  esac
fi
...
if [ -n "$ARCH" ]; then
  "${CC:-/usr/bin/cc}" -arch "$ARCH" -std=c99 -O2 -DNDEBUG \
    -c "$VENDOR/zstddeclib.c" -o "$BUILD/zstddeclib.o"
  "${CXX:-/usr/bin/c++}" -arch "$ARCH" -std=c++17 -O2 -DNDEBUG \
