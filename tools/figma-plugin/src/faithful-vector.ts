...
export interface InteractiveElement {
  ...
  // Human-readable parameter name from the design's own layer name (e.g. the
  // "DEPTH" caption under a knob), when the designer named the layer something
  // meaningful. Empty → the consumer falls back to the synthetic binding key.
  label?: string;
  // Host-parameter binding key for a geometry-detected control, from the opt-in
  // layer-name sigil (`param:`/`bind:`/`meter:` — see paramKeyFromLayerName).
  // Maps 1:1 to IRInteractiveElement::param_key → DesignFrameElement::param_key,
  // which DesignFrameView routes to the framework-agnostic HostParamSurface.
  // Absent for an unbound knob (a descriptive name needs the annotated manifest).
  param_key?: string;
}
...
// ── Layer-name provenance, labels, and host-param binding for knobs ───────────
// Geometry knobs (parseFrameKnobs) are found purely from SVG dome+needle shape:
// they carry no node link. This resolves each knob against the Figma node tree by
// position — the same spatial convention the REST lane uses (figma_rest_export.py
// _label_elements) — to stamp a human `label`, provenance `source_node_id`, and an
// opt-in host-param `param_key`.
...
// and the REST lane (_param_key_from_layer_name) EXACTLY: leading-whitespace
// tolerant, case-insensitive sigil, trimmed value, at least one alphanumeric
// char. The sigil is load-bearing — a bare or DESCRIPTIVE name ("Cutoff") is
// NEVER a binding, so there are no false positives; a descriptive design binds
// via the annotated manifest instead. Returns the stripped key, or "".
const BINDING_SIGILS = ["param:", "bind:", "meter:"];
export function paramKeyFromLayerName(rawName: string): string {
  let s = rawName || "";
  let i = 0;
  while (i < s.length && (s.charAt(i) === " " || s.charAt(i) === "\t")) i++;
  s = s.substring(i);
  const low = s.toLowerCase();
  for (let si = 0; si < BINDING_SIGILS.length; si++) {
    const sig = BINDING_SIGILS[si];
    if (s.length > sig.length && low.substring(0, sig.length) === sig) {
      const key = s.substring(sig.length).replace(/^[ \t]+/, "").replace(/[ \t]+$/, "");
...
// A Figma layer name auto-generated by the editor ("Ellipse 12", "Frame 41", a
// bare number) carries no meaning; a structural/kind word ("Dropdown", "Knob")
// names WHAT a control is, not the parameter it drives. Both yield "" so an
// unnamed control falls back to its synthetic key. Lockstep with the REST lane's
// _node_label (figma_rest_export.py).
const DEFAULT_NAME_RE =
  /^(ellipse|rectangle|rect|frame|group|vector|line|polygon|star|component|instance|union|subtract|intersect|exclude|slice|boolean|arrow|image|mask)\s*\d*$/i;
const LABEL_NOISE: { [k: string]: boolean } = {
  knob: true, dial: true, dropdown: true, stepper: true, tab: true, tabs: true,
  tab_group: true, tabgroup: true, search: true, button: true, btn: true,
  slider: true, fader: true, combo: true, combobox: true, select: true,
  field: true, input: true, control: true, value: true, param: true,
  parameter: true, widget: true, icon: true,
};
export function nodeLabel(name: string): string {
  const s = (name || "").replace(/^\s+/, "").replace(/\s+$/, "");
  if (!s || DEFAULT_NAME_RE.test(s)) return "";
  if (LABEL_NOISE[s.toLowerCase()]) return "";
  return s;
}

// Resolve each control against the frame's Figma node tree: a control that
// already carries source_node_id looks its node up directly; a geometry knob
// (no link) matches the nearest meaningfully-named or sigil-named node whose
// SVG-space center lands within the knob's hit radius. Node centers are mapped
// into the SAME SVG space parseFrameKnobs reports — `(node_abs - root_abs) +
// panelOrigin` — so a frame with a drop-shadow margin (panel not at 0,0) still
// matches (the convention detectOverlayControls uses). A sigil name yields a
// `param_key` binding (and no label — the sigil is not a human name); a plain
// meaningful name yields a `label`. Mutates `elements` in place. Lockstep with
// the REST lane's _label_elements (figma_rest_export.py). `panelOrigin` is
// parsePanelBounds(svg)'s [x, y].
export function labelAndBindKnobs(elements: InteractiveElement[],
                                  frameNode: OverlayNode,
                                  panelOrigin: [number, number]): void {
  const ox = frameNode.absolute_bounds.x;
  const oy = frameNode.absolute_bounds.y;
  const pox = panelOrigin[0];
  const poy = panelOrigin[1];
  const id2node: { [id: string]: OverlayNode } = {};
  const namedPts: Array<{ cx: number; cy: number; area: number; node: OverlayNode }> = [];
  function walk(n: OverlayNode, isRoot: boolean): void {
    if (n.figma_node_id) id2node[n.figma_node_id] = n;
    const bb = n.absolute_bounds;
    // A node participates in knob→node matching if it has a meaningful label OR
    // declares a binding sigil (so a `param:`-named knob resolves even though the
    // sigil is not a human label). Default-named leaves are excluded here, so a
    // bare inner ellipse never steals ownership from its named knob instance. The
    // root frame is excluded too — it is the container, and its name (the design's
    // own name) would otherwise be mis-bound to a knob that sits near its center.
    if (!isRoot && bb &&
        (nodeLabel(n.name) !== "" || paramKeyFromLayerName(n.name) !== "")) {
      namedPts.push({ cx: bb.x - ox + pox + bb.w / 2, cy: bb.y - oy + poy + bb.h / 2,
                      area: bb.w * bb.h, node: n });
    }
    for (let i = 0; i < n.children.length; i++) walk(n.children[i], false);
  }
  walk(frameNode, true);

  function apply(el: InteractiveElement, node: OverlayNode): void {
    const pk = paramKeyFromLayerName(node.name);
    if (pk) {
      el.param_key = pk;
    } else {
      const lbl = nodeLabel(node.name);
      if (lbl) el.label = lbl;
    }
  }
  ...
  for (let ei = 0; ei < elements.length; ei++) {
    ...
    const sid = el.source_node_id;
    if (sid && id2node[sid]) {
      apply(el, id2node[sid]);
      continue;
    }
    // Geometry knob: the best candidate node whose center is within the hit
    // radius. Priority (Codex-guided): a binding-sigil node beats a plain-named
    // one (an explicit binding wins over a nearby caption); within a rank, nearest
    // center wins, then smallest area (the deepest/tightest owner over an
    // enclosing group at the same center).
    if (el.kind === "knob") {
      const kx = el.cx || 0;
      const ky = el.cy || 0;
      const r = el.hit_radius || 0;
      let best: OverlayNode | null = null;
      let bestRank = -1;
      let bd = 1e18;
      let ba = 1e18;
      for (let pi = 0; pi < namedPts.length; pi++) {
        const p = namedPts[pi];
        const d2 = (p.cx - kx) * (p.cx - kx) + (p.cy - ky) * (p.cy - ky);
        if (d2 > r * r) continue;
        const rank = paramKeyFromLayerName(p.node.name) !== "" ? 1 : 0;
        const better = rank > bestRank ||
          (rank === bestRank &&
            (d2 < bd - 1e-6 || (d2 <= bd + 1e-6 && p.area < ba)));
        if (better) {
          bestRank = rank;
          bd = d2;
          ba = p.area;
      ...
      if (best) {
        el.source_node_id = best.figma_node_id || "";  // provenance for the manifest lane
        apply(el, best);
      }
    }
  }
