...
try {
  ...
  if(request.mode==='adaptive_simulation'){
    ...
    for(let i=0;i<p.downshift_after_over_budget_frames;i++){const sample=(await gpuMeasure(page,extraMultiplier)).gpu,before=scale,last=i===p.downshift_after_over_budget_frames-1;if(last)await setScale(downTarget);observations.push({phase:'over-budget',frame_index:i,sample_ms:sample,scale_before:before,scale_after:scale,transition:last?(scale<before?'downshift':'floor-hold'):false})}
    for(let i=0;i<p.upshift_after_under_budget_frames;i++){const sample=(await gpuMeasure(page,1)).gpu,before=scale,last=i===p.upshift_after_under_budget_frames-1;if(last)await setScale(upTarget);observations.push({phase:'under-budget',frame_index:p.downshift_after_over_budget_frames+i,sample_ms:sample,scale_before:before,scale_after:scale,transition:last?(scale>before?'upshift':'ceiling-hold'):false})}
