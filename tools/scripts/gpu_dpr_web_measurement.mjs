...
try {
  ...
  await page.evaluate(()=>{window.__adapter.onParamsChanged([35,.5,0],window.__params);window.__ui.module._pulp_ui_repaint();window.__gl.finish()});const reference=Array.from(await page.locator('#pulp-ui').screenshot({animations:'disabled'}));await page.evaluate(()=>{window.__ui.module._pulp_ui_repaint();window.__gl.finish()});const finalCapture=Array.from(await page.locator('#pulp-ui').screenshot({animations:'disabled'}));
  ...
  await writeFile(capturePath,Buffer.from(finalCapture));await writeFile(referencePath,Buffer.from(reference));fidelity.comparison={method:'browser-canvas-repeat',reference_sha256:await digestFile(referencePath),capture_sha256:await digestFile(capturePath),same_content_token:`${request.expected_content_digest}:params=35,.5,0`};
