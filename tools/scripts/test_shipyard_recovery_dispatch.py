...
def status(state, description="", created="2026-08-14T12:00:00Z", ident=1):
    return {
        "id": ident,
    ...
    def test_any_non_error_dispatch_status_deduplicates_repeated_ticks(self):
        for state in ("pending", "success", "failure"):
            with self.subTest(state=state):
                plan = dispatch.build_plan(report(), census(HEAD_A, status(state)), 2)
                self.assertEqual(plan["candidate_count"], 0)

    def test_dispatch_transport_error_gets_one_bounded_retry(self):
        first_error = status("error", "dispatch_failed attempt=1")
        plan = dispatch.build_plan(report(), census(HEAD_A, first_error), 3)
        self.assertEqual(plan["candidates"][0]["dispatch_attempt"], 2)
        second_error = status(
            "error",
            "dispatch_failed attempt=2",
            "2026-08-14T12:01:00Z",
            2,
        )
        plan = dispatch.build_plan(report(), census(HEAD_A, first_error, second_error), 4)
        self.assertEqual(plan["candidate_count"], 0)
