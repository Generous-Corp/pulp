...
SCHEMA_MINOR = 1
MANIFEST_REVISION = 51
SURFACE_INVENTORY_VERSION = 75
...
def build_surface(root: pathlib.Path) -> tuple[dict[str, Any], list[str]]:
    ...
    discovered = surface.discover_headers(root)
    inventoried_claims = [item for item in claims if item["include"] in discovered]
    external_problems: list[str] = []
    for item in claims:
        include = item["include"]
        if include in discovered:
            continue
        matches = list((root / "core").glob(f"*/include/{include}"))
        if len(matches) != 1:
            external_problems.append(
                f"non-inventoried public binding header must resolve exactly once: {include}"
            )
            continue
        actual = surface.file_fingerprint(matches[0])
        if actual != item["fingerprint"]:
            external_problems.append(
                f"public header fingerprint mismatch for {include}: "
                f"expected {item['fingerprint']}, measured {actual}"
            )
        if include not in REVIEWED_MINIMAL_TARGETS:
            external_problems.append(
                f"non-inventoried public binding header has no reviewed target owner: {include}"
            )
    document, problems = surface.build_surface_document(
        root,
        binding_claims=inventoried_claims,
    ...
    return document, external_problems + problems
...
def compile_fixture() -> str:
    ...
    for row in sorted(public, key=lambda item: item["key"]):
        ...
        lines.append("    }")
    lines.extend(["    return 0;", "}", "", "// clang-format on", ""])
    return "\n".join(lines)
