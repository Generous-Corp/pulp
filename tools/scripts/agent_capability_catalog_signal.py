...
EXPORTS = [
    ...
    capability(
        key="signal.waveguide-junction", domain="signal",
        summary="Bounded two- to four-port acoustic waveguide scattering junction.",
        rt_class="audio",
        lifecycle={"construction": "control", "prepare": "none",
                   "process": "audio", "reset": "audio", "release": "none"},
        state_model="Fixed-capacity port count and positive characteristic impedances.",
        seed_model="none",
        determinism={"repeatability": "bit_exact", "block_partition": "invariant",
                     "platform_scope": "same_build", "transport_history": "irrelevant"},
        input_domain="two to four finite incoming pressure waves and positive impedances",
        output_domain=("outgoing pressure waves from a lossless memoryless junction, or "
                       "cleared output for invalid or unrepresentable input"),
        units=["samples", "ports", "pressure amplitude", "acoustic impedance"],
        latency="zero", tail="zero",
        scheduling="sample-synchronous after control-side port and impedance configuration",
        bindings=[binding(
            role="entrypoint", kind="cpp_type",
            include="pulp/signal/waveguide_junction.hpp",
            qualified_name="pulp::signal::WaveguideJunctionT<float>",
            target="Pulp::signal",
            header_fingerprint="sha256:110575300a78f2c8ce658d0197dbb4ca8fafdbc00d853515ef5dbcbc9bfa0dc1",
        )],
        _link_probes=[{
            "role": "entrypoint",
            "binding": "pulp::signal::WaveguideJunctionT<float>",
            "operation": "member_call",
            "member": "reset",
            "arguments": "",
        }],
