...
std::string pulp_mcp::server::tools_list_json() {
    ...
    out += R"JSON({"name":"pulp_inspect_value_channels","description":"List the named value channels a running plugin publishes — gain reduction, envelopes, spectra, and bounded occurrences: UI values that are NOT parameters. Returns name, unit, shape and neutral per channel; `shape` says which binder applies (meter -> bindMeter, vector -> bindScope, events -> bindEvents). Empty when the processor declares none.","inputSchema":{"type":"object","properties":{}}},)JSON";
