...
{"name":"pulp_content_install","description":"Install a validated data-only content pack for a plugin. .pulpcontent archives must include files.sha256.json. Requires yes=true after reviewing the target.","inputSchema":{"type":"object","required":["path","plugin","yes"],"properties":{"path":{"type":"string","description":"Path to .pulpcontent archive or content-pack directory"},"plugin":{"type":"string","description":"Target plugin id"},"root":{"type":"string","description":"Optional content data root override"},"yes":{"type":"boolean","description":"Must be true after install target review"}}}},
{"name":"pulp_content_update","description":"Update a data-only content pack from an explicit local path. .pulpcontent archives must include files.sha256.json. Requires yes=true after review; rolls back a replaced local version on failure.","inputSchema":{"type":"object","required":["path","plugin","yes"],"properties":{"path":{"type":"string","description":"Path to .pulpcontent archive or content-pack directory"},"plugin":{"type":"string","description":"Target plugin id"},"root":{"type":"string","description":"Optional content data root override"},"yes":{"type":"boolean","description":"Must be true after update target review"}}}},
{"name":"pulp_content_list","description":"List installed data-only content packs.","inputSchema":{"type":"object","properties":{"plugin":{"type":"string","description":"Optional plugin id filter"},"root":{"type":"string","description":"Optional content data root override"}}}},
{"name":"pulp_content_rescan","description":"Rebuild the installed content index from local content-pack manifests. Metadata-only; no package code is executed.","inputSchema":{"type":"object","properties":{"root":{"type":"string","description":"Optional content data root override"}}}},
{"name":"pulp_content_remove","description":"Remove an installed content pack by package id/plugin/version. Requires yes=true.","inputSchema":{"type":"object","required":["id","plugin","yes"],"properties":{"id":{"type":"string","description":"Installed content package id"},"plugin":{"type":"string","description":"Target plugin id"},"version":{"type":"string","description":"Optional content package version"},"root":{"type":"string","description":"Optional content data root override"},"yes":{"type":"boolean","description":"Must be true after installed content review"}}}},
{"name":"pulp_content_reveal","description":"Return the local install path for an installed content pack.","inputSchema":{"type":"object","required":["id","plugin"],"properties":{"id":{"type":"string","description":"Installed content package id"},"plugin":{"type":"string","description":"Target plugin id"},"version":{"type":"string","description":"Optional content package version"},"root":{"type":"string","description":"Optional content data root override"}}}},
...
{"name":"pulp_audio_scope","description":"Return versioned audio scope JSON from a live standalone target or a speakerless offline WAV input. Live target mode opens the audio device; input_wav mode does not emit sound.","inputSchema":{"type":"object","properties":{"target":{"type":"string","description":"Optional standalone target name to pass to `pulp audio scope` for live capture"},"input_wav":{"type":"string","description":"Optional WAV path for speakerless offline scope analysis"},"png_path":{"type":"string","description":"Optional PNG artifact path for offline WAV scope traces"},"frames":{"type":"integer","description":"Frame delay before reading the live scope window (default 90)"},"window":{"type":"integer","description":"Acquisition window in samples (default 2048)"},"trigger":{"type":"string","description":"Trigger mode: none, raw, off, rising-zero"},"channel":{"type":"integer","description":"Source channel index"}}}},
