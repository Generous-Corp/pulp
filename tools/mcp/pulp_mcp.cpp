...
static std::string tools_list_json() {
    ...
    out += R"JSON({"name":"pulp_validate","description":"Run plugin format validators (CLAP, VST3/pluginval, AU, optional AAX). Use --all for vstvalidator and full AAX validation. Returns JSON report.","inputSchema":{"type":"object","properties":{"all":{"type":"boolean","description":"Run all validators including vstvalidator and full AAX validation"},"json":{"type":"boolean","description":"Return JSON report (default true via MCP)"}}}},)JSON";
    ...
    out += R"JSON({"name":"pulp_motion_list_traces","description":"List the inspector-owned motion trace_ids via Motion.listTraces. Returns {trace_ids:[...]} (empty when nothing is active).","inputSchema":{"type":"object","properties":{}}},)JSON";
    out += R"JSON({"name":"pulp_motion_load_fixture","description":"Load a .motion.jsonl fixture into the MotionScrubber via Motion.loadFixture. Returns {ok, event_count, max_frame, header} on success or an error if the fixture is missing or malformed.","inputSchema":{"type":"object","required":["path"],"properties":{"path":{"type":"string","description":"Absolute path to a .motion.jsonl fixture"}}}},)JSON";
    out += R"JSON({"name":"pulp_motion_scrub_to","description":"Scrub a previously loaded fixture to the given frame via Motion.scrubTo. Returns {playhead_frame, emitted_count}. Errors with 'no fixture loaded' when called before pulp_motion_load_fixture.","inputSchema":{"type":"object","required":["frame"],"properties":{"frame":{"type":"integer","description":"Target playhead frame (>= 0)"}}}},)JSON";
    out += R"JSON({"name":"pulp_motion_play","description":"Play a previously loaded fixture from the current playhead via Motion.play. Returns {playing, emitted_count, playhead_frame}. Errors when no fixture is loaded.","inputSchema":{"type":"object","properties":{}}},)JSON";
