...
constexpr auto kInspectorMcpTools = std::to_array<InspectorMcpToolDescriptor>({
    ...
    {"pulp_inspect_inject_midi", methods::kTestInjectMidi, "Inject one bounded note-on or note-off event through the standalone host MIDI path.", Kind::inject_midi, inspector_field_kind | inspector_field_channel | inspector_field_note | inspector_field_velocity, inspector_field_kind | inspector_field_channel | inspector_field_note},
    {"pulp_inspect_set_transport", methods::kTestSetTransport, "Apply one coherent, idempotent standalone transport update through the host control path.", Kind::set_transport, inspector_field_playing | inspector_field_position_samples | inspector_field_tempo_bpm},
