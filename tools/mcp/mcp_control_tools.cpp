...
std::optional<InspectorProfile> automatic_grant_profile(std::string_view id) {
    if (id == "inspect-readonly")
        return InspectorProfile::Observe;
    const auto profile = profile_from_id(id);
    if (profile == InspectorProfile::Observe || profile == InspectorProfile::Develop)
        return profile;
    return std::nullopt;
}
...
std::string ControlMcpAdapter::call_tool(std::string_view name, std::string_view arguments_json,
    ...
    const auto* operation = detail::control_operation_for_tool(name);
    ...
    if (grant_id.empty()) {
        ...
        const auto canonical_profile = automatic_grant_profile(profile);
        if (!canonical_profile)
            return error_payload("invalid-arguments", "profile must be inspect-readonly, observe, or develop");
        const auto capabilities = profile_capabilities(*canonical_profile);
        const bool operation_specific = risk == InspectorCapabilityRisk::Critical ||
            std::ranges::find(capabilities, operation->capability) == capabilities.end();
