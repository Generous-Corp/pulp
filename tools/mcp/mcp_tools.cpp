...
std::string handle_audio_compare(const std::string& params_json) {
    ...
    if (!align.empty()) {
        ...
        if (mode != "none" && mode != "latency" && mode != "varispeed" && mode != "stretch" && mode != "pitch")
            return arg_error("Error: align must be none, latency, varispeed:<ratio>, stretch:<ratio>, or pitch:<semitones>");
