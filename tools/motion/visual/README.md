...
so a short pre-roll doesn't pollute the analysis window. The supported sources
are a macOS window region and a booted iOS Simulator. It exits 3 (CTest SKIP)
when the selected source is unavailable, so it composes cleanly with CI lanes
that lack the platform tooling. Pair with XcodeBuildMCP on macOS / iOS for log
capture and screenshot orchestration.
