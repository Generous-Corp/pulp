...
class LifecycleNode final : public GpuAudioNode {
  public:
    ...
    void process_block(const BufferView<const float>& input,
                       BufferView<float>& output, std::uint32_t n) override {
        ++gpu_like_process_calls;
        for (std::uint32_t i = 0; i < n; ++i)
            output.channel_ptr(0)[i] = input.channel_ptr(0)[i] * 2.0f;
    }

    void process_cpu_fallback(const BufferView<const float>& input,
...
int main() {
    ...
    for (std::uint32_t block = 1; block <= 4; ++block) {
        std::fill(input.channel(0).begin(), input.channel(0).end(),
    ...
    auto output_view = output.view();
    transport.process(input_view, output_view, LifecycleNode::kBlock);
    if (output.channel(0)[0] != -3.0f) return 16;
    std::fill(input.channel(0).begin(), input.channel(0).end(), 4.0f);
    input_view = static_cast<const Buffer<float>&>(input).view();
    output_view = output.view();
    transport.process(input_view, output_view, LifecycleNode::kBlock);
    if (output.channel(0)[0] != -4.0f) return 17;
    if (transport.stats().miss_blocks != 2 || node.fallback_calls < 2) return 18;
    ...
    transport.pump();
    if (!transport.is_prepared()) return 19;
