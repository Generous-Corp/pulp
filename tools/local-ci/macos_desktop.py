"""macOS desktop helper functions for local CI."""
...
def macos_window_info_for_pid(
    pid: int,
    *,
    macos_window_probe_path_fn: Callable[[], Path],
    run_fn: Callable[..., subprocess.CompletedProcess[str]],
) -> dict:
    result = run_fn(
        ["swift", str(macos_window_probe_path_fn()), "window-info", "--pid", str(pid)],
        capture_output=True,
        text=True,
        check=True,
    )
...
def macos_window_info_for_bundle_id(
    ...
    *,
    ...
    result = run_fn(
        ["swift", str(macos_window_probe_path_fn()), "window-info", "--bundle-id", bundle_id],
        capture_output=True,
        text=True,
        check=True,
    )
...
def macos_accessibility_trusted(
    *,
    ...
    result = run_fn(
        ["swift", str(macos_window_probe_path_fn()), "accessibility-trusted"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def activate_macos_pid(
    ...
    *,
    ...
    result = run_fn(
        ["swift", str(macos_window_probe_path_fn()), "activate", "--pid", str(pid)],
        capture_output=True,
        text=True,
        check=True,
    )
...
def dispatch_macos_click(
    ...
    *,
    ...
    result = run_fn(
        [
            "swift",
            str(macos_window_probe_path_fn()),
            "click",
            "--x",
            str(screen_x),
            "--y",
