...
def build_local_ci_parser(
    *,
    ...
    p_desktop_video_matrix.add_argument(
        ...
        help="Existing run manifest to check and substitute into the design-parity compose command.",
    )
    p_desktop_video_matrix.add_argument(
    ...
    p_simulator_video.add_argument("--open-url", help="Optional URL to open on the simulator during recording")
    p_simulator_video.add_argument("--action-after", type=float, default=0.5, help="Seconds after capture start before running the simulator action (default: 0.5)")
    p_simulator_video.add_argument("--action-label", help="Reviewer-facing label for the simulator action marker")
    ...
    p_simulator_video.add_argument("--compose-video-proof", action="store_true", help="Render the raw simulator proof through Remotion and create issue-ready variants")
    p_simulator_video.add_argument("--video-title", help="Reviewer-facing title for the composed proof")
    p_simulator_video.add_argument("--video-note", action="append", default=[], help="Reviewer-facing proof note; repeat for multiple storyboard notes")
    p_simulator_video.add_argument("--video-attachment-budget-mb", type=float, default=100.0, help="Attachment budget for the issue-ready MP4 variant (default: 100)")
    p_simulator_video.add_argument("--small-video", action="store_true", help="Also render/copy a small fallback MP4 for a 10 MB-style attachment budget")
    p_simulator_video.add_argument("--small-video-budget-mb", type=float, default=10.0, help="Attachment budget for --small-video fallback (default: 10)")
    ...
    p_android_video.add_argument("--compose-video-proof", action="store_true", help="Render the raw Android proof through Remotion and create issue-ready variants")
    p_android_video.add_argument("--video-title", help="Reviewer-facing title for the composed proof")
    p_android_video.add_argument("--video-note", action="append", default=[], help="Reviewer-facing proof note; repeat for multiple storyboard notes")
    p_android_video.add_argument("--video-attachment-budget-mb", type=float, default=100.0, help="Attachment budget for the issue-ready MP4 variant (default: 100)")
    p_android_video.add_argument("--small-video", action="store_true", help="Also render/copy a small fallback MP4 for a 10 MB-style attachment budget")
    p_android_video.add_argument("--small-video-budget-mb", type=float, default=10.0, help="Attachment budget for --small-video fallback (default: 10)")
