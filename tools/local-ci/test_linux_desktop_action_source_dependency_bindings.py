...
MODULE_PATH = Path(__file__).with_name("linux_desktop_action_source_dependency_bindings.py")
...
class LinuxDesktopActionSourceDependencyBindingsTests(unittest.TestCase):
    ...
    def test_source_dependency_installer_keeps_unknown_local_fallback(self) -> None:
        bindings = {}
        self.mod.future_linux_desktop_action_source_helper = lambda _bindings: "future"

        self.mod.install_linux_desktop_action_source_dependency_helpers(
            bindings,
            ("future_linux_desktop_action_source_helper",),
        )

        self.assertEqual(bindings["future_linux_desktop_action_source_helper"](), "future")
