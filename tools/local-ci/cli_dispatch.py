...
def dispatch_desktop_command(
    ...
    *,
    ...
    if handler is None:
        print_fn("Error: desktop subcommand required (install, doctor, video-doctor, video-setup, status, config, recent, proof, publish, verdict, compose-video, serve, video, cleanup, smoke, click, inspect)")
        return 1
    return handler(args)
