...
| `queue_lifecycle.py` | Locked queue enqueue/load/claim/finalize lifecycle helpers, active-target/target-state update persistence, stale-validator queue collection, running-job reconciliation action application, drain loop coordination, and job-wait polling that coordinate queue lock acquisition, running-job reconciliation, queue persistence, runner claim records, completion trimming, post-finalize cleanup, and wait-loop callbacks through injected dependencies. | Enqueue/drain policy, runner-info persistence, result creation, target execution, or user-facing CLI output. |
| `runner_state.py` | Runner-info JSON persistence, runner PID liveness checks, and stale runner-info cleanup under the drain lock. | Queue mutation, target execution, runner active-target payload policy, or user-facing CLI output. |
...
| Queue orchestration | Remaining runner-state wrapper calls and CLI-facing queue updates. | later queue modules |
