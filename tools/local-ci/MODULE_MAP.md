...
| `queue_orchestrator.py` | Pure queue policy helpers: branch validation, default priorities, job fingerprints/records, supersedence and cancellation result payloads, job summaries, retention trimming, and priority sort keys. | Queue file locking/mutation, runner liveness, result persistence, drain orchestration, target execution, or user-facing CLI output. |
...
| Queue orchestration | Locking, queue mutation, stale-running requeue, runner info, target state updates, drain/wait loops, and CLI-facing queue updates. | later `queue_orchestrator.py` |
