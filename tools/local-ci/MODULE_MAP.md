...
| `source_prep_bindings.py` | Dependency binding between the `local_ci.py` facade and desktop source-preparation helpers, preserving facade-level monkeypatch seams while moving exact-SHA source wiring out of the entrypoint. | Source-prep policy, remote transport implementation, desktop artifact layout, or queue orchestration. |
...
| `queue_bindings.py` | Dependency binding between the `local_ci.py` facade and queue lifecycle/runner-state helpers, preserving facade-level monkeypatch seams while moving locked queue wiring out of the entrypoint. | Queue policy, queue file locking semantics, result policy, target execution, or CLI output formatting. |
...
| `linux_desktop_bindings.py` | Dependency binding between the `local_ci.py` facade and Linux desktop action execution helpers, preserving facade-level monkeypatch seams while moving action wiring out of the entrypoint. | Linux probe/command-builder policy, manifest/action policy, source preparation, or CLI command parsing. |
...
| `execution_bindings.py` | Dependency binding between the `local_ci.py` facade and validation execution helpers, preserving facade-level monkeypatch seams while moving runner and job-processing wiring out of the entrypoint. | Validation command construction, result policy, queue orchestration, or platform probe implementations. |
| `execution.py` | Subprocess output capture, progress marker parsing, heartbeat updates, optional command log writing, local/POSIX validation command construction and local/POSIX/Windows runner orchestration, Windows validation script construction, job/target result construction and ordering, result file persistence/rendering, target task construction/execution/result collection, submission config resolution, SSH target execution planning, cross-target validation/status orchestration, and target-neutral validation helper policy. | Windows checkout/probe helper internals, queue mutation, runner-info persistence, or desktop automation adapters. |
