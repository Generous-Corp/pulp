...
| `queue_lifecycle.py` | Locked queue load/claim lifecycle helpers that coordinate queue lock acquisition, running-job reconciliation, queue persistence, and runner claim records through injected callbacks. | Enqueue/finalize/drain policy, runner-info persistence, result persistence, target execution, or user-facing CLI output. |
...
| Queue orchestration | Remaining enqueue/finalize/drain locking, queue mutation around extracted helpers, runner-state wrapper calls, drain/wait loops, and CLI-facing queue updates. | later queue modules |
