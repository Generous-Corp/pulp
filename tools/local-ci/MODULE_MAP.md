...
| `execution_bindings.py` | Dependency binding between the `local_ci.py` facade and validation execution helpers, preserving facade-level monkeypatch seams while moving runner wiring out of the entrypoint. | Validation command construction, result policy, queue orchestration, or platform probe implementations. |
