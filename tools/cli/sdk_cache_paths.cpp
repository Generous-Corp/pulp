...
std::string sdk_tarball_filename(const std::string& version,
    ...
    // Pinning the version into the filename prevents a cached v0.85 tarball
    // from shadowing a v0.92 download because the lookup misses on the
    // filename, not on a version field that has to be checked separately.
    // Format:
...
std::string legacy_unversioned_sdk_tarball_filename(const std::string& platform) {
    // The legacy unversioned filename. Used only for best-effort cleanup of
    // stale tarballs from older CLI installs and for back-compat unit tests;
    // never produced by the current code path.
