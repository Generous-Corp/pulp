// cmd_design_subcommands.cpp — offline `pulp design <verb>` handlers.
//
// The pure-data transforms behind `pulp design {lint,diff,compile,lint-adherence,
// record,gallery,handoff,variants,tweak}` live here, split out of cmd_design.cpp
// so that file is just the verb dispatcher plus the live design-tool launcher.
// Each handler reads/writes files and calls the design cores in core/view; none
// opens a window. Helpers are namespace-local implementation details.

#include "cmd_design_subcommands.hpp"
