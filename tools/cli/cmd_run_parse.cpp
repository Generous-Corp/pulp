...
ParseRunResult parse_run_options(const std::vector<std::string>& args) {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        if (a == "--inspect" || a.rfind("--inspect=", 0) == 0
            || a == "--inspect-capability" || a.rfind("--inspect-capability=", 0) == 0
            || a == "--inspect-runtime-eval") {
            r.error = "pulp run inspector flags are retired; use `pulp control` with an "
                      "exact instance and typed operation";
            return r;
        }
