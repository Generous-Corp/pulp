...
| Framework import lane | `cmd_import.cpp` lines 56-115 and `import_run.cpp` lines 469-563 | `pulp import detect/inspect/emit` argument parsing, vendor-free framework detection, importer SPI orchestration, terms gate, provenance-checked emission, and scaffold write planning. | Keep `cmd_import.cpp` as parse/dispatch only. Put new verb orchestration in `import_run.cpp` or focused `import_*` helpers. |
