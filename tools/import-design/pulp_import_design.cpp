...
int main(int argc, char* argv[]) {
    ...
    std::string jsx_bundle_js;  // populated by --from jsx; used in lieu of generate_pulp_js output
    std::string jsx_component_name;
    try {
        switch (*source) {
            ...
            case DesignSource::jsx: {
                ...
                // Stash the compiled bundle + component name so the
                // output-write step below can emit the bundle directly
                // rather than going through generate_pulp_js. The
                // generated codegen path (createCol/createRow/createLabel)
                // is lossy for JSX: SVG primitives, dynamic React
                // animations, computed inline styles, and per-component
                // layout intent are not represented in the DesignIR + can
                // not be faithfully replayed via the IR walker. Emitting
                // the bundle preserves the live React render — which is
                // what `pulp-screenshot --script <bundle>` already proves
                // works end-to-end (planning/screenshots/jsx-import/
                // jsx-chainer-instrument-render-*.png).
                jsx_bundle_js = std::move(bundle_js);
                jsx_component_name = component_name;
    ...
    // Generate output JS. For --from jsx, the compiled bundle is the
    // output verbatim (the IR is a debugging artifact only — see the
    // long comment in the jsx case above). For every other source the
    // existing generate_pulp_js codegen runs.
    std::string js;
    if (*source == DesignSource::jsx && !jsx_bundle_js.empty()) {
        std::ostringstream banner;
        banner << "// Generated by pulp import-design --from jsx\n"
               << "// Source: " << ir.source_file << "\n"
               << "// Component: " << jsx_component_name << "\n"
               << "// Bundle: " << jsx_bundle_js.size() << " bytes (esbuild IIFE of "
               << "React + ReactDOM + user component + sandbox shim banner)\n"
               << "//\n"
               << "// The bundle below is the canonical native-build output. Loading\n"
               << "// it through `pulp-screenshot --script <this-file>` or a standalone\n"
               << "// host runs the real React tree through Pulp's web-compat DOM shim,\n"
               << "// materializing native widgets via __domAppend. The DesignIR codegen\n"
               << "// (createCol / createLabel calls) is lossy for JSX content and is\n"
               << "// only used internally for reporting + tokens extraction.\n\n";
        js = banner.str() + jsx_bundle_js;
    } else {
        CodeGenOptions opts;
