"You are building the Pulp cross-platform audio plugin and application framework. For this loop, limit work strictly to Linux tasks that are unchecked or incomplete. Do not perform macOS or Windows work unless it is directly required to support the Linux work.
...
- Windows access: use `ssh win2` for runtime testing.
- Linux access: use `ssh ubuntu` for runtime testing.
- Linux-related work must be done in a dedicated Linux worktree and branch. Do not mix with macOS, Windows, or general framework work unless strictly required.
- Only work on Linux features that are explicitly:
  (a) marked as incomplete or unchecked, or
  (b) currently blocked and need resolution.
  Do not proactively build new Linux features, refactor unrelated code, or expand scope.
- The source of truth for what qualifies as outstanding or blocked work is:
  ~/Code/pulp/planning/STATUS.md
  Read it at the start of EVERY iteration and select work only from there.
- The Linux environment is Ubuntu running in a QEMU virtual machine (UTM on Apple Silicon).
  Use it for functional validation, dependency setup, and basic build/runtime checks.
- If GPU issues arise on Linux:
  - Attempt reasonable fixes (drivers, flags, software rendering).
  - Do not over-invest in issues likely caused by virtualization limits.
  - If real hardware is required, document the limitation and move on.
