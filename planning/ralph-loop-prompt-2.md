...
- Phase 4: Audio/MIDI I/O and Standalone Apps (parallel with Phase 3) + audio file I/O (CHOC), sample format converters
- Phase 5: Plugin Formats and Parameter/State
- Phase 6: GPU Rendering, JS Scripting, Design System + animation system (JS tweens), MIDI file I/O (CHOC), clipboard, native file dialogs, popup/context menus, SVG loading (Skia), auto-generated parameter UI, drag-and-drop
- Phase 7: CLI + MCP Server, Claude Code Plugin + programmatic UI testing (synthetic events via MCP: simulate_click/drag/get_value/screenshot)
- Phase 8: Signing, Packaging, Auto-Updates
- Phase 9: Web/WASM, Examples, Documentation, DSP Library + OSC (pulp-osc), DSP scope: FFT/Convolver/SVF/Oversampling/DelayLine/Compressor/Limiter/Oscillator/WaveShaper/Reverb/Chorus/Phaser/Panner
- Phase 10: Hardening, DAW Validation, Public Launch + plugin hosting (pulp-host), full accessibility (Windows UIA, Linux AT-SPI)
...
- Phase 4 gaps:
  - Audio file I/O via CHOC choc::audio::AudioFileFormat (WAV/FLAC/OGG/MP3)
  - Audio data converters (int16/24/32 <-> float sample format conversion)
- Phase 6 gaps:
  - Animation/tween system in the JS scripting layer (frame-rate-independent, easing functions)
  - MIDI file I/O via CHOC choc::midi::File (needed for PulpDrums pattern export)
  - Clipboard (platform copy/paste: text, custom data)
  - Native file/color dialogs (open/save via platform APIs or CHOC)
  - Popup/context menus (platform-native NSMenu/TrackPopupMenu for correct DAW behavior)
  - SVG loading (via Skia built-in SVG support or nanosvg MIT)
  - Auto-generated parameter UI (generic JS widget that reads param definitions and creates controls)
  - Drag-and-drop (OS-level file drops + internal widget drag)
  - Headless event simulation (View::simulateMouseDown/Up/Drag/KeyPress for Catch2 CI tests, no window needed)
  - Complete NSAccessibility on PulpPluginView (enables XCUITest/XcodeBuildMCP UI automation on macOS)
  - Keyboard focus traversal (Tab/Shift-Tab through widgets)
- Phase 7 gaps:
  - Programmatic UI testing via MCP (simulate_click/drag, get_view_value, screenshot endpoints — reuses Phase 6 synthetic event layer)
- Phase 9 gaps:
  - OSC module (pulp-osc: sender/receiver over UDP, from OSC 1.0 spec)
  - DSP library scope — enumerate explicitly: FFT, Convolver, Biquad/IIR, SVF/TPT, Oversampling, DelayLine, SmoothedValue/Ramp, ADSR, Oscillator (polyBLEP + wavetable), Gain, DryWetMixer, Compressor, Limiter, NoiseGate, Chorus, Phaser, Reverb (FDN), Panner, WaveShaper, LadderFilter, LinkwitzRiley, WindowingFunction
- Phase 10 gaps:
  - Plugin hosting (pulp-host: Scanner, PluginSlot, SignalGraph for DAW-like apps)
  - Full cross-platform accessibility (Windows UI Automation, Linux AT-SPI)
- Intentional passes (do NOT implement):
  ...
  - WebView embedding (contradicts GPU-first vision; CHOC choc::ui::WebView available if someone needs it, but not a Pulp feature — license text and docs use native Label/TextView widgets)
