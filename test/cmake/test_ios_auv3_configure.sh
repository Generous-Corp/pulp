...
# Phase iOS-B: the HostApp target generated by pulp_add_ios_host_app()
# must also be present in the Xcode project so `cmake --build` can drive
# the .app build that wraps the .appex for Simulator + physical iPad
# install. Configure-only assertion — the actual HostApp build runs
# behind PULP_IOS_AUV3_SMOKE_BUILD=1 below.
...
if [[ "${PULP_IOS_AUV3_SMOKE_BUILD:-1}" == "1" ]]; then
    ...
    # has a real bundle to install. Smoke-test only — the HostApp's
    # SwiftUI runtime behaviour is validated by the user in the
    # Simulator + GarageBand iOS flow per
