...
TEST_CASE("ExecutionSnapshot parameter events target the pinned graph generation",
          "[host][graph][parameter][snapshot]") {
    using Result = SignalGraph::PreparedTopologyEdit::Result;

    SignalGraph graph;
    auto old_slot = std::make_unique<ParameterMailboxProbe>();
    auto* old_probe = old_slot.get();
    const auto old_plugin =
        graph.add_plugin_node(std::move(old_slot), 1, 1, "old probe");
    REQUIRE(graph.prepare(48000.0, 8));
    ...
    const auto pinned = edit->committed_execution_snapshot();
    REQUIRE(pinned);

    graph.clear();
    auto live_slot = std::make_unique<ParameterMailboxProbe>();
    auto* live_probe = live_slot.get();
    const auto live_plugin =
        graph.add_plugin_node(std::move(live_slot), 1, 1, "live probe");
    REQUIRE(live_plugin == old_plugin);
    REQUIRE(graph.prepare(48000.0, 8));

    ParameterEventQueue events;
    REQUIRE(events.push({ParameterMailboxProbe::kParamId, 3, 0.75f, 0}));
    REQUIRE(pinned.inject_parameter_events(old_plugin, events));

    pulp::audio::BufferView<const float> in;
    pulp::audio::BufferView<float> out;
    pinned.process(out, in, 8);
    REQUIRE(old_probe->received_count() == 1);
    CHECK(old_probe->received(0).sample_offset == 3);
    CHECK(old_probe->received(0).value == 0.75f);

    graph.process(out, in, 8);
    CHECK(live_probe->received_count() == 0);
}

TEST_CASE("ExecutionSnapshot parameter events are consumed exactly once",
          "[host][graph][parameter][snapshot]") {
    using Result = SignalGraph::PreparedTopologyEdit::Result;
    ...
    ParameterEventQueue events;
    REQUIRE(events.push({ParameterMailboxProbe::kParamId, 5, 0.625f, 0}));
    REQUIRE(pinned.inject_parameter_events(plugin, events));
