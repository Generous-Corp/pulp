...
std::filesystem::path self() {
    ...
    if (size == 0 || _NSGetExecutablePath(bytes.data(), &size) != 0) return {};
...
std::string mcp_call(int id, std::string_view name, std::string_view instance,
    ...
    request.setMember("jsonrpc", "2.0"); request.setMember("id", id);
    request.setMember("method", "tools/call");
    auto params = choc::value::createObject(""); params.setMember("name", std::string(name));
    ...
    params.setMember("arguments", arguments); request.setMember("params", params);
...
std::vector<choc::value::Value> run_mcp(const std::filesystem::path& binary,
    ...
    pulp::platform::ProcessOptions options;
    options.capture_stdout = true; options.capture_stderr = true; options.timeout_ms = 30'000;
    pulp::platform::ChildProcess process;
    const std::vector<std::uint8_t> bytes(transcript.begin(), transcript.end());
    REQUIRE(process.start_with_standard_input("/usr/bin/env",
        {"TMPDIR=" + runtime.string(), binary.string()}, bytes, options));
    const auto result = process.wait();
    INFO(result.stdout_output); INFO(result.stderr_output);
    REQUIRE(result.exit_code == 0);
    std::vector<choc::value::Value> responses;
    std::size_t start = 0;
    while (start < result.stdout_output.size()) {
        const auto end = result.stdout_output.find('\n', start);
        const auto line = result.stdout_output.substr(start,
            end == std::string::npos ? std::string::npos : end - start);
        if (!line.empty()) {
            auto response = choc::json::parse(line);
            if (response.hasObjectMember("id")) responses.push_back(std::move(response));
        }
...
void require_manifest(choc::value::ValueView detail,
                      std::string_view expected_plugin_id,
...
TEST_CASE("sample-region control reaches editable and frozen products through CLI",
    ...
    ControlBrokerDaemon daemon({.runtime_root = root.runtime,
                                .state_root = root.state,
                                .sdk_version = "c4-sample-region",
                                .executable_path = executable,
                                .process_generation = 4001,
                                .installed_host_selections = {{.host_id = "sample-region-editable",
                                                               .intent = allow()},
    ...
    const auto prepared = management.manage("host-prepare-installed",
                                             choc::json::toString(selection, false), 10s);
    REQUIRE(prepared.status_id == "prepared");
    auto launch = choc::value::createObject("");
    launch.addMember("inventory_id", choc::value::createString(
        choc::json::parse(prepared.data_json)["inventory_id"].getString()));
    REQUIRE(management.manage("host-launch", choc::json::toString(launch, false), 10s).status_id == "launched");
    ...
    const auto read = run(cli, root.runtime, {"control", "call", "--instance", instance,
    ...
    const auto region_read = run(cli, root.runtime, {"control", "call", "--instance", instance,
    ...
    const auto cli_call = [&](std::string_view operation, std::string params,
        ...
        std::vector<std::string> args{"control", "call", "--instance", instance,
            std::string(operation), "--params", std::move(params), "--json"};
        if (!grant.empty()) { args.emplace_back("--grant"); args.push_back(grant); }
        return run(cli, root.runtime, std::move(args));
    };
    const auto before_state = cli_call("dev.pulp.state/read@1", "{}");
    INFO(before_state.stdout_output); INFO(before_state.stderr_output);
    ...
    CHECK(definition["promoted_parameters"][0]["minimum"].getFloat64() == coefficient["min"].getFloat64());
    CHECK(definition["promoted_parameters"][0]["maximum"].getFloat64() == coefficient["max"].getFloat64());

    const auto develop = run(cli, root.runtime, {"control", "grant-request", "--instance",
        instance, "--profile", "develop", "--json"});
    INFO(develop.stdout_output); INFO(develop.stderr_output);
    REQUIRE(develop.exit_code == 0);
    const std::string develop_grant(choc::json::parse(develop.stdout_output)["data"]["grant_id"].getString());
    const auto lease = cli_call("dev.pulp.session/control@1", R"({"action":"acquire"})", develop_grant);
    INFO(lease.stdout_output); INFO(lease.stderr_output);
    REQUIRE(lease.exit_code == 0);
    const auto gesture = cli_call("dev.pulp.state/parameter-gesture@1",
        R"({"parameter_id":2901,"normalized_value":0.625,"idempotency_key":"coefficient-cli"})", develop_grant);
    INFO(gesture.stdout_output); INFO(gesture.stderr_output);
    ...
    CHECK(choc::json::parse(after_region.stdout_output)["detail"]["graph_generation"].getInt64() == graph_generation);

    const auto observe = run(cli, root.runtime, {"control", "grant-request", "--instance",
        instance, "--profile", "observe", "--json"});
    INFO(observe.stdout_output); INFO(observe.stderr_output);
    REQUIRE(observe.exit_code == 0);
    const std::string observe_grant(choc::json::parse(observe.stdout_output)["data"]["grant_id"].getString());
    const auto forbidden_gesture = cli_call("dev.pulp.state/parameter-gesture@1",
        R"({"parameter_id":2901,"normalized_value":0.75,"idempotency_key":"observe-denied"})", observe_grant);
    ...
    const auto edit_input = "{\"region_id\":901,\"expected_graph_generation\":" +
        std::to_string(graph_generation) + ",\"actions\":[" +
        ...
        ",\"source_port\":" + std::to_string(source_port) + ",\"destination_node_id\":" + std::to_string(output_node) +
        R"(,"destination_port":0},)" +
    ...
    INFO(edited.stdout_output); INFO(edited.stderr_output);
    ...
    CHECK(unchanged_state_document["detail"]["state_generation"].getInt64() == state_generation + 1);
    CHECK(unchanged_state_document["detail"]["catalog_generation"].getInt64() == catalog_generation);
    const auto release = cli_call("dev.pulp.session/control@1", R"({"action":"release"})", develop_grant);
    REQUIRE(release.exit_code == 0);

    const auto editable_status = run(cli, root.runtime,
        {"control", "status", "--instance", instance, "--json"});
    REQUIRE(editable_status.exit_code == 0);
    require_manifest(choc::json::parse(editable_status.stdout_output)["instance"],
                     editable_plugin_id,
                     {"dev.pulp.instance/read@1", "dev.pulp.session/control@1",
                      "dev.pulp.state/read@1", "dev.pulp.state/parameter-gesture@1",
                      "dev.pulp.graph/sample-region.read@1",
    ...
    frozen_launch.addMember("inventory_id", choc::value::createString(
    ...
    const auto list = list_document["instances"];
        ...
        if (frozen_instance.empty()) std::this_thread::sleep_for(10ms);
    }
    ...
    require_manifest(frozen_read_receipt["detail"], frozen_plugin_id,
                     {"dev.pulp.instance/read@1", "dev.pulp.session/control@1",
                      "dev.pulp.state/read@1", "dev.pulp.state/parameter-gesture@1",
                      "dev.pulp.graph/sample-region.read@1"});
    const auto frozen_region_read = run(cli, root.runtime,
        {"control", "call", "--instance", frozen_instance,
         "dev.pulp.graph/sample-region.read@1", "--params", "{}", "--json"});
    REQUIRE(frozen_region_read.exit_code == 0);
    const auto frozen_status = run(cli, root.runtime,
        {"control", "status", "--instance", frozen_instance, "--json"});
    REQUIRE(frozen_status.exit_code == 0);
    require_manifest(choc::json::parse(frozen_status.stdout_output)["instance"],
                     frozen_plugin_id,
                     {"dev.pulp.instance/read@1", "dev.pulp.session/control@1",
                      "dev.pulp.state/read@1", "dev.pulp.state/parameter-gesture@1",
                      "dev.pulp.graph/sample-region.read@1"});
    const auto frozen_edit = run(cli, root.runtime,
        {"control", "call", "--instance", frozen_instance,
         "dev.pulp.graph/sample-region.edit@1", "--params",
    ...
    for (const auto& id : {instance, frozen_instance}) {
        ...
        const auto responses = run_mcp(mcp, root.runtime,
            ...
            mcp_call(6, "pulp_control_state_parameter_gesture", id,
        ...
        for (std::size_t index = 1; index < responses.size(); ++index) {
            ...
            REQUIRE(responses[index]["result"]["structuredContent"]["state"].getString() == "completed");
        }
        ...
        CHECK(state_after["state_generation"].getInt64() == state_before["state_generation"].getInt64() + 1);
        CHECK(state_after["catalog_generation"].getInt64() == state_before["catalog_generation"].getInt64());
        ...
        CHECK(full["regions"][0]["definition"]["promoted_parameters"][0]["smoothing_ramp_seconds"].getInt64() == 0);
        CHECK(summary_after["graph_generation"].getInt64() == summary_before["graph_generation"].getInt64());
        if (!editable_product) {
            const auto edit_responses = run_mcp(mcp, root.runtime,
