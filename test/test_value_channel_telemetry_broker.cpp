...
TEST_CASE("real inspector server backpressure feeds broker debt without crossing clients",
          "[inspect][telemetry][broker][server][backpressure]") {
    ValueChannelSet channels;
    auto* events = channels.declare_events("events");
    REQUIRE(events != nullptr);
    auto now = std::chrono::steady_clock::now();
    ...
    std::string slow_identity;
    std::string healthy_identity;
    InspectorSession session({"session-telemetry-backpressure", "instance", "plugin", "1"}, policy,
                             [&](const pulp::inspect::InspectorRequestContext& context,
                                 const InspectorMessage& message) {
                                 if (message.method ==
                                     pulp::inspect::methods::kTelemetrySubscribe) {
                                     std::lock_guard lock(identity_mutex);
                                     if (slow_identity.empty())
                                         slow_identity = context.client_id;
                                     else if (context.client_id != slow_identity)
                                         healthy_identity = context.client_id;
                                 }
                                 return broker.handle(context, message);
                             });
    InspectorServer server;
    broker.set_event_sink([&](std::string_view client_id, const InspectorMessage& event,
                              std::string_view loss_owner) {
        return server.send_to_client(client_id, event, loss_owner);
    });
    session.set_client_disconnect_handler(
        [&](std::string_view client_id) { broker.disconnect(client_id); });
    ...
    InspectorServerConfig server_config{
        ...
        .record = record,
        .token = *token,
        .max_message_bytes = pulp::inspect::kInspectorExtendedMessageBytes,
    };
    ...
    Socket slow;
    InspectorClient healthy;
    REQUIRE(slow.create(SocketType::TCP));
    REQUIRE(slow.set_read_timeout(5s));
    REQUIRE(slow.connect("127.0.0.1", static_cast<std::uint16_t>(server.port())));
    REQUIRE(authenticate_raw(slow, *token));
    const auto slow_subscription = pulp::inspect::make_request(
        2, pulp::inspect::methods::kTelemetrySubscribe, R"({"channels":["events"],"rateHz":60})");
    REQUIRE(send_frame(slow, encode_message(slow_subscription)));
    const auto slow_subscription_frame = receive_frame(slow);
    REQUIRE(slow_subscription_frame.has_value());
    InspectorMessage slow_subscription_response;
    REQUIRE(decode_message(*slow_subscription_frame, slow_subscription_response));
    REQUIRE_FALSE(slow_subscription_response.is_error);
    REQUIRE(healthy.connect(records.front(), reader));
    std::mutex event_mutex;
    std::condition_variable event_cv;
    std::vector<std::int64_t> healthy_debts;
    healthy.set_event_handler([&](const InspectorMessage& event) {
        if (event.method != pulp::inspect::methods::kTelemetrySample)
            return;
        const auto debt =
            json(event)["transportDroppedSincePrevious"].getWithDefault<std::int64_t>(0);
        {
            std::lock_guard lock(event_mutex);
            healthy_debts.push_back(debt);
        }
        event_cv.notify_all();
    });

    REQUIRE_FALSE(healthy
                      .request(pulp::inspect::methods::kTelemetrySubscribe,
                               R"({"channels":["events"],"rateHz":60})")
                      .is_error);
    {
        std::lock_guard lock(identity_mutex);
        REQUIRE_FALSE(slow_identity.empty());
        REQUIRE_FALSE(healthy_identity.empty());
        REQUIRE(slow_identity != healthy_identity);
    }

    broker.poll();
    {
        std::unique_lock lock(event_mutex);
        REQUIRE(event_cv.wait_for(lock, 2s, [&] { return !healthy_debts.empty(); }));
    }

    std::array<pulp::view::ValueEvent, static_cast<std::size_t>(pulp::view::EventFrame::kMaxEvents)>
        large_batch{};
    ...
    bool real_slow_eviction = false;
    broker.set_event_sink([&](std::string_view client_id, const InspectorMessage& event,
                              std::string_view loss_owner) {
        const auto result = server.send_to_client(client_id, event, loss_owner);
        std::lock_guard lock(identity_mutex);
        if (client_id == slow_identity &&
            (result == InspectorTargetedEventResult::QueuedAfterLossyEviction ||
             result == InspectorTargetedEventResult::DroppedLossy)) {
            real_slow_eviction = true;
        }
        return result;
    });

    std::size_t expected_healthy = 1;
    constexpr std::size_t kMaxBackpressureAttempts = 4096;
    for (std::size_t attempt = 0; attempt < kMaxBackpressureAttempts && !real_slow_eviction;
         ++attempt) {
        events->publish(large_batch.data(), static_cast<int>(large_batch.size()));
        now += 20ms;
        broker.poll();
        ++expected_healthy;
        std::unique_lock lock(event_mutex);
        REQUIRE(
            event_cv.wait_for(lock, 2s, [&] { return healthy_debts.size() >= expected_healthy; }));
    }
    REQUIRE(real_slow_eviction);

    // The real server result above becomes debt in the broker's next attempt.
    events->publish(large_batch.data(), static_cast<int>(large_batch.size()));
    now += 20ms;
    broker.poll();
    bool slow_received_debt = false;
    for (std::size_t attempt = 0; attempt < kMaxBackpressureAttempts + 64 && !slow_received_debt;
         ++attempt) {
        const auto frame = receive_frame(slow);
        REQUIRE(frame.has_value());
        InspectorMessage event;
        REQUIRE(decode_message(*frame, event));
        if (event.method == pulp::inspect::methods::kTelemetrySample) {
            slow_received_debt =
                json(event)["transportDroppedSincePrevious"].getWithDefault<std::int64_t>(0) == 1;
        }
    }
    CHECK(slow_received_debt);
    {
        std::lock_guard lock(event_mutex);
        REQUIRE_FALSE(healthy_debts.empty());
        CHECK(std::all_of(healthy_debts.begin(), healthy_debts.end(),
                          [](std::int64_t debt) { return debt == 0; }));
    }
    slow.close();
    healthy.disconnect();
    server.stop();
