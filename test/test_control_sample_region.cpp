...
bool accepts(std::string_view operation, std::string_view json) {
    ...
    const auto accepted =
        validate_control_json_schema(json, descriptor->input_schema_json, &diagnostics);
...
TEST_CASE("region edits require a bounded ordered action union",
          "[inspect][sample-region][schema]") {
    const auto request = [](std::string_view action) {
        return std::string("{\"region_id\":1,\"expected_graph_generation\":1,\"actions\":[") +
               std::string(action) + "]}";
    };
    CHECK(accepts(edit_operation,
                  request(R"({"op":"set_finite_constant","node_id":2,"value":0.5})")));
    CHECK(accepts(
        edit_operation,
        request(
            R"({"op":"connect","source_temporary_node_id":"t1","source_port":0,"destination_node_id":3,"destination_port":0})")));
    for (
        const auto* action :
    ...
    CHECK_FALSE(
        accepts(edit_operation, R"({"region_id":1,"expected_graph_generation":1,"actions":[]})"));
...
struct RegionFixture {
    ...
    RegionFixture() {
        ...
        region.members = {{x, "pulp.core.sample-region.input", 1, boundary},
                          {p,
                           ...
                           1,
                           {SampleKernelConfigKind::PromotedParameterId, 29}},
        ...
        region.output_boundaries = {output};
        SampleRegionPromotedParameter parameter;
        parameter.param_id = 29;
        parameter.key = "gain";
        parameter.name = "Gain";
        ...
        for (const auto& info : contract.parameters())
            store.add_parameter(info);
        binding = contract.bind(store);
        REQUIRE(binding);
    ...
    ControlExecutionOutcome change(std::string value,
                                   ControlExecutionContext context = {.checkpoint = [] {
                                       return ControlExecutionCheckpoint::Continue;
                                   }}) {
        ControlAdmissionPlan plan;
        plan.registration_id = ControlRegistrationId{"r1"};
        plan.receipt_id = ControlReceiptId{"receipt-1"};
        ControlRequestEnvelope request{
            .registration_id = "r1",
            .operation_id = edit_operation,
            .operation_version = 1,
            .params_json = "{\"region_id\":77,\"expected_graph_generation\":1,\"actions\":[{\"op\":"
                           "\"set_finite_constant\",\"node_id\":" +
                           std::to_string(constant) + ",\"value\":" + value + "}]}"};
        return make_control_sample_region_edit_executor(
            [t = target](const ControlAdmissionPlan&) { return t; })(plan, request, context);
    }
    ControlExecutionOutcome actions(std::string action_json, std::uint64_t expected = 1) {
        ...
        return make_control_sample_region_edit_executor([t = target](const ControlAdmissionPlan&) {
            return t;
...
TEST_CASE("region publication isolates graph and parameter generations",
          "[inspect][sample-region][transaction]") {
    RegionFixture f;
    const auto old_state = f.store.state_generation();
    const auto old_catalog = f.store.parameter_display_revision();
    int calls = 0;
    const auto outcome = f.change(
        "0.25", {.checkpoint = [&] {
            if (++calls == 2) {
                const auto applied =
                    f.store.apply_normalized_gesture_if_generation(old_state, 29, 0.75f);
...
TEST_CASE("region cancellation after prepare leaves topology and state untouched",
          "[inspect][sample-region][transaction]") {
    RegionFixture f;
...
TEST_CASE("full region read returns its frozen promoted contract",
          "[inspect][sample-region][read]") {
    RegionFixture f;
    ControlAdmissionPlan plan;
    plan.registration_id = ControlRegistrationId{"r1"};
    ControlRequestEnvelope request{.registration_id = "r1",
                                   .operation_id = read_operation,
                                   .operation_version = 1,
                                   .params_json = R"({"region_id":77,"include_definition":true})"};
    auto executor = make_control_sample_region_read_executor(
        [t = f.target](const ControlAdmissionPlan&) { return t; });
    auto outcome = executor(plan, request,
...
TEST_CASE("region edit preserves ordered temporary mapping",
          "[inspect][sample-region][transaction]") {
    RegionFixture f;
    const auto outcome = f.actions(
        ...
        "{\"op\":\"connect\",\"source_temporary_node_id\":\"t1\",\"source_port\":0,\"destination_"
        "node_id\":" +
...
TEST_CASE("region edit rejects promotion mutation and instantaneous cycles",
          "[inspect][sample-region][transaction]") {
    RegionFixture f;
    const auto promotion =
        f.actions("{\"op\":\"set_finite_constant\",\"node_id\":" + std::to_string(f.parameter) +
                  ",\"value\":0.25}");
    ...
    const auto cycle =
        f.actions("{\"op\":\"disconnect\",\"source_node_id\":" + std::to_string(f.input_kernel) +
...
TEST_CASE("region edit rejects stale graph generation", "[inspect][sample-region][transaction]") {
    RegionFixture f;
    const auto stale =
        f.actions("{\"op\":\"set_finite_constant\",\"node_id\":" + std::to_string(f.constant) +
                      ",\"value\":0.25}",
                  2);
