...
// Declaration snapshot from the retained Pulp SDK 0.837.0. The compatibility
// target links the receipted v2 libpulp-host implementation; keeping this
// declaration in the fixture prevents current v3 headers from changing the
// old-reader probe at compile time.
...
class GraphSerializer {
  public:
    using MigrationFn =
        std::function<bool(const std::string& source_json, std::string& destination_json)>;

    static int current_format_version();
    static bool register_migration(int from_version, int to_version, MigrationFn migration);

    // Encode the entire graph as JSON. The `editor_layout` map (NodeId →
    // (x,y)) is optional — pass an empty map if the graph has no UI.
    static std::string
    to_json(const SignalGraph& graph,
