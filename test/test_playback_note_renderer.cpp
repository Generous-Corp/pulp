...
TEST_CASE("many note merge sort advances one charged work unit per slice") {
    ...
    PlaybackProgramCompiler compiler(store, executor, std::chrono::microseconds(0));
    ProgramCompileRequest request;
    request.project = note_project(*map, std::move(notes), 2'100);
    request.sequence_id = {2};
...
TEST_CASE("compiler rejects note ranges outside their clip and sub-sample notes") {
    ...
    ProgramCompileRequest sub_request;
    sub_request.project = sub_sample_project;
    sub_request.sequence_id = {2};
    sub_request.tempo_map = map;
    sub_request.sample_rate = map->sample_rate();
    sub_request.document_revision = 1;
    sub_request.dirty.all = true;
