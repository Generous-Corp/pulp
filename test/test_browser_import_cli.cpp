...
TEST_CASE("materialized validation composes captured canvas evidence without shipping it",
          "[import-design][browser-capture][validation][materialized]") {
    TempTree tree;
    pulp::view::DesignIR ir;
    ir.root.type = "frame";
    ir.root.style.width = 32.0f;
    ...
    pulp::view::IRNode canvas;
    canvas.type = "canvas";
    canvas.attributes["materialized_role"] =
        "executable-canvas-target";
    canvas.attributes["asset_ref"] = "canvas:proof";
    ir.root.children.push_back(std::move(canvas));

    auto root =
        pulp::view::build_native_view_tree(ir, ir.asset_manifest);
    REQUIRE(root);
    const auto native_base = pulp::view::render_to_png(
        *root, 32, 32, 2.0f, pulp::view::ScreenshotBackend::skia);
    REQUIRE_FALSE(native_base.empty());
    auto expected = id::decode_png_rgba(
        native_base.data(), native_base.size());
    REQUIRE(expected.valid());

    const auto plate_path = tree.root / "chrome-proof.png";
    tree.write(plate_path, native_base);
    pulp::view::IRNode plate;
    plate.type = "image";
    plate.attributes["materialized_role"] =
        "captured-paint-authority";
    plate.attributes["asset_ref"] = "reference:chrome-proof";
    ir.root.children.insert(ir.root.children.begin(), std::move(plate));
    ir.asset_manifest.assets.push_back({
        .asset_id = "reference:chrome-proof",
        .original_uri = "pulp-capture:///chrome-proof.png",
        .local_path = plate_path.string(),
        .mime = "image/png",
        .width = expected.width,
        .height = expected.height,
    });

    id::ImportPngImage evidence;
    ...
    for (int y = 8; y < 16; ++y) {
        for (int x = 8; x < 16; ++x) {
            ...
            expected.rgba[pixel + 2] = evidence.rgba[pixel + 2];
            expected.rgba[pixel + 3] = 255;
        }
    }

    const auto evidence_path = tree.root / "canvas-proof.png";
    const auto reference_path = tree.root / "reference.png";
    tree.write(evidence_path, id::encode_png_rgba(evidence));
    tree.write(reference_path, id::encode_png_rgba(expected));
    ir.asset_manifest.assets.push_back({
        .asset_id = "canvas:proof",
        .original_uri = "pulp-capture:///canvas-proof.png",
        .local_path = evidence_path.string(),
        .mime = "image/png",
    ...
    SECTION("removing the evidence fails closed") {
        ...
        const auto missing = id::validate_browser_capture_design_ir(
            ir,
            {.reference = reference_path,
             .rendered = tree.root / "render-missing.png",
             .diff = tree.root / "diff-missing.png",
             .width = 32,
             .height = 32});
        CHECK_FALSE(missing.valid);
        CHECK(missing.error.find(
                  "could not read materialized canvas composite evidence") !=
              std::string::npos);
    }
}
