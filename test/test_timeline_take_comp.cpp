...
Project project_with_take(bool record_armed = false) {
    auto lane = ok(TakeLane::create({4}, "comp A", {make_take({5}, {6})}));
    auto track = ok(Track::create(TrackInput{
        .id = {3}, .name = "track", .take_lanes = {lane}, .record_armed = record_armed}));
    auto sequence = ok(Sequence::create({2}, "sequence", TickDuration{100}, {track}));
    return ok(Project::create(
        ProjectInput{{1}, "project", 7, {2}, {audio_asset({6})}, {sequence}}));
}
...
TEST_CASE("Track owns take lanes and record-arm intent") {
    const auto project = project_with_take(true);
...
TEST_CASE("A take referencing a missing or out-of-range asset is rejected") {
    // Missing: the take references {6} but the project only owns asset {99}.
    {
        ...
        REQUIRE_FALSE(Project::create(
            ProjectInput{{1}, "p", 7, {2}, {audio_asset({99})}, {sequence}}));
    }
...
TEST_CASE("A take identity cannot collide with any other track-owned id") {
    ...
    auto colliding =
        Track::create(TrackInput{.id = {3},
                                 .name = "t",
                                 .device_chain = {DevicePlacement{{8}}},
                                 .take_lanes = {lane}});
