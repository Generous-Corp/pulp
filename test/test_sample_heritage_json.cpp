...
TEST_CASE("Sample heritage v3 record filters require complete design parameters",
          "[audio][sampler][heritage][json][reject]") {
    constexpr std::string_view elliptic_parameters =
        "\"cutoff_value\":9000,\"order\":4,\"ripple_db\":0.5,\"stopband_attenuation_db\":60";

    auto butterworth = replace_once(std::string(canonical_json),
                                    "\"filter_family\":\"elliptic\"",
                                    "\"filter_family\":\"butterworth\"");
    butterworth = replace_once(
        std::move(butterworth), elliptic_parameters,
        "\"cutoff_value\":9000,\"order\":4,\"ripple_db\":0,\"stopband_attenuation_db\":0");
    REQUIRE(parse_sample_heritage_profile_json(butterworth).valid());

    auto chebyshev = replace_once(std::string(canonical_json),
                                  "\"filter_family\":\"elliptic\"",
                                  "\"filter_family\":\"chebyshev\"");
    chebyshev = replace_once(
        std::move(chebyshev), elliptic_parameters,
        "\"cutoff_value\":9000,\"order\":4,\"ripple_db\":0.5,\"stopband_attenuation_db\":0");
    REQUIRE(parse_sample_heritage_profile_json(chebyshev).valid());

    auto one_pole = replace_once(std::string(canonical_json),
                                 "\"filter_family\":\"elliptic\"",
                                 "\"filter_family\":\"one_pole\"");
    one_pole = replace_once(
        std::move(one_pole), elliptic_parameters,
    ...
    REQUIRE(validation.status ==
            SampleHeritageProfileStatus::InvalidStageParameter);
    REQUIRE(validation.block_domain == SampleHeritageBlockDomain::RecordCommit);
    REQUIRE(validation.stage_index == 1);
}
