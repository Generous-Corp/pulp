...
// ─── Yoga node-tree reuse across layout passes ──────────────────────────────
//
// yoga_layout() keeps the YGNode tree it built alive between passes and
// revalidates it against the live View tree, so Yoga's own dirty flags and
// measure caches survive. These tests pin the observable: a structurally
// unchanged tree is built once no matter how many passes run over it, and any
// change to the child list forces a fresh build.
//
// Declared here rather than in a public header — the counters exist for this
// invariant, not as API.
namespace pulp::view {
...
std::unique_ptr<pulp::view::View> make_reuse_tree(int leaves) {
    using namespace pulp::view;
    ...
    for (int i = 0; i < leaves; ++i) {
        ...
        leaf->flex().preferred_height = 12;
        root->add_child(std::move(leaf));
    }
    return root;
}
...
TEST_CASE("repeated layout passes over an unchanged tree build one Yoga tree",
          "[view][yoga][layout-perf]") {
    using namespace pulp::view;
    auto root = make_reuse_tree(24);

    yoga_layout_reset_stats();
    for (int i = 0; i < 8; ++i) root->layout_children();

    REQUIRE(yoga_layout_pass_count() == 8);
    REQUIRE(yoga_layout_tree_build_count() == 1);

    // Reuse must not cost correctness: the solved geometry is the same one a
    // from-scratch pass produces.
    const auto first = root->child_at(0)->bounds();
    REQUIRE(first.width > 0.0f);
    REQUIRE(first.height > 0.0f);
    REQUIRE_THAT(root->child_at(23)->bounds().y,
                 WithinAbs(first.y + 23 * first.height, 0.01f));
}

TEST_CASE("a style change reuses the Yoga tree and still resolves the new value",
          "[view][yoga][layout-perf]") {
    using namespace pulp::view;
    auto root = make_reuse_tree(8);

    yoga_layout_reset_stats();
    root->layout_children();
    REQUIRE(yoga_layout_tree_build_count() == 1);

    root->child_at(3)->flex().preferred_height = 40;
    root->layout_children();

    // A style write is not a structural change, so the tree is reused — and
    // the new value still reaches Yoga, because every style is re-applied on
    // every pass.
    REQUIRE(yoga_layout_tree_build_count() == 1);
    REQUIRE_THAT(root->child_at(3)->bounds().height, WithinAbs(40.0f, 0.01f));
    REQUIRE_THAT(root->child_at(4)->bounds().y,
                 WithinAbs(root->child_at(3)->bounds().y + 40.0f, 0.01f));
}

TEST_CASE("adding a child forces a fresh Yoga tree", "[view][yoga][layout-perf]") {
    using namespace pulp::view;
    auto root = make_reuse_tree(6);
    ...
    extra->flex().preferred_width  = 40;
    extra->flex().preferred_height = 12;
    auto* extra_ptr = extra.get();
    root->add_child(std::move(extra));
    root->layout_children();

    REQUIRE(yoga_layout_tree_build_count() == 2);
    REQUIRE_THAT(extra_ptr->bounds().height, WithinAbs(12.0f, 0.01f));
}

TEST_CASE("live label text re-measures through a reused Yoga node",
          "[view][yoga][layout-perf]") {
    using namespace pulp::view;
    auto root = std::make_unique<View>();
    root->set_bounds({0, 0, 400, 300});
    // A row so the label's main-axis size is its measured width rather than
    // the stretched cross-axis size of the root.
    root->flex().direction = FlexDirection::row;
    root->flex().align_items = FlexAlign::start;
    auto label = std::make_unique<Label>("short");
    ...
    root->layout_children();
    REQUIRE(yoga_layout_tree_build_count() == 1);
    const float narrow = label_ptr->bounds().width;
    REQUIRE(narrow > 0.0f);

    // Yoga caches a measured leaf's size against its constraints. A reused
    // node keeps that cache, so the measure inputs are what decide whether it
    // is still valid.
    label_ptr->set_text("a considerably longer run of text");
    root->layout_children();

    REQUIRE(yoga_layout_tree_build_count() == 1);
    REQUIRE(label_ptr->bounds().width > narrow);
}
