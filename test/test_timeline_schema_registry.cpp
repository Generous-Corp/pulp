...
template <typename T, typename E> T take_value(runtime::Result<T, E> result) {
    REQUIRE(result.has_value());
    return std::move(result).value();
}

runtime::Result<SchemaWriteSuccess, PersistenceError>
replace_version(std::string_view source, BoundedJsonSink& output, const void* context) noexcept {
...
MigrationStep migration(std::uint32_t from, std::uint32_t to) {
    return {from, to, std::make_shared<const std::pair<std::uint32_t, std::uint32_t>>(from, to),
            replace_version};
}
...
runtime::Result<SchemaWriteSuccess, PersistenceError>
scripted_migration(std::string_view, BoundedJsonSink& output, const void* context) noexcept {
...
TEST_CASE("Built-in command registry covers the complete mutation variant") {
    constexpr std::array expected{
        ...
        "pulp.timeline.command.move_device",
        ...
        "pulp.timeline.command.retarget_device",
...
TEST_CASE("Timeline schema builder rejects ambiguous registrations") {
    ...
    fields.fields = {{"same", SchemaValueKind::String}, {"same", SchemaValueKind::U32}};
    ...
    codec.codec.decode =
        [](const JsonValue&,
           const void*) noexcept -> runtime::Result<std::shared_ptr<const void>, PersistenceError> {
...
TEST_CASE("Timeline schema migrations are bounded and verify every envelope") {
    ...
    constexpr std::string_view source = R"({"data":{},"type_name":"vendor.migrating","version":1})";
    auto upgraded = built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 3, source);
    REQUIRE(upgraded.has_value());
    REQUIRE(upgraded.value().find("\"version\":3") != std::string::npos);
    auto downgraded =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 3, 1, upgraded.value());
    ...
    auto bounded =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 3, source, one_step);
    REQUIRE_FALSE(bounded.has_value());
    REQUIRE(bounded.error().code == PersistenceErrorCode::LimitExceeded);

    auto missing = built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 4, source);
    ...
    auto no_op_oversized =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 1, source, undersized);
    REQUIRE_FALSE(no_op_oversized.has_value());
    REQUIRE(no_op_oversized.error().code == PersistenceErrorCode::LimitExceeded);
    auto stepped_oversized =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 2, source, undersized);
    ...
    auto empty_no_op =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 1, {}, zero);
    REQUIRE_FALSE(empty_no_op.has_value());
    REQUIRE(empty_no_op.error().code == PersistenceErrorCode::InvalidJson);
    auto zero_rejects_nonempty =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 1, "x", zero);
    REQUIRE_FALSE(zero_rejects_nonempty.has_value());
    REQUIRE(zero_rejects_nonempty.error().code == PersistenceErrorCode::LimitExceeded);

    auto malformed =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 2, "{broken");
    ...
    auto duplicate_source =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 2, duplicate);
    ...
    auto non_object = built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 1, "[]");
    REQUIRE_FALSE(non_object.has_value());
    REQUIRE(non_object.error().code == PersistenceErrorCode::InvalidSchema);

    constexpr std::string_view wrong_type = R"({"data":{},"type_name":"vendor.other","version":1})";
    auto mismatched_type =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 1, wrong_type);
    ...
    auto mismatched_version =
        built.value().migrate(SchemaDomain::Content, "vendor.migrating", 1, 2, wrong_version);
...
TEST_CASE("Built-in timeline schemas require explicit registration") {
    ...
    REQUIRE(builtins.value().find(SchemaDomain::Content, "pulp.timeline.content.notes") != nullptr);
    const auto* sequence = builtins.value().find(SchemaDomain::Document, "pulp.timeline.sequence");
    REQUIRE(sequence != nullptr);
    const auto required = [&](std::string_view name) {
        const auto field =
            std::find_if(sequence->fields.begin(), sequence->fields.end(),
...
TEST_CASE("Timeline migrations reject non-exact envelopes before the next callback") {
    constexpr std::string_view source = R"({"data":{},"type_name":"vendor.spy","version":1})";
    ...
    initial_spy->output = R"({"data":{},"type_name":"vendor.spy","version":2})";
    ...
    for (const auto& candidate : invalid_initial) {
        initial_spy->calls = 0;
        auto result =
            initial_registry.migrate(SchemaDomain::Content, "vendor.spy", 1, 2, candidate);
    ...
    for (const auto& candidate : invalid_intermediate) {
        ...
        second->output = R"({"data":{},"type_name":"vendor.spy","version":3})";
        SchemaRegistryBuilder builder;
        auto schema = type("vendor.spy", 3);
        schema.upgrades = {{1, 2, first, scripted_migration}, {2, 3, second, scripted_migration}};
        REQUIRE(builder.register_type(std::move(schema)).has_value());
        auto registry = take_value(std::move(builder).build());
        auto result = registry.migrate(SchemaDomain::Content, "vendor.spy", 1, 3, source);
        REQUIRE_FALSE(result.has_value());
...
TEST_CASE("Timeline migration callbacks cannot grow output beyond their bounded sink") {
    constexpr std::string_view source = R"({"data":{},"type_name":"vendor.hostile","version":1})";
    ...
    auto result = registry.migrate(SchemaDomain::Content, "vendor.hostile", 1, 2, source, limits);
    REQUIRE_FALSE(result.has_value());
