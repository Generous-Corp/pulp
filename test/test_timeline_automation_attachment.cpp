...
TEST_CASE("Project identities describe automation lane and point ownership") {
    ...
    REQUIRE(point_location->automation_lane_id == ItemId{31});
}
...
TEST_CASE("Project remap preserves automation tombstones and their owners") {
    ...
    identities.push_back({{51}, {ItemKind::AutomationLane, {3}, {10}, {}, false, {51}}});
    identities.push_back({{52}, {ItemKind::AutomationPoint, {3}, {10}, {}, false, {51}}});
    project = take(
    ...
    REQUIRE(lane_tombstone->automation_lane_id == *remapped.ids.find({51}));
    REQUIRE(point_tombstone.has_value());
    REQUIRE_FALSE(point_tombstone->active);
    REQUIRE(point_tombstone->automation_lane_id == *remapped.ids.find({51}));
}
