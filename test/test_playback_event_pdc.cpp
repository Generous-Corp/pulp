...
TEST_CASE("a compensating shift fails closed on a host-beat-mapped range", "[event-pdc]") {
    ...
    // Control first, and on its own renderer: a shift is latched on first use
    // and a changed one is held until the transport stops, so running the
    // control through `renderer` would latch a zero and the compensating call
    // below would be held rather than refused. The control says the same
    // host-mapped range renders normally when nothing asks it to compensate,
    // so the refusal that follows is about the shift and not about the range.
    ArrangementNoteRenderer uncompensated({10});
    REQUIRE(uncompensated.prepare(256));
    const auto control = uncompensated.process(block, snapshot, {});
    REQUIRE(control.code == NoteRenderCode::Ok);

    const auto result = renderer.process(block, snapshot, {256});
    REQUIRE(result.code == NoteRenderCode::CompensationUnsupported);
    REQUIRE(result.applied_shift.samples == 256);
}
