...
TEST_CASE("mac GPU resize keeps a pinned root and presents before returning",
    ...
    const auto live_resize = pt::simulate_live_resize(*host, 400.0f, 300.0f);
    REQUIRE(live_resize.resize_applied);
    CHECK(live_resize.display_sync_before);
    CHECK_FALSE(live_resize.display_sync_during);
    CHECK_FALSE(live_resize.display_sync_after_resize);
    CHECK(live_resize.display_sync_after);
