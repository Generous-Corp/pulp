...
TEST_CASE("parse_template_areas: double-quoted rows are accepted",
          "[view][bridge][css][issue-1434-grid]") {
    // CSS spec accepts both single and double quotes for grid-template-areas.
    // Many @pulp/react / web-compat callers pass double-quoted strings.
    auto areas = GridStyle::parse_template_areas(
        "\"header header\" \"main aside\"");
    // Three named areas: header (spans 2 cols on row 1), main, aside.
    REQUIRE(areas.size() == 3);
    auto find = [&](const std::string& n) -> const GridStyle::NamedArea* {
        for (const auto& a : areas) if (a.name == n) return &a;
        return nullptr;
    };
    auto* header = find("header");
    REQUIRE(header != nullptr);
    REQUIRE(header->row_start == 1);
    REQUIRE(header->col_start == 1);
    REQUIRE(header->row_end == 2);
    REQUIRE(header->col_end == 3);
    auto* main = find("main");
    REQUIRE(main != nullptr);
    REQUIRE(main->row_start == 2);
    REQUIRE(main->col_start == 1);
    REQUIRE(main->row_end == 3);
    REQUIRE(main->col_end == 2);
    auto* aside = find("aside");
    REQUIRE(aside != nullptr);
    REQUIRE(aside->row_start == 2);
    REQUIRE(aside->col_start == 2);
    REQUIRE(aside->row_end == 3);
    REQUIRE(aside->col_end == 3);
}
