...
TEST_CASE("logical-edge unset slots don't override per-side margin/padding",
    ...
    // must not zero out the left edge. The apply_logical_margin lambda
    // gates on Dimension::is_set, which defaults to false for unset
    // logical-edge slots (so the per-side margin_left wins).
    ScriptEngine engine;
    View root;
    root.set_bounds({0, 0, 400, 100});
    StateStore store;
...
// Codex PR #1562 P1 (comment 3196980951) — "Honor explicit zero for
// logical start/end edges". Before the fix, apply_logical_margin gated
// on `value != 0`, so an explicit `margin_start: 0` (or padding_start /
// start = 0) was indistinguishable from "unset" and the dispatcher
// dropped it. That left a previously-set per-side / uniform value
// active, which contradicts CSS / RN logical-edge semantics where the
// start/end pair WINS for the resolved start/end side.
//
// The fix: Dimension carries an `is_set` flag; the bridge sets it
// whenever the user writes the field; the dispatcher gates on `is_set`
// instead of `value != 0`. An explicit 0 now reaches Yoga and clears
// the resolved start-side margin / padding / position.
TEST_CASE("logical-edge explicit zero clears earlier per-side margin (LTR)",
          "[view][bridge][issue-1542]") {
    // Setup: child has margin_left: 12 (per-side). Then margin_start: 0
    // is set explicitly. Under LTR, start == left, so the resolved
    // left-side margin must be 0, not 12.
    ScriptEngine engine;
    View root;
    root.set_bounds({0, 0, 400, 100});
    StateStore store;
    ...
    bridge.load_script(R"(
        ...
        setFlex('c', 'height', 50);
        setFlex('c', 'margin_left',  12);
        setFlex('c', 'margin_start', 0);
    )");
    // Force LTR so start resolves to the left edge.
    bridge.widget("p")->flex().writing_direction =
        pulp::view::FlexStyle::WritingDirection::ltr;
    root.layout_children();
    auto pb = bridge.widget("p")->bounds();
    auto cb = bridge.widget("c")->bounds();
    // With the fix: child sits flush against the parent's left edge
    // (margin_start: 0 wins over margin_left: 12). Without the fix:
    // child.x - parent.x = 12.
    REQUIRE_THAT(cb.x - pb.x, WithinAbs(0.0f, 0.5f));
}

TEST_CASE("logical-edge explicit zero clears earlier uniform padding (LTR)",
          "[view][bridge][issue-1542]") {
    // Setup: parent has uniform padding: 20, then padding_start: 0
    // explicitly. Under LTR, start == left, so the first child sits
    // flush against the parent's left edge (no left padding) but still
    // gets the right/top/bottom 20px padding.
    ScriptEngine engine;
    View root;
    root.set_bounds({0, 0, 400, 200});
    StateStore store;
    ...
    bridge.load_script(R"(
        ...
        setFlex('p', 'width',  400);
        setFlex('p', 'height', 200);
        setFlex('p', 'padding', 20);
        setFlex('p', 'padding_start', 0);
        createPanel('c', 'p');
        setFlex('c', 'width',  60);
        setFlex('c', 'height', 50);
    )");
    ...
    auto pb = bridge.widget("p")->bounds();
    auto cb = bridge.widget("c")->bounds();
    // With the fix: child.x = parent.x + 0 (padding_start: 0 wins over
    // uniform padding: 20). Without the fix: child.x = parent.x + 20.
    REQUIRE_THAT(cb.x - pb.x, WithinAbs(0.0f, 0.5f));
}

TEST_CASE("logical-edge explicit zero on RTL clears right-side per-side margin",
          "[view][bridge][issue-1542]") {
    // Mirror coverage: in RTL, start == right. So margin_right: 12
    // followed by margin_start: 0 must clear the right-side margin.
    // Child should sit flush against the parent's right edge.
    ScriptEngine engine;
    View root;
    root.set_bounds({0, 0, 400, 100});
    StateStore store;
    ...
    bridge.load_script(R"(
        ...
        setFlex('c', 'width',  60);
        setFlex('c', 'height', 50);
        setFlex('c', 'margin_right', 12);
        setFlex('c', 'margin_start', 0);
    )");
    bridge.widget("p")->flex().writing_direction =
        pulp::view::FlexStyle::WritingDirection::rtl;
    root.layout_children();
    auto pb = bridge.widget("p")->bounds();
    auto cb = bridge.widget("c")->bounds();
    // With the fix: child.right = parent.right (margin_start: 0 wins
    // over margin_right: 12 because start==right under RTL). Without
    // the fix: child.right = parent.right - 12.
    float child_right_offset = pb.right() - cb.right();
    REQUIRE_THAT(child_right_offset, WithinAbs(0.0f, 0.5f));
}

TEST_CASE("logical-edge explicit zero on absolute position clears per-side left",
          "[view][bridge][issue-1542]") {
    // Setup: absolute child with View::set_left(30) then `start: 0`
    // via setFlex. Under LTR, start == left, so the resolved left
    // position is 0, not 30. View's per-side left/right/top/bottom
    // positions are set via View::set_left / set_right / etc., not
    // through setFlex (the bridge only exposes start/end on setFlex
    // for logical edges; the per-side keys live on View itself).
    ScriptEngine engine;
    View root;
    root.set_bounds({0, 0, 400, 200});
    StateStore store;
    ...
    bridge.widget("p")->flex().writing_direction =
        pulp::view::FlexStyle::WritingDirection::ltr;
    bridge.widget("c")->set_position(View::Position::absolute);
    bridge.widget("c")->set_left(30.0f);
    root.layout_children();
    auto pb = bridge.widget("p")->bounds();
    auto cb = bridge.widget("c")->bounds();
    // With the fix: child sits at parent.x (start: 0 wins). Without:
    // child.x = parent.x + 30.
    REQUIRE_THAT(cb.x - pb.x, WithinAbs(0.0f, 0.5f));
}
