// Tests for the shared square-wave oscillator bank behind the metallic 808
// voices (cymbal, open/closed hat, cowbell). The bank's whole job is to turn a
// list of fixed inharmonic frequencies into a clangorous tone, so the tests are
// spectral: render the bank, take the spectrum, and prove that every configured
// oscillator shows up as its own partial, that the summed output stays inside
// its declared bound, and that the band-limiting keeps folded aliases far below
// the signal. A per-sample allocation probe holds the audio-thread contract.
//
...
TEST_CASE("SquareOscBank recovers every configured partial", "[signal][square-osc-bank]") {
    // A six-oscillator inharmonic cluster like the metallic voices use.
...
TEST_CASE("SquareOscBank recovers a two-oscillator cowbell pair", "[signal][square-osc-bank]") {
    // The cowbell is two squares; both fundamentals must be present.
