...
TEST_CASE("Forge modulation: non-finite flanger waveform injection stays finite",
    ...
    const auto render = [&](float value) {
        auto fx = Fixture(mod::flanger::make_flanger_node(), kSr, kFrames);
        auto inj = fx.claim_injector();
        REQUIRE(inj.inject(immediate(mod::flanger::kWave, value)) == InjectStatus::Ok);
        return fx.settle({t, t}, 8);
    };
    const auto out = render(std::numeric_limits<float>::quiet_NaN());
    REQUIRE(out == render(0.0f));
    for (const auto& channel : out)
...
TEST_CASE("Forge modulation: flanger latency controls are frozen realizations",
    ...
    const double adjacent_offset = std::nextafter(4.0, 5.0);
    REQUIRE(mod::flanger::type_id(Mode::through_zero, 4.0) !=
            mod::flanger::type_id(Mode::through_zero, adjacent_offset));
    REQUIRE(mod::flanger::make_flanger_node(Mode::through_zero, adjacent_offset).type_id ==
            mod::flanger::type_id(Mode::through_zero, adjacent_offset));
    REQUIRE(mod::flanger::type_id(Mode::through_zero, 1000.0) ==
            mod::flanger::make_flanger_node(Mode::through_zero, 1000.0).type_id);
