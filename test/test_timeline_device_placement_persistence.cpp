...
TEST_CASE("Timeline device chains round trip in authored order") {
    ...
    REQUIRE(encoded.json.find("\"type_name\":\"pulp.timeline.track\",\"version\":2") !=
            std::string::npos);
...
TEST_CASE("Timeline permanent fixtures cover Track versions one and two") {
    ...
    REQUIRE(upgraded.json.find("\"type_name\":\"pulp.timeline.track\",\"version\":2") !=
            std::string::npos);
